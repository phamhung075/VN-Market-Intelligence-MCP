...
**BUILD STATUS 2026-05-30T11:30Z — ✅ SIGNED OFF (AR-EXIT, APPROVE-WITH-CONDITIONS). Pivoted to Option-Y (architect §0.7): orchestration moved to the host fleet-cron CC session; mcp-server reduced to a pure data service. QA cycle-153 GREEN on all 7 §0.7.5 DV gate items via live FPT+ACB bake-off at HEAD `3b4c62a2` (100/100 AR tests, tsc clean). PO independently verified: in-container cron removed, host cron skill armed `'0 9,14,20 * * *'` UTC, tools #141-144 registered, `spawn("claude")` deleted, PEK subtree 0-diff. ONE follow-up seeded (AR-FU-DETERMINISM, MEDIUM): the idempotent store is stable, but Haiku refine fan-outs emit non-deterministic markdown coverage (FPT run-1=91 vs run-2=18 rows) — store-correctness unaffected, coverage variance is a trust follow-up since refined rows feed the 6 expert passes. Optional/future: Mistral OCR bake-off swap (user-LOCKED later swap, not a gap).** Original kickoff (HISTORICAL): the architectural root-cause replacement for the recurring BCTC over-merge bug. The user complaint that started it: *"it merges all tables not correct."* We have shipped four sprints against the geometry middle (BCTC-MD-TABLE → BCTC-LAYOUT-FIRST → PEK-INTEGRATE → BCTC-TABLE-BOUNDARY, just signed off 2026-05-30). The boundary fix kept the system honest meanwhile, but the **root cause is the deterministic geometry middle itself** — DocLayout-YOLO page-type-from-image-with-no-text + the `bctc_page_grouper.py` 5-state machine that stitches by geometry. This sprint REPLACES that middle OUTRIGHT with an **agent refine step** that reads OCR text + crisp page images together and produces TRUSTED content the existing expert analyst flow consumes unchanged.
...
<!-- HISTORICAL: 2026-05-29T22:16Z — OPEN (PO INCIDENT TRIAGE). Ops fully diagnosed a multi-source data-pipeline failure: live-probed + VPS-audited. VPS infra HEALTHY — all 5 push pipelines HTTP 200. 4 CODE bugs root-caused. -->

> Goal-ARMED clause below stays as the standing DoD; it is now MET for DPI-1/2/2b and CODE-MET/live-pending for DPI-3/4 (FU-MON flips). Retained for FU-MON reference.
...
**BUILD STATUS 2026-05-27T20:52Z — DONE (build) / PO SIGNED OFF; goal ARMED on USER comprehensibility only (verbal G9).** PO sign-off at NEWS-CMD-EXIT 2026-05-27T20:52:50Z — build deliverable complete, every machine-checkable Success-Metric AC MET (PO independently re-verified LIVE in-container against `market.db`: `/news` returns plain-Vietnamese digest `Tin tức hôm nay (N bài):`, sentiment-as-words, zero jargon, chunked ≤4096, `Chưa có tin hôm nay.` empty-fallback + `Tin tức gần đây` recent-fallback header all live). The ONLY remaining axis is subjective USER comprehensibility (verbal G9) — human-judged forever (lane-c). One LOW follow-up backlogged: NEWS-CMD-HTML-STRIP (raw HTML in ingested `summary`, spec-conformant per REQ § 8, did NOT block). Original kickoff: PO from an EXPLICIT USER FEATURE REQUEST — full autonomy. User (non-technical, French-based, verbatim broken English): *"make /news on telegram command for get all content if user need"*. PO interpretation: an **on-demand Telegram pull command** `/news` for the full current news digest — "all content they need", not a teaser. User-pull request/response, NOT a scheduled push.
