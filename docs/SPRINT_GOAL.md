...
- 🔴 **AUD-ND-1 (agents-architect → agent-father) — CRITICAL, SHIPS FIRST IN THIS SPRINT.** system-auditor flow MUST be detect/PLAN-ONLY: forbid ALL destructive/runtime-mutating shell ops (docker stop/kill/rm/restart, compose down, kill, rm -rf of live dirs). Any "remediation" emits a signal/DASHBOARD row, never acts. Add explicit "NEVER run destructive docker/kill/rm" invariant to auditor .md + guard/allowlist narrowing effective Bash to read-only probes. AC: simulated ENOSPC false-positive → DASHBOARD/signal row + ZERO infra mutation; invariant present in flow; regression note links incidents. Route po→architect→agent-father.
  - **Trigger 1 (downtime):** 2026-05-31 21:08Z P0 false-positive A-30-HOST-DISK ENOSPC → auditor `docker stop` mcp-server → ~9–129min outage (9c381ed3 + Telegram report 3016).
  - **Trigger 2 (DATA LOSS — escalation 2026-06-01):** false-positive auditor `docker stop`-ped mcp-server during Monday 2026-06-01 TRADING HOURS (~09:00→15:19 UTC). Price pipeline is live-only with NO backfill → Monday's intraday market dataset was PERMANENTLY DESTROYED (only the close survived). 3rd fleet-danger manifestation; severity graduated downtime → irreversible data loss. This is why AUD-ND-1 is now CRITICAL and jumps ahead of all MEDIUM tool-surface work. Context: auditor-incident memory + DASHBOARD signals.
  - **AC addendum (data-loss):** the deliberate-violation regression must assert ZERO destructive op AND must be run/verified during a simulated market-open window (the live-only-no-backfill property makes any trading-hours stop unrecoverable).
