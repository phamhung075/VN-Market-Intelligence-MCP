...
**STATUS 2026-05-31T11:30Z — OPEN (PO self-initiated, SPLIT from FU-TRUST-REFRESH per recurring-bug escalation). Priority: HIGH.** Zone: `apps/mcp-server/` (consumers) — architect to confirm whether any pdf-extractor extraction-tagging is in scope. Brief: PENDING (architect FIRST — this is a MANDATORY root-cause escalation, NOT a point-fix). Trigger: the bank-vs-corporate consumer-assumption class was patched 3× in FU-TRUST-REFRESH (FU-6d aggregator, FU-6f B-1 eval anchors, FU-6f B-3 PUB-3) and STILL blocks ACB `get_bctc_full` — exactly the `feedback_recurring_bug_escalation` (≥2 fix commits same class → architect root-cause before any new fix) + `feedback_silent_swallow_serial_bugs` (one-pass enumeration, fail-loud, stop discovering sites one rebuild at a time) trigger.
