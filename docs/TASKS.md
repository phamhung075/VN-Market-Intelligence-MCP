...
## mcp-server SCALE Pilot — Phase-0 CLOSED / Phase-1 READY-SOLO-DEFERRED / SEQUENCED LAST (CLOSED 2026-05-25T10:50Z)

**Status:** Phase-0 **CLOSED** + phase advanced **0→1** (PO P0-MCP-EXIT sign-off 2026-05-25T10:50:43Z). All 4 Phase-0 deliverables DONE (commits verified in git log): P0-MCP-1 brownfield `44530a26` (verdict FULL, 12 barrels, 146 tools), P0-MCP-2 bug-inventory `05dc494a` (~9408 pass/~348 fail/35 skip; 7 open bugs), P0-MCP-3 dev-mcp-server flow G12 gate `0a4f1f28`, P0-MCP-5 Phase-1 plan `7d78abb1` (FULL, 10 tasks, 78 ACs). P0-MCP-4 anchor tag **`mcp-server-pre-refactor` @ `7d78abb1`** created (local-only, no push). SSOT: `docs/data/pilot-status-mcp-server.json` (status=ACTIVE; phase=1; phase0.status=CLOSED; exit_gate=CLOSED; phase1.status=ACTIVE; `sequencingGate.decision=PHASE0-CLOSED-PHASE1-READY-SOLO-DEFERRED`). **The mcp-server BUILD wave (WAVE B / dev-mcp-server) is READY-SOLO-DEFERRED** — Phase-1 plan locked + SOLO conditions met host-side, but P1-A is NOT dispatched this cycle: every wave is QA-gated against the live server + finalized by a docker rebuild, both of which belong in the SEPARATE docker session (memory cap). This is the FINAL pilot — closes the 2026-05-22 rollout at 11/11.
...
**Phase-1 backlog (READY-SOLO-DEFERRED — plan locked `7d78abb1`; WIP=1 strictly sequential; dispatch P1-A only when the SEPARATE docker session opens, no other dev/scale terminal touching the mcp zone). Anchor: `mcp-server-pre-refactor` @ `7d78abb1`. Full ACs: `docs/architecture-briefs/2026-05-22-refactor/scale/mcp-server-phase-1-task-plan.md`.**

| Task ID | Title | Goals | Owner | Status | Blocked by |
|---------|-------|-------|-------|--------|-----------|
| P1-A | Sandbox runner + scenario scaffolding (sparkline primitive) | G1, G7 | dev-mcp-server | **READY-DEFERRED** (first task; dispatch when SOLO docker session opens) | — |
| P1-B | Three-tier trust dashboard stub (**G12 streak #1**) | G6, G8, G9, G12 | dev-mcp-server | BLOCKED | P1-A |
| P1-C | Barrel wave 1: `system/` 21→5 sub-barrels (**G12 streak #2**); tag `mcp-server-pre-barrel-wave1` | G2, G12 | dev-mcp-server | BLOCKED | P1-B |
| P1-D | Barrel wave 2: `macro/` 14→HTTP-proxy vs local-computation (**G12 streak #3**); tag `mcp-server-pre-barrel-wave2` | G2, G12 | dev-mcp-server | BLOCKED | P1-C |
| P1-E | Barrel wave 3: `sector/` 15→3 cluster cuts; tag `mcp-server-pre-barrel-wave3` | G2 | dev-mcp-server | BLOCKED | P1-D |
| P1-F | **G5-inverse remediation (R-CRITICAL):** kinhDichWrapper bypass in `marketTools.ts` + `news-analysis/analysis.ts` + `portfolioTools.ts` QUE_META import; tag `mcp-server-pre-g5-remediation`; every-handler-HTTP-routed evidence | G5 | dev-mcp-server | BLOCKED | P1-E |
| P1-G | **G5-inverse remediation (R-MEDIUM):** `pdf.ts`/`pdfOcrWorker.ts` post-1954c verify (move to `_deprecated/` or annotate KEEP) | G5 | dev-mcp-server | BLOCKED | P1-F |
| P1-H | G1 primitive scaffolding: signal-bus-helper + sector-classifier + scenarios | G1, G7 | dev-mcp-server | BLOCKED | P1-E (barrel stability) → after P1-G in chain |
| P1-QA | Phase-1 close-gate verification (tsc, bun test ≥9408/≤348, tool 146, scheduler 68, G12 streak 3/3, G5-inverse evidence, dashboard) | G1,G2,G5,G6,G7,G8,G12 | qa | BLOCKED | P1-H |
| P1-EXIT | PO: SSOT reconciliation + pilot-status Phase-1 close | — | po | BLOCKED | P1-QA |
...
- **WAVE B — mcp-server build SOLO (per its Phase-1 plan). STATUS: READY-SOLO-DEFERRED (2026-05-25T10:50Z).** All three dispatch preconditions now MET: (i) WAVE A's build wave quiet, (ii) 3-condition gate cleared (frontend Phase 0→1 done + mcp zone quiesced + WIP free), (iii) **mcp-server Phase 0 opened+CLOSED this rollout** (P0-MCP-EXIT 2026-05-25T10:50Z; anchor `mcp-server-pre-refactor` @ `7d78abb1`). Owner: `dev-mcp-server`, RUN-SOLO — **no other dev/scale terminal active**. **DEFERRED:** P1-A is NOT dispatched yet — each barrel split is QA-gated against the live full tool suite and the wave closes with a docker rebuild, both of which belong in the SEPARATE docker session (memory cap). Dispatch P1-A first when that solo session opens. Each barrel split QA-gated against the full tool suite. **Gate B→C:** all targeted barrel splits + G5-inverse HTTP-route-proofs landed; full tool suite green; mcp-server sandbox/dashboard green.
