...
|---------|-------|----------|------|-------|---------|------------|
| 1950-T1 | **Sprint 1950 — Chef WORK-channel telemetry on every cycle.** Patch `.claude/flows/unified-agent/chef.md` so every cycle emits WORK Telegram on entry (Step 0) + close (Step 8 success / regime-state-only / FAILED). Wrap Steps 0-7 in `cowork-boundary` skill. Failure path: WORK + BUG one-line, exit non-zero, no retry, no partial MARKET dish. Existing intraday silent-exit line unchanged. Size=S. Zone=`.claude/flows/unified-agent/`. **PRE-CONDITION:** ship before 2026-05-19T05:23Z (first guaranteed dish). **BA spec:** `docs/handoffs/REQ_1950.md` | HIGH | TASK | agent-father | docs/handoffs/TASK_1950-T1.md | — |
| 1950-T2 | **Sprint 1950 — TNB audit gains chef-cycle coverage check.** Add to `tran-ngoc-bau.md` audit scope: verify WORK channel shows ≥3 chef start + ≥3 chef close in last 24h. Missing start = CRITICAL. Start-without-close = FAILED. Size=XS. Zone=`.claude/agents/` + `.claude/flows/tran-ngoc-bau/`. **Depends on:** 1950-T1 (telemetry must exist before audit checks for it). | MEDIUM | TASK | agent-father | docs/handoffs/TASK_1950-T2.md | 1950-T1 |
