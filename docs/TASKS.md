...
| Task ID | Title | Priority | Type | Owner | Handoff | Blocked by |
|---------|-------|----------|------|-------|---------|------------|
| 1870a | VERIFY-HIGH: Confirm FPT BCTC Q4 2025 reparse post-hotfix — force reparse + verify `get_bctc_full('FPT')` returns non-zero revenue within sane magnitude (trillions VND, not quadrillions). Closes open QA checklist in `TASK_hotfix_bctc_parser2`. Bugs fixed: (1) axios `file://` empty text → confidence=0 skip, (2) `detectUnitMultiplier` returned 1000 for raw VND integers, guard only caught `m===1`. Blocks financial-analyst from producing valid FPT signals (cycle 2026-05-11 01:00 UTC still logs `FPT revenue parse=0`; evening report empty). AC: (a) `get_bctc_full('FPT')` revenue > 0 and in trillions VND magnitude, (b) financial-analyst next cycle produces non-partial signal for FPT, (c) TASK_hotfix_bctc_parser2 QA checklist closed. | HIGH | VERIFY | dev-pdf-extractor + qa | TASK_1870a.md | hotfix_bctc_integrity, hotfix_bctc_parser2 |
