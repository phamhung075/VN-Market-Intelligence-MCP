...
| 1899a-factory | SCAFFOLD: Shared infrastructure — PlaywrightBrowserFactory (launch/configure/close Chromium stealth). Zone: apps/news-fetch/. (Tier 2, depends 1899a-core, ~1h). NOTE: evaluate playwright-extra + puppeteer-extra-plugin-stealth as maintained replacement for playwright-stealth 0.0.1 (only published version — see qa-bug signal 2026-05-13). | MEDIUM | FEATURE | developer | TASK_1899a-factory.md | — |
...
| 1899a-core-SHIPPED-c75 | SCAFFOLD **DONE 2026-05-13 c75**: news-fetch service skeleton — cherry-picked `8329294c` (from `120e16ca`) onto main. 3/3 smoke tests pass, TSC clean, DDD PASS (domain/application/infrastructure/interface dirs created, 0 cross-layer imports), security PASS (Bun.env, no process.env, no secrets). playwright-stealth pinned 0.0.1 — signal filed for 1899a-factory (see `docs/signals/qa-bug-playwright-stealth-version-2026-05-13T*.json`). Branch contamination: FlareSolverr adapter (47a85265) + worldbank merge artifact (1e8a707a) excluded via cherry-pick. | MEDIUM | FEATURE | developer + qa | 2026-05-13 |
| 1901a-flaresolverr-adapter-SHIPPED | FIX-MEDIUM **DONE 2026-05-13**: investing-calendar FlareSolverr adapter — cherry-picked `5395f966 feat(macro-indicators): investing-calendar FlareSolverr adapter` + `5ee72b46 chore(infra/flaresolverr): provision CF Turnstile v2 bypass container` onto main. Adds `flaresolverr_helper.py` (shared JSON-RPC client, 25min cf_clearance cache, REDACTED cookie logs), rewrites `investing_calendar_fetch.py` (FlareSolverr fast-path + curl_cffi fallback), touches `investing-economic-calendar.ts` (sleepMs removed), +10 unit tests all green. Tests: 103 pass / 0 fail / 12 skip. TSC: 0 production errors (22 pre-existing Bun Mock typing gap in test files). DDD PASS: 0 domain→infrastructure imports. Security PASS: cf_clearance REDACTED in logs, no process.env, no hardcoded secrets. Branch contamination detected (merge `f77bc3aa` dropped feature files) — recovered via cherry-pick. Ops: rebuild macro-indicators image to activate. | MEDIUM | FIX | dev-mainserver-crawls + qa | 2026-05-13 |
| 1899a-reuters-rss-SHIPPED-c75 | SCAFFOLD **DONE 2026-05-13 c75**: Reuters RSS scraper — merge `ade4a0a8` from `fix/1899a-news-fetch-reuters-rss`. 55/55 tests green (26 new unit tests in 1899a-reuters-rss.test.ts), TSC 0 errors, DDD PASS (scraper imports only domain/repositories.js + domain/models.js), security PASS (no process.env, no secrets, all HTTP mocked via globalThis.fetch). ReutersRssScraper implements ReutersNewsPort: plain fetch() + DOMParser/regex fallback, HIGH confidence, RFC 2822→ISO 8601 date normalization, ~30-50MB RAM. Branch deleted. | MEDIUM | FEATURE | dev-mainserver-crawls + qa | 2026-05-13 |
| 1899a-domain-SHIPPED-c75 | SCAFFOLD **DONE 2026-05-13 c75**: news-fetch domain layer — cherry-picked `d7302f75` (from `b71ba215`) onto main. 16/16 unit tests pass (29 expect() calls), TSC 0 errors, DDD PASS (0 infra imports in domain/), security PASS (pure TS interfaces/enums, no runtime code). NewsSource enum (REUTERS/BLOOMBERG), Article interface (url/publishedAt nullable, confidence HIGH/LOW), FetchResult envelope, ReutersNewsPort + BloombergNewsPort port interfaces. Branch `fix/1899a-news-fetch-domain` deleted. | MEDIUM | FEATURE | developer + qa | 2026-05-13 |
| 1902a-domain-defaults-DONE-pre-c76 | DDD-VIOLATION **DONE pre-c76** (PO discovered c76 c75 backfill): commit `e7a21d60 fix(macro-indicators/arch): move DEFAULT_SYMBOLS + DEFAULT_CNBC_SYMBOLS to domain layer` already shipped. App now imports from `../domain/defaults.js` (line 33). Layer violation closed. Residual leftover: `infrastructure/scrapers/cnbc-world-markets.ts:25` still exports its own `DEFAULT_CNBC_SYMBOLS` copy (now DRY-stale leftover, NOT layering violation) — reclassified as JANITOR-class cleanup if needed. | MEDIUM | CHORE | (auto-discovered) | 2026-05-13 |
