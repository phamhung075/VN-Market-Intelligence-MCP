...
| Task ID | Title | Priority | Type | Owner | Handoff | Blocked by |
|---------|-------|----------|------|-------|---------|------------|
| 1900b-worldbank | FIX-LOW (c73 in-flight, not gated): WorldBank `fetchVnMacroBatch` parallelize — `task/worldbank-parallelize-fetch-vn-macro-batch` HEAD `2c847d8c` carries `7a12913f` fix + docs (same pattern as FRED parallel fix `e777d83e`). qa signal `qa-worldbank-sequential-loop` flagged. Action: qa run merge gate on branch (87+ unit tests + smoke `summary.ok=5/6` expected). After merge → close 1900b. ⚠ branch ALSO carries `0a335b72 pm/1899a decompose` already cherry-picked to main as `ef21a754` → use `--no-ff` merge or rebase to skip duplicate. | LOW | FIX | qa | — | — |
