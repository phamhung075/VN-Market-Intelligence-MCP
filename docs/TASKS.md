...
## Todo
| ARCH-1944 | **Sprint 1944 BLOCKING** — Architect brief: per-zone task split for 1944a (the only `multi` task). 1944a touches `vps-scripts/vps-proxy-server.js` (ops zone) + `apps/mcp-server/src/infrastructure/fetchers/bctcHttpFetcher.ts` (dev-mcp-server zone). Brief outputs two per-zone child tasks (1944a-vps, 1944a-mcp) with explicit handshake order: VPS deploys route first, then mcp-server header injection lands. ≤2 pages. Output: `docs/architecture-briefs/2026-05-18-vps-bctc-discover-route-zone-split.md`. Blocks 1944a only. | HIGH | ARCH | architect | — | — |
| BA-1944a | **Sprint 1944 TIER 1** — Write requirement spec for 1944a after ARCH-1944 lands. Spec covers `/proxy/bctc-discover/:ticker?year=&quarter=` route shape, `X-API-Key` header injection rule in `bctcHttpFetcher`, expected JSON response shape (array of `{filename, url, published_at}`), error handling (401/404/500), test fixtures for live VPS probe + unit-test mock. References SPIKE-1916 + SPIKE-1943 root causes. | HIGH | BA-SPEC | ba | — | ARCH-1944 |
