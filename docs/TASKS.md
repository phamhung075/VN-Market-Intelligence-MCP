...
|---------|-------|----------|------|-------|---------|------------|
| 1950-T1 | **Sprint 1950 — Chef WORK-channel telemetry on every cycle.** Patch `.claude/flows/unified-agent/chef.md` so every cycle emits WORK Telegram on entry (Step 0) + close (Step 8 success / regime-state-only / FAILED). Wrap Steps 0-7 in `cowork-boundary` skill. Failure path: WORK + BUG one-line, exit non-zero, no retry, no partial MARKET dish. Existing intraday silent-exit line unchanged. Size=S. Zone=`.claude/flows/unified-agent/`. **PRE-CONDITION:** ship before 2026-05-19T05:23Z (first guaranteed dish). | HIGH | TASK | agent-father | docs/handoffs/TASK_1950-T1.md | — |
