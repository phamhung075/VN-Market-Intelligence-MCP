...
| 1903-doc-pair | DOC-CHORE PAIRED (TNB c47 rec #1 + #3 bundled): (a) **1903a-labels** — strip stale `[UNVERIFIED — tool not found 2026-05-11]` label on `write_alert_verdict` at `.claude/tools/package/alert-commander.md` L41 (code fix shipped c77/c82 `d5251193`); audit other tool packages for similar stale UNVERIFIED labels referencing now-shipped tools. (b) **1903b-doc-self-heal** — add `get_macro_snapshot` regime-fallback note to `.claude/flows/alert-commander/stage-bootstrap.md` (news-context workaround when canonical snapshot unavailable). Single-PR pair; both 1-line / few-line edits. AC: both files updated, tree-verify PASS, no SSOT drift. Zone: `.claude/` (cross-service). | MEDIUM | CHORE | developer | — | — |
---
