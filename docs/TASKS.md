...
| Task ID | Title | Priority | Type | Owner | Handoff | Blocked by |
|---------|-------|----------|------|-------|---------|------------|
| 1869a | FIX-HIGH: Raise price_drop threshold -5% → -7% in signalDetector.ts. Precision gate (50% → target 60%). Eliminates FP Pattern A borderline drops. 3 files, const change + fixture updates. AC: DEFAULT_DROP_PCT = -7, baseline tests pass (8804), <5 files touched. | HIGH | FIX | developer | TASK_1869a.md | — |
| 1869b | SPRINT-S: Wire per-watchlist thresholds into scanMarket line 283 — pass watchlistThresholds from DB to detectSignals(snapshot, context). 5 files, ~30 lines. AC: custom threshold overrides default, unit test passes, baseline unchanged. Depends on 1869a (logically sequenced). | HIGH | SPRINT-S | developer | TASK_1869b.md | — |
