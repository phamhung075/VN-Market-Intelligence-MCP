...
- 🔄 **DWF-BA** (BA, NEXT): decompose § DYN-WF-FOUNDATION into spec `docs/REQ_DYN-WF-FOUNDATION.md` — Phase 0 (prune dead slots, `routing-policy.json` SSOT, `is_trading_day` tool, per-tick `pressure-state.json` instrument-only) + Phase 2 (leader lock, per-work-item idempotent token, `published:<work-id>` belt); R1/R3 BLOCKING ACs, R2 documented ops invariant; every AC a deliberate-violation proof not "exit 0". → PO review.
- 🐛 **DWF-TSC-DEBT** (tracked, from TRUST-QA): 19 tsc errors in `DWF-routing-policy-fence.test.ts` introduced by commit 8105f8fd (`lastRule` possibly undefined); pre-existing at `caf6865d~1`. Fix within DWF, not BCTC-TRUST-RED.
- ⏳ DWF-ARCH → DWF-PM → DWF-DEV → DWF-QA (sequential, after BA spec): full binding detail (zone split, leader-lock/token seam reusing `task_claim` kind `cowork-slot` no-new-enum, `is_trading_day` contract, `routing-policy.json`/`pressure-state.json` schemas, R1 explicit-TTL + R3 suffix-free key + R2 dark-window runbook, ordering 0→2→1 WIP≤2, DV RED-before/GREEN-after counter-tests, ops rebuild) → `docs/SPRINT_GOAL.md` § DYN-WF-FOUNDATION + brief. → PO sign-off (DWF-EXIT) gates DWF-PHASE1.
