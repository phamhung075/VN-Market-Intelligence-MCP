...
| 1903a-labels | DOC-CHORE (TNB c47 rec #1): Clear `[UNVERIFIED — tool not found 2026-05-11]` label on `write_alert_verdict` in `.claude/tools/package/alert-commander.md` L41. Underlying code fix landed c77/c82 (`d5251193`); only label remains stale. 1-line edit + verify any similar UNVERIFIED labels in other tool packages reference shipped tools. Zone: `.claude/` (cross-service). | MEDIUM | CHORE | developer | — | — |
| 1903b-doc-self-heal | DOC-CHORE (TNB c46→c47 carry rec #3): Apply alert-commander doc_self_heal proposals — add note to `.claude/flows/alert-commander/stage-bootstrap.md` about `get_macro_snapshot` workaround (regime fallback via news context when canonical snapshot unavailable). `.claude/` write-protected in cowork; dev-team applies. Zone: `.claude/` (cross-service). | MEDIUM | CHORE | developer | — | — |
---
