...
**Status:** DECOMPOSED — PM task breakdown complete 2026-05-30. **Priority: CRITICAL (user-facing live market data wrong/missing on 4 surfaces).** Zone: `multi` (`apps/macro-indicators` DPI-1+2 + `apps/mcp-server` DPI-3+4). VPS infra HEALTHY (all 5 pushes HTTP 200); blockers are 4 root-caused CODE bugs. Goal `docs/SPRINT_GOAL.md` § DATA-PIPELINE-INTEGRITY. **Owner chain:** ba → architect → pm → dev-macro-indicators + dev-mcp-server (parallel) → ops (REBUILD mcp-server first, then macro-indicators) → qa (live re-probe) → po (DPI-EXIT). **Rebuild order:** mcp-server FIRST (DPI-3/4 write fresh delta + foreign-flow), then macro-indicators (DPI-1/2 read SBV + fresh timestamps). DoD per task: see handoff ACs; final DoD = live MCP-tool re-probe of all four.

- ✅ DPI-BA: Spec `docs/REQ_DATA-PIPELINE-INTEGRITY.md` — 4 bugs decomposed; DDD layer mapped; zero PO blockers. NEXT: architect.
- ✅ DPI-ARCH (architect): Design complete — `docs/handoffs/DPI-ARCH.md`. Option A canonical SBV FX; SBVRateSQLiteAdapter pattern; DPI-2 inline time.Now(); DPI-3 pre-tx prev-close read + delta; DPI-4 UPSERT + R-1 race fix (server.ts INSERT OR REPLACE destructive). NEXT: pm.
- ✅ **DPI-1** (dev-macro-indicators) [zone: `apps/macro-indicators`]: **FX Canonical Source** — new SBVRateSQLiteAdapter + usecases.go patch + main.go wiring. Handoff: `docs/handoffs/DPI-1.md`. DoD: both `get_macro_snapshot` and `get_cycle_bootstrap` USDVND values identical (live). **QA PASS 2026-05-30T00:21Z — both surfaces return 26115 (SBV official, NOT Yahoo 26255). AC-6 GATE PASS.**
- ✅ **DPI-2** (dev-macro-indicators) [zone: `apps/macro-indicators`]: **Carry/Yield ComputedAt** — delete `fixtureComputedAt` const, inline `time.Now().UTC().Format(time.RFC3339)` in Execute(). Handoff: `docs/handoffs/DPI-2.md`. DoD: carry.computedAt & yield.computedAt reflect today (not 2026-05-23). **QA PASS 2026-05-30T00:21Z — carry.computedAt=2026-05-29T23:20:58Z, yield.computedAt=2026-05-29T23:20:58Z. NOT 2026-05-23. AC-6 GATE PASS.**
- ✅ **DPI-2b** (dev-macro-indicators) [zone: `apps/macro-indicators`]: **Carry/Yield LIVE Inputs** — retire frozen fixtures (4.7/5.33/8.2) as PRIMARY; wire 3 live sources from shared market.db via new `CarryYieldInputsSQLiteAdapter` (mirrors DPI-1 pattern). PO-VERIFIED all 3 live-wirable, ZERO documented-gaps: VND deposit=`sbv_rates.max_deposit_rate_pct`, Fed funds=`fred_series_daily` EFFR-latest, earning yield=`tracked_indicators` market_earning_yield-latest. Fixtures KEPT as explicit safe-degrade fallback only (not deleted — re-inlining a literal would re-hardcode). Handoff: `docs/handoffs/DPI-2b.md`. DoD: live DB rows drive carry/yield regime (AC-6 regime-flip DV proves not-frozen) + AC-7 live re-probe cross-checked vs direct market.db read. REBUILD: after DPI-1/2 (same macro-indicators container). **QA PASS 2026-05-30T00:21Z — vndDepositRate=5 (LIVE, matches sbv_rates row); fedFunds=5.33 (SAFE-DEGRADE: EFFR row date=2026-05-14 genuinely stale >96h); earningYield=8.2 (SAFE-DEGRADE: tracked_indicators empty). Adapter staleness machinery PROVEN working (deposit wired live 5≠fixture 4.7). AC-7 GATE PASS.**
- 📋 **DPI-3** (dev-mcp-server) [zone: `apps/mcp-server`]: **Brent/Gold Delta** — prev-close lookup + computeDelta() helper + updated upsertMacroPrice ON CONFLICT + call-site updates. Handoff: `docs/handoffs/DPI-3.md`. DoD: `get_market_snapshot` BRENT & GOLD change_pct non-zero & directional. REBUILD ORDER: DPI-3 BEFORE DPI-1.
- 📋 **DPI-4** (dev-mcp-server) [zone: `apps/mcp-server`]: **Foreign-Flow UPSERT + R-1 Race Fix** — ohlcvForeignFlowStore.ts UPSERT + server.ts:1078 INSERT OR REPLACE→ON CONFLICT + taOhlcvBackfillJob.ts audit. Handoff: `docs/handoffs/DPI-4.md`. DoD: `get_foreign_flow(HPG)` populated + in-container DB count >0 + stub-row race test PASS. CRITICAL R-1 race-fix in scope (server.ts destructive write must be fixed same task). REBUILD ORDER: DPI-4 WITH DPI-3 (parallel mcp-server).
- 🔄 DPI-OPS (ops): REBUILD sequence — (1) mcp-server `build` + `up -d --no-deps --force-recreate` (DPI-3+4 live), (2) macro-indicators `build` + `up -d --no-deps --force-recreate` (DPI-1+2 read SBV). NOT restart. `feedback_rebuild_after_dev_change`.
- 🔄 DPI-QA (qa): live re-probe all four MCP tools post-rebuild; verify per task AC; DPI-4 race gate via direct DB count (not push echo). PROVEN-RED→GREEN.
