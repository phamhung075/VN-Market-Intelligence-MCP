...
|---------|-------|----------|------|-------|---------|--------|-----------|
| PI-1 | Design served viewer: 3 GET routes + PDF→file mapping (real unknown) + data-source ruling + render approach + DDD placement + SI-2 boundary. DESIGN ONLY. | HIGH | TASK | architect | docs/handoffs/TASK_PDF-INSPECT.md | DONE | — |
| PI-2 | Implement served viewer per PI-1: 3 GET routes (list / pdf-bytes / extracted) + viewer page (LEFT=pdf.js render, RIGHT=text/tables) + honest-degrade. Sandbox surface untouched; fence green; pytest green. | HIGH | TASK | dev-pdf-extractor | docs/handoffs/TASK_PDF-INSPECT.md | DONE | — |
| PI-3 | Verify under USER's real served-URL-in-browser path (L9): list→select→PDF-left/text-right side-by-side; honest-degrade; SI-2/pilot freeze not regressed; smoke green. Emit `qa-pdf-inspect-<UTC>.json`. | HIGH | TASK | qa | docs/handoffs/TASK_PDF-INSPECT.md | DONE | — |
| PI-EXIT | PO sign-off against user acceptance condition + PI-3 ACs | CRITICAL | GATE | po | docs/handoffs/TASK_PDF-INSPECT.md | DONE | — |

**PI-EXIT sign-off (2026-05-24T17:47Z):** RATIFIED. User acceptance condition MET — QA verified under the REAL served URL (`http://localhost:15001/inspect`, Playwright headless): list populates → select doc → LEFT = pdf.js-rendered original, RIGHT = extracted text/tables/confidence side-by-side; VNM decimal-shift bug (`net_profit 0.000051`) visible beside the rendered page (the literal user goal). PI-3 ACs 1-6 all PASS. Honest-degrade fires (amber PDF/extraction-missing, 404s, UUID-400 gate, path-traversal rejected). 186/186 pytest, Fence-A/B KEPT, frozen sandbox/dashboard/pilot-status untouched (empty diff). Commit chain: PI-1/PI-2 `4651c080` + PI-3 `0d10f310`, both zero-foreign-file. Deployment: SERVED surface on the running pdf-extractor service — `http://localhost:5001/inspect` once the container is up on next `docker compose up` (new routes require the container to reload the updated image/code; ops dispatch needed if the running container predates `4651c080`). **PILOT UNTOUCHED** — pdf-extractor SCALE pilot stays DONE 12/12 frozen; `pilot-status-pdf-extractor.json` not edited.
