...
| ARCH-1944 | **DONE 2026-05-18** — Brownfield audit: VPS route + API key injection already landed (commits 1b8f8cd5, 8f9c2d55). Real blocker = response shape mismatch: vps-proxy-server.js L329 returns bare `string[]` but `extractVpsPlaywrightUrls()` expects `{results:[{url,source,confidence}],error}` envelope. Fix scoped to 1944a-vps (shape wrap) + 1944a-mcp (integration test). Brief: `docs/architecture-briefs/2026-05-18-vps-bctc-discover-route-zone-split.md`. | HIGH | ARCH | architect | 2026-05-18 |
