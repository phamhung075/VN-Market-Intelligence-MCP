...
|---------|-------|----------|------|-------|---------|---------|
| 1894a-cloudflare-tunnel-routing | OPS-HIGH / UNBLOCK: Cloudflare tunnel `/api/*` routing fix (BLOCKER for user pollNews resolution). **c69 RCA BREAKTHROUGH (2026-05-13)**: cloudflared on this Mac is running in **NAMED TUNNEL TOKEN MODE** (launchd plist) — local `~/.cloudflared/config.yml` is IGNORED. Ingress rules come from the **Cloudflare dashboard**, NOT the local file. **All prior cycles' config.yml edits + service restarts were ineffective for this reason.** Verified by ops c69: localhost:4000/api/push-news returns 401 (route healthy); https://zenmidi.com/api/push-news still 404 after `brew services restart cloudflared` succeeded. **Precise USER action now defined**: (1) Log into https://dash.cloudflare.com. (2) Cloudflare Tunnel → vn-market-mcp → Public Hostnames. (3) Add/edit ingress rule: Path `^/api/*` → Service `http://localhost:4000`. (4) Save (auto-pushes to tunnel). (5) Verify: `curl -X POST https://zenmidi.com/api/push-news` returns 200 or 401 (not 404). **(awaits user dashboard action — 16th cycle ask; previous attempts misdiagnosed as config.yml issue)** | CRITICAL | UNBLOCK | user | — | 2026-05-12 |
