...
| 1871a | SPRINT-S: Update ARCHITECTURE.md MCP Server tool+cron counts. Drift: doc says "112 tools, 50 cron jobs, 38 files" but code has 132 tools, 56 cronConfig keys, 62 scheduler files. Files: docs/ARCHITECTURE.md, docs/data/project-stats.json. AC: reconciliation-direction = doc-updated, lines 78+188 corrected to match project-stats.json + actual file counts, divergence root-cause in task report. Brief: docs/architecture-briefs/2026-05-11-1871-reconciliation.md (D1). | MEDIUM | SPRINT-S | developer | TASK_1871a.md | — |
| 1871b | SPRINT-S: Expand ARCHITECTURE.md infrastructure/ tree to all 11 subdirs. Drift: doc lists 4 (db/fetchers/notifiers/rag/) but code has 11 — missing adapters/agents/cache/fileStore/microservices/observability/vps/. Critical: fileStore/alertVerdictStore.ts (Sprint 1863) entirely absent. Files: docs/ARCHITECTURE.md. AC: all 11 subdirs in tree with one-line description, divergence root-cause in task report. Brief D2. | HIGH | SPRINT-S | developer | TASK_1871b.md | 1871a |
...
| 1871d | SPRINT-S: Backfill 12+ missing jobs in cron-registry.json + correct project-stats.json schedulerFileCount. Drift: cron-registry.json has 41 jobs, cronConfig.ts defines 56 keys; schedulerFileCount=50 but 60 .ts files. Missing: verdictResolutionJob, alertOutcomeJob, signalOutcomeJob, vpsServiceHealth, freshnessSlaMonitor, dailyDashboard, foreignFlowFetch, bctcPdfPull, bctcQueueEnricher, marketEarningYield, bctcBatchSweep, integrityCheck. Files: docs/data/cron-registry.json, docs/data/project-stats.json. AC: entry count matches cronConfig ±startup probes, schedulerFileCount corrected. Brief D4. | MEDIUM | SPRINT-S | developer | TASK_1871d.md | — |
| 1871e | SPRINT-S: Fix tran-ngoc-bau flow get_agent_signals call — passes wrong params (limit/hours), omits required agent. Drift: tool schema requires {agent: string, status?} but flow calls get_agent_signals(limit=200, hours=24) — Zod validation fails at runtime. Files: .claude/flows/tran-ngoc-bau/main.md. AC: call updated to pass agent="tran-ngoc-bau" with correct schema, runtime error eliminated, divergence root-cause in task report. Brief D5 (= former F8). | HIGH | SPRINT-S | developer | TASK_1871e.md | — |
| 1871f | SPRINT-S: Resolve DDD violation in domain/repositories/IVnstockRepository.ts importing infrastructure/fetchers/vnstockBridge.js. Drift: doc rule "domain/ never imports infrastructure/" but line 17 violates; domain/models/shared-types.ts has 4 secondary sourcing-comment couplings. Files: apps/mcp-server/src/domain/repositories/IVnstockRepository.ts, apps/mcp-server/src/domain/models/shared-types.ts. AC: developer decides direction = code-fixed (extract types to domain/models/vnstockTypes.ts) OR doc-updated (document exemption in ARCHITECTURE.md), divergence root-cause in task report. Brief D6. | HIGH | SPRINT-S | dev-mcp-server | TASK_1871f.md | 1871g |
| 1871g | SPRINT-S: Correct alert-policy.md verdict storage description — fileStore is primary write target, not DB column aggregate. Drift: policy says "Verdict outcomes stored in agent_signals.outcome column. Aggregate data → alert-verdicts.json" but actual flow is: write_alert_verdict → alert-verdicts.json (primary, via fileStore/alertVerdictStore.ts) → verdictResolutionJob reads JSON → writes outcome to agent_signals.outcome. Files: .claude/knowledge/alert-policy.md. AC: Signal Verdict Lifecycle section accurately describes two-stage write (JSON first → DB second), divergence root-cause in task report. Brief D7. | HIGH | SPRINT-S | developer | TASK_1871g.md | — |
---
