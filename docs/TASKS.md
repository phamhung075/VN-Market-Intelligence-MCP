...
| 1937a-cowork-scheduler-mcp-gap | **SPIKE** — TNB c66 Finding #4. Cowork agents triggered by scheduler/automated context cannot access MCP tools (alert-commander 20:28 UTC, qa-responder 13:49 UTC both BLOCKED — "MCP connector not available"). Manually-triggered Cowork cycles succeed. Distinct from Docker/VPS outages. N=2 evidence points; root cause unknown (env var scope? scheduler runner context? MCP client init path?). Possibly same upstream as Finding #3 (news-scout 14:19 transient). AC: identify root cause, propose fix path (env var injection vs scheduler MCP shim vs runner config). Time-box 2h. Output: findings doc + FIX task if scoped. Zone: TBD by architect (multi suspected — scheduler + cowork bootstrap). | MEDIUM | SPIKE | architect | — | — |
