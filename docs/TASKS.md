...
## Todo
| BA-1944a | **Sprint 1944 TIER 1** — Write requirement spec for 1944a after ARCH-1944 DONE. Spec covers `/proxy/bctc-discover/:ticker?year=&quarter=` route shape (response envelope `{results:[{url,source,confidence}],error}`), `X-API-Key` header injection rule in `bctcHttpFetcher` (already landed — verify wiring), VPS deploy requirement (shape fix + `deploy-vinahost.sh`), error handling (401/404/500), test fixtures for live VPS probe + unit-test mock. References SPIKE-1916 + SPIKE-1943 + ARCH-1944 brief. Brief: `docs/architecture-briefs/2026-05-18-vps-bctc-discover-route-zone-split.md`. | HIGH | BA-SPEC | ba | — | — |
| 1944a-vps | **Sprint 1944 TIER 1 / VPS zone** — Fix response shape in `vps-scripts/vps-proxy-server.js`: wrap `runDiscoverScript()` string[] output in `{results:[{url,source,confidence}],error}` envelope (L329). Deploy updated `vps-proxy-server.js` to VPS via `scripts/deploy-vinahost.sh`. AC: `curl -H "X-API-Key: <key>" http://125.212.251.27:8765/proxy/bctc-discover/VCB?year=2026&quarter=1` returns `{results:[...],error:null}` with ≥1 `.pdf` URL. Zone: `vps-scripts/`. Owner: dev-vps-crawls. Size: S. | HIGH | FIX | dev-vps-crawls | — | — |
| 1944a-mcp | **Sprint 1944 TIER 1 / MCP zone** — Verify `X-API-Key` injection wiring in live stack (already in code, 6 unit tests GREEN). Add guarded integration test `1944a-vps-live-probe.test.ts` (`VPS_INTEGRATION=true` guard). Confirm `VPS_PUSH_API_KEY` in `.env` and `BCTC_DISCOVER_URL` in docker-compose. AC: unit tests GREEN + live probe returns ≥1 `.pdf` URL for VCB + enricher logs show `source_url populated` within one cron cycle. Zone: `apps/mcp-server/`. Owner: dev-mcp-server. Size: S. Handshake: run after 1944a-vps is deployed (live probe only; unit tests independent). | HIGH | FIX | dev-mcp-server | — | — |
| 1944b | **Sprint 1944 TIER 2** — Replace dead cafef Strategy 2 in `apps/mcp-server/src/.../bctcDiscovery.ts` OR delete it with deprecation comment. Same treatment for SSC iboard (NXDOMAIN) and vietstock (JS-rendered) strategies. AC: zero strategies left that return [] for every ticker; live tests run for at least one strategy returning ≥1 URL for VCB + DPM. Zone: `apps/mcp-server/`. Sequencing: lands after 1944a. | MEDIUM | FIX | dev-mcp-server | — | 1944a |
| 1944c | **Sprint 1944 TIER 3** — End-to-end smoke verification: after 1944a + 1944b deploy + ≥1 enricher tick + ≥1 sweep tick, produce smoke report in `reports/TASK_REPORT_1944c.md` with counts (source_url populated / PDFs fetched / financial_reports rows / banking Q1-2026 cohort coverage). AC: report exists + ≥5 of 7 watchlist banks have Q1-2026 row OR populated source_url. Zone: `apps/mcp-server/`. Owner: ops + dev-mcp-server collab. | MEDIUM | TASK | ops | — | 1944b |
| BA-1942d | **DEFERRED post-1944** — Write requirement spec for accuracy digest frontend card. Renders top-3/bottom-3 from new gateway endpoint backed by `getSystemAccuracyDigestStats` (Sprint 1941c). Frontend zone, independent of backend. Picked up only if Sprint 1944 finishes early. | LOW | BA-SPEC | ba | — | Sprint 1944 close |
