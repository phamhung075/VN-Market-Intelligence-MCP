...
|---------|-------|----------|------|-------|-----------|
| SPIKE_006-c61-T4-SHIPPED-c65 | FIX-S **DONE 2026-05-13 c65**: Add `insufficientSample: boolean` field to `AccuracyReport` type + guard in `formatAccuracyReport` — when n<20 samples, return warning + skip accuracy % line. Commit `80493433` (atomic, C2-clean after split from contaminated 7bc5853b). 20/20 tests pass on `183-alert-accuracy.test.ts`. tsc clean. Files: `apps/mcp-server/src/interface/mcp/tools/alerts/alertAccuracy.ts` + test. Addresses unified-agent telegram id=2874 (alert_precision 22%). | HIGH | FIX | developer | 2026-05-13 |
| SPIKE_006-c61-T2-SHIPPED-c64 | FIX-S **DONE 2026-05-13 c64**: Delete `scoreAlert()` function from `alertAccuracy.ts`. Wire Path 2 (NULL-outcome rows) to call `alertOutcomeScorer.classifyAlertType()` + `scoreAlertOutcome()` instead. Updated `183-alert-accuracy.test.ts` (AC-1: NULL-outcome alert must match direct scorer call). Commit `214957b0` impl. 19/19 tests passing, tsc clean. | HIGH | FIX | developer | 2026-05-13 |
| 1897c-ESCALATED-c64 | SPIKE **ESCALATED 2026-05-13 c64**: Worktree isolation contamination pattern CONFIRMED (2nd event: qa-responder + market-watcher committed to task branch mid-T2 build). Cherry-pick recovery overhead: 15+ min per cycle. **ARCHITECT ESCALATION:** Rethink Phase 4 agent isolation semantics (worktree tool expectations vs actual isolation). Move 1897c to Backlog HIGH. | HIGH | SPIKE | architect | 2026-05-13 |
| CLEAN-stale-remotes-c63 | CLEAN-SMALL **DONE 2026-05-13 c63**: 5 branches deleted (task/1872a-1, task/1872a-2, task/1880a-investment-clock-phase, task/signal-T1, task/signal-T4). PREFLIGHT lsof committed `6c561f7b` on main. Carry-forward: task/1872a-5-api-gateway-wording has 3 unmerged commits (PM/developer triage required). | MEDIUM | CLEAN | agent-father | 2026-05-13 |
| SPIKE_F2a_RECURRENCE_14-CLOSED-c63 | FIX-S **DONE 2026-05-13 c63**: F2a partial (only ./docs/data fully decomposed); ./reports + ./docs/agent-memory dir mounts sustain PID 51247 root access. Brief: `docs/architecture-briefs/2026-05-13-headlock-recurrence-post-F2a.md` (commit `74331a6b`). Verdict: formally accept PREFLIGHT cure as permanent (SPRINT-S doc update to head-lock-self-cure.md) + ask user for F1 (Docker file-sharing exclude .git/). Carry-forward 1897b (USER bundle). | HIGH | FIX | developer | 2026-05-13 |
| SPIKE_006-c61-T1-SHIPPED-c61 | FIX-S **DONE 2026-05-13 c61**: `hitThresholdPct` 0.1→1.0 in `alertOutcomeScorer.ts` price-signal class (both directions). Atomic commit `55085c1c` on `task/spike006-c61-t1-threshold-raise`; merged to main as `d6d3c5d9` (QA Phase 5 GREEN). Tests 9426→9428 (+TEST-15: 0.5%→UNKNOWN, +TEST-16: 1.1%→HIT). AC-3 satisfied. | HIGH | FIX | developer | 2026-05-13 |
| F2a-VERIFY-BLOCKED-c59 | FIX-S **BLOCKED 2026-05-13 c59**: Developer verify-first audit found BOTH `./reports/` and `./docs/data/` have active host-writers. Named-volume migration would silently drop host writes (QA + cowork agents write 713 reports/, all JSON in docs/data/ written from host with `:ro` container mount). No `docker-compose.yml` edit shipped. **Architect c60 ask:** re-scope F2a — Option A safe partial (per-file mounts for 4 JSON files in docs/data) eliminates dir-level VirtioFS scan surface without breaking host writes. F4 (Tier 3) ships defense-in-depth regardless. | HIGH | FIX | developer | 2026-05-13 |
