...
## mcp-server SCALE Pilot — Phase-1 HOST-SIDE BUILD COMPLETE (P1-A→P1-H SHIPPED) / AWAITING DOCKER-SESSION QA+REBUILD (reconciled 2026-05-25T11:20Z)

**Status (2026-05-25T11:20Z, PO triage reconcile — supersedes the stale READY-SOLO-DEFERRED prose below):** **Phase-1 WAVE-B host-side BUILD is COMPLETE.** The RUN-SOLO constraint authorized host-side execution (notebook 2026-05-25T09:18Z, commit 68de127d: Gate-2b `/health` = pure in-process probe → host-side `bun run + curl` self-certifies, zero live fleet) and dev-mcp-server then SHIPPED the full P1-A→P1-H chain SOLO. **Git proof (all committed, mcp-server zone CLEAN, 30+ min quiet):** P1-A sandbox `195ef1a3`, P1-B dashboard `3bc85cf7`, P1-C barrel-wave1 system/ `d053b0d4`, P1-D barrel-wave2 macro/ `d7f4129f`, P1-E barrel-wave3 sector/ `9da207fc`, P1-F G5-inverse kinhDichWrapper+QUE_META `970f4b38`, P1-G G5-inverse pdf.ts/pdfOcrWorker.ts `2148d2be`, P1-H signal-bus+sector-classifier sandbox `a9212ad2`, handoff `bce7c559`, ops notebook "Phase-1 rebuild complete" `ee1b77a4`. **REMAINING (NOT host-side, NOT dispatchable this tick): P1-QA close-gate (live full-tool-suite ≥9408/≤348 + tool 146 + scheduler 68 + dashboard) + P1-EXIT (PO terminal) — both REQUIRE the live docker fleet → belong to the SEPARATE docker session** (16GB host memory cap; do NOT trigger from a dev agent). This is the FINAL pilot — host-side code closes the 2026-05-22 rollout at 11/11 pending the docker-session QA gate.

**Prior (historical) status:** Phase-0 CLOSED + phase 0→1 (PO P0-MCP-EXIT 2026-05-25T10:50:43Z); 4 deliverables DONE (P0-MCP-1 `44530a26`, P0-MCP-2 `05dc494a`, P0-MCP-3 `0a4f1f28`, P0-MCP-5 `7d78abb1`); anchor `mcp-server-pre-refactor` @ `7d78abb1`. The "READY-SOLO-DEFERRED / P1-A not dispatched" framing below is STALE as of 11:20Z — host-side build shipped fully through P1-H.
...
**3-CONDITION GATE — ALL CLEARED 2026-05-25T08:40:43Z (PO UNBLOCK verdict):**
1. **Frontend Phase 0→Phase 1 transition complete — MET (2026-05-25T08:32Z).** frontend Phase 0 CLOSED + Phase 1 MVR build COMPLETE + QA-APPROVED (`c85f577c`, cycle-114; Vitest 179/0 + Playwright 4/0). Frontend refactor CODE is DONE; lingering `status=ACTIVE` is EARNED-PENDING bookkeeping awaiting USER verbal G9 sign-off + a Wave-C container rebuild — neither is a scale REFACTOR terminal occupying the lane.
2. **`apps/mcp-server/` quiesced — MET.** Working tree CLEAN (zero uncommitted edits). NEWS-INGEST-2b (the ONLY NEWS-INGEST task in the mcp-server zone) CODE LANDED `e1e08a29` + notebook `9e850fbf`; ops Docker-network fix `a5b6203d` landed. NEWS-INGEST-2 (`vps-scripts/fetch-vn-news.sh`) + NEWS-INGEST-LIVE (ops deploy-prove) remain open but live OUTSIDE the mcp-server barrels/source/docs-signals/docs-data/scheduler write surface — NOT shared-substrate contenders against Phase-0. DEPLOY-DRIFT tasks touch other zones (macro / kinh-dich / cross-service CI).
3. **Scale dev WIP free — MET (prior 'WIP FULL' claim was STALE).** P2-A1 LANDED (`9561fee9` `.golangci.yml` depguard; follow-up `9d364329` cycle-20 G4 unblock). P2-F2 LANDED (`cc7578f1` G12 dashboard-green DoD step into dev-technical-analysis flow; P0-FE-3 row already noted "P2-F2 (cleared)"). TA Phase-2 has progressed past these to G11/G12; last `apps/technical-analysis/` commit 2026-05-24 09:04 (~25h prior) — that terminal is QUIET and is a different (Go) zone that does not contend with mcp-server Phase-0 analysis/planning.

**Phase-0 backlog (ALL DONE — Phase-0 CLOSED 2026-05-25T10:50Z; commits verified in git log before crediting):**

| Task ID | Title | Priority | Type | Owner | Handoff | Status | Blocked by |
|---------|-------|----------|------|-------|---------|--------|-----------|
| P0-MCP-1 | **Brownfield inventory of `apps/mcp-server/`** (~132 tools, 10-module barrel structure, decomposed 8-slice schema, ~29 cron jobs, all HTTP-client wiring) + barrel-decomposition seams + candidate primitives (signal-bus / sector-classifier / portfolio-aggregator / ops-debug) + G5-inverse map (dead/migrated tool code now in microservices, each MCP tool handler proven to route via HTTP). | HIGH | TASK | architect | docs/handoffs/TASK_P0-MCP-1-brownfield-inventory.md | **DONE (`44530a26`)** — verdict FULL, 12 barrels, 146 tools, G5-inverse map | — |
| P0-MCP-2 | Bug-inventory baseline: live test/tool/scheduler counts + 7 open bugs (incl. R-CRITICAL G5-inverse BUG-5) + G10 primitive-class baseline | HIGH | TASK | qa | docs/handoffs/TASK_P0-MCP-2-bug-inventory-baseline.md | **DONE (`05dc494a`)** — ~9408 pass/~348 fail/35 skip; scheduler 68 (Gate-2d); 7 open bugs | — |
| P0-MCP-3 | Agent-father: confirm `dev-mcp-server` agent + bake/confirm `.claude/flows/dev-mcp-server/main.md` with G12 DoD gate (sandbox-green + full-tool-suite-pass before RETURN) + ESLint-fence note (TS service → G4 = ESLint per SI-3) | HIGH | TASK | agent-father | .claude/flows/dev-mcp-server/main.md | **DONE (`0a4f1f28`)** — G12 DoD Gate + RUN-SOLO baked; streak = P1-B/C/D | — |
| P0-MCP-5 | Phase-1 task plan authoring (architect) — barrel-decomposition order (smallest-blast-radius first), per-split full-tool-suite QA gate, scheduler/cron render-verification, G5-inverse HTTP-route-proof per tool, RUN-SOLO commit discipline | HIGH | TASK | architect | docs/architecture-briefs/2026-05-22-refactor/scale/mcp-server-phase-1-task-plan.md | **DONE (`7d78abb1`)** — FULL scope, 10 tasks P1-A..P1-EXIT, 78 ACs | P0-MCP-1, P0-MCP-2 |
| P0-MCP-4 | Set anchor commit (`mcp-server-pre-refactor` tag, local-only) + update `pilot-status-mcp-server.json` deliverable flags | MEDIUM | TASK | po (pm absorbed) | docs/data/pilot-status-mcp-server.json §phase0.anchor_tag | **DONE** — annotated tag **`mcp-server-pre-refactor` @ `7d78abb1`** (local-only, no push); pilot-status phase0=CLOSED + anchor recorded in SSOT | P0-MCP-1, P0-MCP-2, P0-MCP-3 |
| P0-MCP-EXIT | Phase 0 exit gate verification → PO approves Phase 0→Phase 1 | CRITICAL | GATE | architect+po | — | **DONE — CLOSED 2026-05-25T10:50Z (PO sign-off)** — all 4 deliverables verified; phase 0→1; WAVE B READY-SOLO-DEFERRED | P0-MCP-1..5 all DONE |
...
**Phase-1 backlog (HOST-SIDE BUILD COMPLETE — P1-A→P1-H all SHIPPED SOLO; P1-QA/P1-EXIT BLOCKED on docker session. Plan `7d78abb1`. Anchor: `mcp-server-pre-refactor` @ `7d78abb1`. Full ACs: `docs/architecture-briefs/2026-05-22-refactor/scale/mcp-server-phase-1-task-plan.md`.)**

| Task ID | Title | Goals | Owner | Status | Blocked by |
|---------|-------|-------|-------|--------|-----------|
| P1-A | Sandbox runner + scenario scaffolding (sparkline primitive) | G1, G7 | dev-mcp-server | **DONE (`195ef1a3`)** | — |
| P1-B | Three-tier trust dashboard stub (**G12 streak #1**) | G6, G8, G9, G12 | dev-mcp-server | **DONE (`3bc85cf7`)** | — |
| P1-C | Barrel wave 1: `system/` 21→5 sub-barrels (**G12 streak #2**) | G2, G12 | dev-mcp-server | **DONE (`d053b0d4`)** | — |
| P1-D | Barrel wave 2: `macro/` 14→HTTP-proxy vs local-computation (**G12 streak #3**) | G2, G12 | dev-mcp-server | **DONE (`d7f4129f`)** | — |
| P1-E | Barrel wave 3: `sector/` 15→3 cluster cuts | G2 | dev-mcp-server | **DONE (`9da207fc`)** | — |
| P1-F | **G5-inverse remediation (R-CRITICAL):** kinhDichWrapper bypass in `marketTools.ts` + `news-analysis/analysis.ts` + `portfolioTools.ts` QUE_META import; every-handler-HTTP-routed evidence | G5 | dev-mcp-server | **DONE (`970f4b38`)** | — |
| P1-G | **G5-inverse remediation (R-MEDIUM):** `pdf.ts`/`pdfOcrWorker.ts` post-1954c verify | G5 | dev-mcp-server | **DONE (`2148d2be`)** | — |
| P1-H | G1 primitive scaffolding: signal-bus-helper + sector-classifier + scenarios | G1, G7 | dev-mcp-server | **DONE (`a9212ad2`; handoff `bce7c559`)** | — |
| P1-QA | Phase-1 close-gate verification (tsc, bun test ≥9408/≤348, tool 146, scheduler 68, G12 streak 3/3, G5-inverse evidence, dashboard) — **REQUIRES LIVE DOCKER FLEET** | G1,G2,G5,G6,G7,G8,G12 | qa | **BLOCKED-ON-DOCKER-SESSION** (host-side build done; full-tool-suite live verify needs the rebuilt container) | P1-H (done) + docker rebuild |
| P1-EXIT | PO: SSOT reconciliation + pilot-status Phase-1 close (12/12 terminal flip PO-only) | — | po | BLOCKED | P1-QA |

- **G5-INVERSE CARRY (R-CRITICAL — explicitly preserved at the phase boundary):** the 3 G5-inverse violations from P0-MCP-1 §9 / P0-MCP-2 BUG-5 are carried as **P1-F** (kinhDichWrapper.appendKinhDich() bypass in `marketTools.ts` + `news-analysis/analysis.ts` + `portfolioTools.ts` QUE_META import — all R-CRITICAL) and **P1-G** (`pdf.ts`/`pdfOcrWorker.ts` post-1954c — R-MEDIUM). Both end with the "every handler proven HTTP-routed" evidence gate. NOT lost — confirmed in plan Task Ledger + §G5-Inverse Remediation Track + `pilot-status-mcp-server.json` phase1.progress_notes.
- **WAVE-B docker rebuild DEFERRED:** Phase-1 host-side TS/Bun edits + Vitest are memory-safe, but every wave's QA gate (live server health 200 + tool 146 + scheduler 68) + the closing container rebuild belong in the SEPARATE docker session (16GB host memory cap). Do NOT run a docker build in this cycle.
...
- **WAVE B — mcp-server build SOLO (per its Phase-1 plan). STATUS: HOST-SIDE BUILD COMPLETE (2026-05-25T11:20Z) — P1-A→P1-H all SHIPPED.** RUN-SOLO host-side execution authorized (notebook 68de127d Gate-2b finding: host `bun run + curl` self-certifies, zero live fleet) and dev-mcp-server delivered the full chain SOLO: P1-A `195ef1a3`, P1-B `3bc85cf7`, P1-C `d053b0d4`, P1-D `d7f4129f`, P1-E `9da207fc`, P1-F `970f4b38`, P1-G `2148d2be`, P1-H `a9212ad2` (handoff `bce7c559`). mcp-server zone CLEAN, quiet 30+ min. **Gate B→C now OPEN:** all targeted barrel splits + both G5-inverse remediations landed; host-side sandbox/dashboard built. **REMAINING = WAVE C/D (docker session only):** P1-QA full-tool-suite live verify + container rebuild + P1-EXIT terminal flip — all require the live fleet and are NOT dispatchable from a dev agent (16GB host cap).
...
| BT-3 | **Produce + store structured table.** TEXT-path extractor emits code→value table + balance check in `process_report()`; persist to NEW `bctc_table_rows` schema per doc+page. Replace blind regex column-picking with `select_period_column` over real cells. Zero creds in sandbox. | HIGH | TASK | dev-pdf-extractor | docs/handoffs/TASK_BCTC-TABLE.md | BLOCKED | BT-2, BT-1 |
| BT-3i | **Inspector schema read + table render** (SI-2 boundary). `bctcInspectHandler.ts` + `/api/bctc-inspect` viewer read new schema, render table next to OCR text + balance PASS/FAIL badge. **Closes the user's complaint.** | HIGH | TASK | dev-mcp-server | docs/handoffs/TASK_BCTC-TABLE.md | BLOCKED | BT-2, BT-3 |
| BT-4 | **Deploy extractor to main server.** Host TEXT-path extractor (CPU-feasible 4s/page, no GPU — Q2 resolved). NO model on Mac in prod (D6). | HIGH | TASK | ops + dev-mainserver-crawls | docs/handoffs/TASK_BCTC-TABLE.md | BLOCKED | BT-2 |
| BT-4b | **One-shot re-extraction of 14 stranded docs.** Old-parser figures → re-extract ONCE post-integration, pre-QA (not twice). | MEDIUM | TASK | dev-pdf-extractor + ops | docs/handoffs/TASK_BCTC-TABLE.md | BLOCKED | BT-3, BT-3i, BT-5 |
| BT-5 | **Cross-check confidence gate (self-hosted).** Wire `reconcile_figures` into app layer: >10× divergence → block insert + WORK alert; surface in `/api/bctc-inspect`. Image-track = SELF-HOSTED VLM only, DEFERRED. NO external API. | MEDIUM | TASK | dev-pdf-extractor | docs/handoffs/TASK_BCTC-TABLE.md | BLOCKED | BT-3, BT-4 |
| BT-6 | **QA regression gate.** Re-run harness across WIDER 14-doc gold-set (not just FPT): figure-accuracy ≥95%±0.5%; VNM/DHG green; rows stored+rendered+balance badge; cross-check fires >10×; sandbox exit-0+zero creds; fence intact; pilot-status diff empty; zero off-infra send. Also closes QA-on-BT1 (pending). Emit `qa-bctc-table-<UTC>.json`. | HIGH | TASK | qa | docs/handoffs/TASK_BCTC-TABLE.md | BLOCKED | BT-4b, BT-5 |
| BT-EXIT | PO sign-off vs DoD on REAL gold-set + verify live `/api/bctc-inspect` shows detected table + balance badge (complaint closed) + privacy audit. Main terminal commits in-tree work. | CRITICAL | GATE | po | docs/handoffs/TASK_BCTC-TABLE.md | BLOCKED | BT-6 |

**Notes:**
- **Dispatch order (UPDATED 2026-05-25T17:17Z):** BT-1 DONE, BT-0 DONE, BT-0-PICK DONE (PICK=TEXT path). **NEXT = BT-2 (architect blueprint)** — gates all integration. Then BT-3 (produce+store) + BT-4 (deploy) in parallel → BT-3i (inspector render, dev-mcp-server) → BT-5 (cross-check) → BT-4b (one-shot re-extract) → BT-6 QA → BT-EXIT.
