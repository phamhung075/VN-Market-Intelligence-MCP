...
## Todo
| 1939a-tnb-critic-gate-sprint-a | **NEW c171** — TNB Critic Gate Sprint A: schema migration (3 columns: critic_score/critic_notes/retry_count) + CriticInput/CriticResult types + pure `scoreWithTnbCritic()` scorer in `tnbCriticScorer.ts` + unit tests. No behavior change — safe to ship independently. Brief: `docs/architecture-briefs/2026-05-17-tnb-critic-gate.md` § Steps 1,2,3,8. | HIGH | SPRINT-S | dev-mcp-server | docs/handoffs/TASK_1939a.md | — |
| 1939b-tnb-critic-gate-sprint-b | **NEW c171** — TNB Critic Gate Sprint B: implement `postSignalWithCriticGate()` + wire `post_agent_signal` MCP tool + extend INSERT paths + full tests. Depends on 1939a. Brief: `docs/architecture-briefs/2026-05-17-tnb-critic-gate.md` § Steps 4,5,6,7. | HIGH | SPRINT-S | dev-mcp-server | docs/handoffs/TASK_1939b.md | 1939a |
