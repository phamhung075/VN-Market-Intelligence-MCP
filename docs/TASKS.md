...
| 1942c | **HANDOFF READY** — HPG OCF all-zeros fix. BA spec + Architect design complete. 2 primary files: `vnstockBridge.ts` (CASH_FLOW_SCRIPT fallback keys + NULL policy) + `cashFlowExtractor.ts` (steel-sector label variant). 1 domain type change: `VnstockCashFlow.operatingCashFlow: number\|null`. 1 new test file: `1942c-hpg-cashflow-fix.test.ts` T1-T6. No schema changes. Handoff: `docs/handoffs/TASK_1942c.md`. | MEDIUM | TASK | dev-mcp-server | docs/handoffs/TASK_1942c.md | 1942b |
