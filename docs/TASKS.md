...
## Follow-On Enhancement (news-fetch SERVED dashboard — NF-LD-4) — OPEN 2026-05-24

**Status:** OPEN 2026-05-24T18:50Z (PO self-initiated from user feedback, signal `po-news-fetch-served-dashboard-20260524T185027Z.json`). Follow-on to the CLOSED NF-LD chain. **NOT a pilot reopen** — news-fetch SCALE pilot stays DONE (12/12, verdict=scale; pilot-status FROZEN). Handoff/spec: `docs/handoffs/TASK_NF-LD.md` `# TASK NF-LD-4` section. Chain: **architect (NF-LD-4-design) → owning dev-\* (architect names) → qa (NF-LD-4-QA) → PO (NF-LD-4-EXIT) → ops rebuild + PROVE served URL** (NO `pm`/`ba` agent — PO absorbs close-out).
...
| NF-LD-4-dev | **SPLIT:** NF-LD-4-dev-A (dev-mcp-server): newsFetchDashboardHandler.ts + news-fetch-dashboard/ dir + server.ts wiring. NF-LD-4-dev-B (generic developer): `apps/news-fetch/dashboard/index.html` ENDPOINT relative-path change only. Both can run in parallel. | HIGH | TASK | dev-mcp-server (A) + generic developer (B) | docs/handoffs/TASK_NF-LD.md | OPEN | — |
| NF-LD-4-QA | QA: verify Security Clause (sandbox process credential-free, no creds in served dashboard), served dashboard works (sandbox panels + live panel both render when served), file:// degrade still present as fallback, NO sandbox/`data.js`/pilot regression, pilot 12/12 frozen. Emit `qa-news-fetch-served-<UTC>.json`. | HIGH | TASK | qa | docs/handoffs/TASK_NF-LD.md | BLOCKED | NF-LD-4-dev |
| NF-LD-4-EXIT | PO sign-off vs goal + Security Clause + anti-regression; then DISPATCH ops to rebuild the chosen container. | CRITICAL | GATE | po | docs/handoffs/TASK_NF-LD.md | BLOCKED | NF-LD-4-QA |
| NF-LD-4-OPS | ops: rebuild the chosen container (`docker compose up -d --build <service>`) + PROVE — real http GET of the served URL returns the page AND the live endpoint is reachable from that origin AND live rows render. This is the DONE gate. | CRITICAL | TASK | ops | docs/handoffs/TASK_NF-LD.md | BLOCKED | NF-LD-4-EXIT |
