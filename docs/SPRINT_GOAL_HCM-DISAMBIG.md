...
**Status:** COMPLETE — SIGNED OFF 2026-05-28 by PO (HCM-EXIT gate). QA verdict APPROVED 6/6 ACs at `docs/signals/qa-hcm-disambig-2026-05-28T214341Z.json` (commit a2ff3356). Sprint chain: PO sprint creation @4b0608cd → HCM-BA (24 ACs in `docs/REQ_HCM-DISAMBIG.md`) → HCM-ARCH (`docs/architecture-briefs/2026-05-28-hcm-disambig.md`, R-1/R-5 closed, suppress-only rule) → HCM-PM (4 handoffs, WIP=0) → HCM-D1 @10438892 (newsNormalizer.ts GEOGRAPHIC_CONTEXT_MAP +2 entries `tp. hcm` / `tp-hcm`; test 19/19 + 31 total regression green) || HCM-D2 @de391d9b (chef.md line 192 Block A narrative rule `HCM (cổ phiếu)` vs `TP. HCM`) → HCM-OPS (mcp-server force-recreated, container 34a9b5165c828, image f09264113a71, /health 200, toolCount 146) → HCM-QA @a2ff3356 → HCM-EXIT. Evidence: fence-false-green proof (deliberate fail injected → 1 fail non-zero exit → revert 19 pass 0 fail), post-rebuild pollNewsJob success at 21:41:48Z (2189ms), 0 HCM false-positive in DB after 21:37Z rebuild, #4144 replay clean. Originally kicked off 2026-05-28T21:20Z by PO from EXPLICIT USER bug report (content accuracy / non-technical user MARKET dish trust).
