...
|------|---------|--------|---------|
| [sprints-064-080.md](archive/sprints-064-080.md) | 064–115 | 2026-04-12 → 04-17 | Knowledge sync, prediction resolution, Bun.env purge, briefing enrichment, market message review, calibration labels, per-ticker intelligence, BCTC pipeline fix, evening pipeline fix, RSS Atom support, pipeline health tool, pipeline watchdog, TE RSS fallback chain, evening summary empty-content fallback, domain bug batch (cascade/NER/relevance), push-news all 9 VPS sources, direction-aware macro deviation labels, test isolation (137 Step E 30s timeout fix), fix defaultComputeTa reads daily_ohlcv, pollNews SOURCE_DISPLAY_NAMES map (eliminates 2 test-1227 failures), test hygiene (297 DB_PATH isolation + 296 OCR 30s timeout cap), alert delivery medium severity fix, test/fix (ta-fallback) defaultComputeTa falls back to market_prices_history when daily_ohlcv < 15 rows, france-summary stale alert 24h filter + same-day dedup guard (1344+1345) |
