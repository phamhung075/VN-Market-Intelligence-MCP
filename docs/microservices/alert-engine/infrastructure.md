...
```sql
SELECT id, stocks, signal_types as signalTypes, message, fingerprint,
       severity, triggered_at as triggeredAt, sent_telegram as sentToTelegram
...
```sql
SELECT COUNT(*) as cnt FROM alert_engine_records
...
## TelegramClient
- **File:** `apps/alert-engine/src/infrastructure/telegram.ts`
- Implements `TelegramPort`
...
2. If no token or chatId → return false (silently skip in dev/test)
