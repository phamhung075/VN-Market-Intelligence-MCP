...
| 2 | Has `Error Boundary` in flow | `Grep -i "error boundary" docs/agents/<agent-id>/flow/*.md` (case-insensitive — live text uses both cases). **One-hop delegation resolution (mandatory before FAIL):** if the thin flow file has zero hits AND contains a one-hop pointer line — `-> Run shared flow: <path>` or `-> Run sub-flow: <path>` (e.g. dev-* zone agents' `flow/main.md` pointing to `docs/agents/developer/flow/microservice-main.md`) — re-run the same grep against the resolved `<path>` before returning FAIL. Only FAIL if BOTH the thin file and its one-hop pointer target (when one exists) show zero hits. | NO — needs manual authoring |
