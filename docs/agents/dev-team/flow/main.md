...
| `backlog[]` | F | T | F | **RESIDUAL GAP, documented, out of scope for this task** — `plan_only` alone (no `supervised`) is excluded from BOUNDED-1 (plan_only gate) and from SLS (requires BOTH true); covered by **DRS** only if `next_agent` is non-dev + allowlisted, otherwise no automated picker today. Surfaced non-gating by the report script's SECONDARY section; not silently assumed fine. |
| `backlog[]` | T | F | F | **DRS** if `next_agent` is non-dev + allowlisted (DRS excludes only the sup&&po-BOTH class, not `sup` alone). If `next_agent` is dev-role: **explicit documented no-picker verdict** — PO-adjudicated row-by-row (§ Design-Router Sweep "Additional gap flagged" paragraph above) — auto-dispatching a `supervised:true` dev-role row would defeat the reason the flag exists; NOT folded into any predicate. |
...
| `ready[]` | F | T | F | **RESIDUAL GAP, documented, out of scope for this task** — RLC excludes any `sup`/`po` alone; no dedicated picker exists for `po`-only ready[] rows today. Same class as the `backlog[]` analog above, one lane over — surfaced non-gating (§ report script's new READY-XOR section), not silently uncovered. |
| `ready[]` | T | F | F | **RESIDUAL GAP, documented, out of scope for this task** — RLC excludes it; SLS-claim's FALLBACK requires BOTH `sup` AND `po` true (mirrors SLS-promote's own gate, deliberately not widened to sup-XOR-po — widening SLS's identity-check-free fallback to single-flag rows was judged out of this task's scope, mirroring the `backlog[]` XOR gaps above). Surfaced non-gating. |
...
| `review[]` | any | any | any | **Review-Lane QA-Drain**, unconditionally, whenever `effective_next_agent == "qa"` — deliberately does NOT gate on `sup`/`po`/`wrap` (AC-3, see § Review-Lane QA-Drain below). `next_agent != "qa"` = **Review-Lane SECONDARY-Drain** (FIX-DEVTEAM-REVIEW-LANE-SECONDARY-DRAIN, 2026-08-01 — see § Review-Lane SECONDARY-Drain below), which dispatches the row's own resolved owner (or `"po"` if unresolved) for triage/sign-off; also does not gate on `sup`/`po`/`wrap`. |
...
FIX-BOUNDED1-NONDEV-NEXTAGENT-RESIDUAL-NO-DISPATCH-LANE (architect brief 2026-07-29, PO-ratified 2026-07-30 — `docs/agent-memory/decisions/ruling-20260730T0906Z-po-triage-po.md` STEP po-4). Runs immediately after the Ready-Lane Consumer block above, still inside the same head-idle fall-through, still BEFORE Step 1 PO triage — reached ONLY when RLC did NOT claim+dispatch this tick. Control flow guarantees `.head` is still idle whenever this block runs (same argument as SLS/RLC's own placement).
...
if [ "$WIP4" -lt 2 ]; then
  ...
  jq --arg now "$NOW" \
    ...
    docs/data/orch/orch-state.json | bash "$PROJECT_ROOT/scripts/orch-apply.sh" || true
  jq --arg now "$NOW" -f "$PROJECT_ROOT/scripts/devteam-backlog-claim-design-router-sweep.jq" \
    docs/data/orch/orch-state.json | bash "$PROJECT_ROOT/scripts/orch-apply.sh" || true
...
- **Claim** (`scripts/devteam-backlog-claim-design-router-sweep.jq`): moves the DRS-stamped `ready[]` row → `in_progress[]`, sets `.head.next_agent` to the row's own already-resolved `dispatch_lane` DIRECTLY (never a `"developer"` fallback). **`.head` write uses the MANDATORY conditional guard** (`$head_free = (.head.status in {idle,done}) or .head.active_task_id == null`) — never an unconditional replace, per the ratification's hard AC (Q3) and the live `qadrain-head-slot-decouple` precedent that proved "safe by current placement" is not a durable invariant in this flow-doc. No-op if nothing DRS-stamped is waiting.
...
UNBLOCK-DEVTEAM-DISPATCH-GATE-STAGING-DEADLOCK (architect, 2026-07-22), PO ruling item (3) — FOLDS `FIX-DEVTEAM-REVIEW-LANE-QA-DRAIN` (backlog since 2026-07-12; this section + its scripts ARE that row's own SUGGESTED REMEDY, implemented). Runs immediately after the Design-Router Sweep block above, still inside the same head-idle fall-through, still BEFORE Step 1 PO triage — reached ONLY when DRS did NOT claim+dispatch this tick.
