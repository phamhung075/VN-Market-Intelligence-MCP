...
- `head.status == "in_progress"` AND `head.next_agent` non-null →
  ...
  **WF-4 STALE-AGE check (FIX-DEVTEAM-HEAD-PIN-STALE-THRESHOLD-24H-VS-TICK-CADENCE — run SIXTH,
  ...
  ```
  If task is NOT BLOCKED, NOT terminal, NOT ready-lane-resident, NOT supervised-held, NOT
  resume-attempt-bound-exceeded, and NOT stale-age-exceeded → dispatcher-wrap then spawn
  `head.next_agent`. JUMP TO `execute`.
  ```
  # S2 dispatcher-wrap:
