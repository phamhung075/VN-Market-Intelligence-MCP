...
- `head.status == "in_progress"` AND `head.next_agent` non-null →
  ...
  **WF-4 STALE-AGE check (FIX-DEVTEAM-HEAD-PIN-STALE-THRESHOLD-24H-VS-TICK-CADENCE — run SEVENTH,
  ...
  if not outer_claim.claimed:
    ...
    # raising it: that only moves the cliff, it does not remove it). AC-2 (crash recovery preserved):
    # this renewal fires ONLY on ticks that actually run and actually reach this branch — a genuinely
    # dead dispatcher (cron itself stops ticking) renews nothing, so `resume_key` still organically
    # lapses on its original, unchanged 3600s TTL; WF-3 (resume-attempt-bound)/WF-4 (2h stale-age +
    # git-log corroboration) above — BOTH byte-unchanged by this fix — remain the sole, independent
    # authority for deciding a task is genuinely abandoned and safe to reset/respawn; this keepalive
    # step never resets `.head`, never spawns, and never overrides either of them.
    if outer_claim.current_holder:
      ...
      log "[dev-team] SKIP pipeline resume " + resume_key + " — held by peer session (renewed, hb.ok=" + hb.ok + ")"
    else:
      # claimed:false with no current_holder = coordination.db unavailable/degraded (F3/F5,
      # docs/protocols/task-lock-protocol.md § Failure Modes) — nothing to renew; fall through
...
**UNCONDITIONAL — no `head.status` gate, no rotation gate** (brief §4d: ILC strictly dominates a rotation-gated site — it fires on every tick, idle or busy, `$SELECTED` irrelevant — so unlike QA-Drain, which kept BOTH its rotation-gated site AND a head-decoupled one, ILC needs only this ONE site). Independent `INCIDENT_CAP` budget, entirely separate from the shared `WIP≤2` slot (`incident_wip_in_progress`, `scripts/lib/devteam-eligibility.jq` — counts ONLY rows carrying `claimed_by == "dev-team (incident-lane consumer)"`; `wip_in_progress` excludes them by the same marker) — never competes with BOUNDED-1/SLS/RLC/DRS for their slot. `INCIDENT_CAP=2` bounds how many `po_expedited_at` rows can ever be in flight through this lane at once, regardless of how many PO ever marks (brief §4b) — the entire answer to "must not become a 4th priority tier".
  ...
  if not outer_claim.claimed:
    ...
    try:
      Agent(row.next_agent,
            ...
            + " Consumer's own independent INCIDENT_CAP budget, never the shared WIP<=2 slot. Take"
...
- **Independent concurrency budgets, both OUTSIDE the shared WIP≤2 slot above (never compete with BOUNDED-1/SLS/RLC/DRS for it) — named together here so a future reader auditing concurrency budgets sees all three in one place, not two:** `qa[] < QA_CAP` (`QA_CAP=10`, Review-Lane QA-Drain — both its idle-tick and head-decoupled invocation sites share this ONE budget) and `INCIDENT_CAP` (`=2`, dev-team incident-lane consumer — `incident_wip_in_progress`, § Incident-Lane Consumer (ILC) — Head-Decoupled Invocation above, FIX-DEVTEAM-INCIDENT-LANE-CONSUMER-MAINFLOW 2026-08-25).
