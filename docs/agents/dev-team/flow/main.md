...
for row in $picked_batch:
  ...
  if not outer_claim.claimed:
    log "[dev-team] QA-DRAIN-HEADDECOUPLED SKIP " + row.id + " — held by peer session"
    continue   # do NOT abort the whole batch over one peer-held row
  else:
    try:
      Agent("qa", context... + "task_id=" + row.id + " mode=verify-committed", run_in_background=true)
...
# Durable-inbox CLEAR (brief §3.2) — subtracts by envelope_id, never a blind `= []`, defensive against an
# entry landing between the read above and this clear (SF-1 + fire-election make this vanishingly unlikely
# with a single dev-team session active, but costs nothing to guard, per brief §3.2). Duplicate-safe, not
# loss-safe (brief §3.2's deliberate asymmetry): if this write never lands (session crash between spawn
# success and here), the next step1_triage turn re-delivers the SAME envelope_ids to PO — a low-cost
# duplicate PO's own triage-signals.md dedup guards (e.g. zone_missing_tier3 dedup on taskId) already
# absorb, never a silent loss.
# FIX-ORCHAPPLY-CONSERVATION-FLOOR-BLOCKS-SANCTIONED-PO-INBOX-DRAIN-CLEAR (2026-08-14): this write is
# the SOLE sanctioned remover of `.dev_team_idle_chain.pending_triage_inbox[]` entries. Declare the
# exact ids being removed via ORCH_APPLY_DECLARED_INBOX_TRIAGED (mirrors orch-cold-evict.sh's existing
# ORCH_APPLY_DECLARED_SIGNAL_EVICTIONS pattern for signal_queue.rows[]) — `consumed_ids` is ALREADY the
# id list this write removes, so no new computation, just pass it through. This closes the gap where a
# pending inbox that grew across several rotation-starved ticks (this section's own "may contain
# entries carried over from N prior ticks" note above) could not be cleared in ONE write — the inbox
# was previously part of the signal_total magnitude-ratio floor, so a large legitimate clear tripped
# the same 0.5-ratio circuit-breaker built to catch accidental mass-deletion, with NO sanctioned bypass
# (ORCH_APPLY_ALLOW_SHRINK is forbidden to every caller except orch-cold-evict.sh/task-archive.md) —
# forcing artificial sequential sub-batching (REPRODUCED LIVE: 29 envelopes/4 writes on 2026-08-11, 248
# envelopes/4 writes on 2026-08-14). The inbox is now excluded from that magnitude metric entirely and
# guarded by its own row-identity check instead (scripts/orch-conservation-check.mjs) — an UNDECLARED
# drop (this env var omitted or incomplete) is still hard-rejected, so an accidental wipe stays caught.
