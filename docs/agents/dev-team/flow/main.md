...
| `review[]` | any | any | any | **Review-Lane QA-Drain**, unconditionally, whenever `effective_next_agent == "qa"` — deliberately does NOT gate on `sup`/`po`/`wrap` (AC-3, see § Review-Lane QA-Drain below). `next_agent != "qa"` = SECONDARY/PO-triage queue (pre-existing since 2026-07-22, unchanged, non-gating). |
