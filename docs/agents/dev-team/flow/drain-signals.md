...
Neither is optional. Curate-and-route without persist+commit = incomplete drain. After drain, commit ONLY these paths: `docs/signals/processed/`, `docs/signals/*.json` (deletions), `docs/data/orch/orch-state.json` (signal_queue section only). `signals.db` is untracked + gitignored (UC-GCP-P2 2026-07-16, `*.db` rule) — the mtime freshness check in item 2 stays (still gates whether the DB write itself is required this tick); do NOT `git add docs/signals/signals.db` — git refuses already-ignored paths and exits 1.
