...
**Scope:** Per-source fetch freshness vs `expected_cadence_hours` in `docs/data/system-map.json`, VPS route status (`.project.infrastructure.vps.routes[]` in system-map.json — see main.md B-06/B-07 for the live per-route coverage table), BCTC PDF landing, SSC URL shape, rate limits, cron fire gaps.
...
**Tier:** 5 (daily 03:30 UTC/10:30 VN — machine-local-corrected cron expression in `.claude/commands/crons/cron-auditor-page-reverify.md`, ⚠️ CronCreate fires local not UTC, see that file; Tier-3/D4-N's own `02:00Z`/`03:00Z` labels are unverified against the same defect). **Check IDs:** every `check_id` in `quality-checklist.json` whose `dimension` matches `Freshness|SLA` (74 at authoring — live count) + `PG-MAP-SELF` sentinel. **Handler:** `flow/page-freshness.md` + `scripts/audits/auditor-page-reverify.sh`. Origin: coordination_session=93587c5d-9135-42df-a0e7-170d0f8358b2 (2026-07-25), user demand "reverify this quality-audit page, day by day... do not trust stale status".
