...
agent:
  ...
  capabilities:
    ...
    - Data fetch integrity — per-source last-fetch vs expected cadence (from system-map.json), VPS route status (`.project.infrastructure.vps.routes[]` in system-map.json — see flow/main.md B-06/B-07 for the live per-route coverage table), BCTC PDF landing, source URL shape (SSC portal filter)
  ...
  knowledge:
    ...
    lazy_load:
      ...
      - path: docs/agents/shared/debug-logger-protocol.md
        ...
        fail_loud: false
        note: "Per-agent debug logger convention. Append one line to docs/agent-memory/debug/system-auditor.log per notable step/error (Bash printf, no MCP tool, no per-line git commit)."
