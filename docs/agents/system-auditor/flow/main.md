...
Runs on EVERY tier's EVERY cycle, before any tier-specific work, mechanically, independent of which tier this invocation itself is running as. `$MARKERS_FILE` for THIS cycle does not exist yet at this point (created only after Step 0d below wins its own election) — this sweep only ever finds a PRIOR cycle's abandoned scratch file. This step is now REAL, EXECUTED bash with its own checkable trace (FIX-AUDITOR-NOTEBOOK-COMPOSE-ACTUATOR-BUILT-TESTED-NEVER-WIRED AC-6b) — previously this sweep's own dead-ness was invisible because it was narrated, not run. `+20` minutes comfortably exceeds Tier-1's own normal cycle time (observed: single-digit minutes even for extended multi-probes) while staying well under Tier-2's 4h gap — never fires on an in-progress sibling tier's cycle. Filename-key shape is validated before trusting it — live evidence has shown both a `auditor-t1:<tick>`-keyed file (colon-bearing, breaks the old "parsed directly from the filename" assumption) and an empty-key file; a `malformed-key` sentinel now replaces either shape rather than passing the raw string through:
```bash
...
for f in $STALE_MARKERS; do
  ...
  case "$raw_tick" in
    [0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]Z) FIRE_TICK_SWEPT="$raw_tick" ;;
    *) SWEEP_MALFORMED=$((SWEEP_MALFORMED+1)); FIRE_TICK_SWEPT="malformed-key" ;;
  esac

  # For each hit, emit via the same dedup-ledger every other check in this flow already uses:
  ...
  # On any non-ABORT marker outcome (OK/SKIP-dedup/OK-escalation-bypass) → rm -f "$f" (the loss is
  # now durably recorded in the signal_queue row; safe to reclaim the scratch file). On ABORT →
  # leave the file in place for the next cycle's sweep to retry — never delete the only evidence
  # of a loss on a failed emit.
done
```
Reuses the SAME `docs/data/auditor-dedup-ledger.json` 7-day ledger every other check in this flow already goes through — a retried sweep on the same orphaned tick within 7 days is a `SKIP-dedup`, not a re-alert (no new dedup bookkeeping invented here; a `malformed-key` dedup_key still uniquely identifies "the malformed-key class was swept", never re-colliding with a clean tick's own dedup_key). This sweep is NOT itself an entry in THIS cycle's own `$MARKERS_FILE` and does not count toward this cycle's own `signals_posted` in §OUTPUT-CONTRACT below (it is bookkeeping about a PAST cycle) — paste its `[emit-signal]` marker into THIS cycle's notebook entry only if one gets written anyway. `SWEEP_COUNT`/`SWEEP_MALFORMED`/`STALE_MARKERS` are buffered in plain shell variables here (this step runs BEFORE `$MARKERS_FILE` exists — Step 0d creates it below); immediately after `$MARKERS_FILE` is initialized in §Step 0d's Cycle Markers Log init, its own `[stale-marker-sweep]` trace line is appended there — this gives the sweep the same mechanical, grep-able trace every other actuator in this file already has, closing the specific gap that made its own dead-ness invisible.
...
: > "$MARKERS_FILE"
echo "[stale-marker-sweep] swept=${SWEEP_COUNT:-0} malformed=${SWEEP_MALFORMED:-0} found=${STALE_FOUND_COUNT:-0}" >> "$MARKERS_FILE"
```
(`${SWEEP_COUNT:-0}`/`${SWEEP_MALFORMED:-0}`/`${STALE_FOUND_COUNT:-0}` all default to `0` when Step 0b.1 found zero stale markers that cycle (its `for` loop body never ran) — this line runs unconditionally, giving Step 0b.1 the same mechanical, grep-able trace every other actuator in this file already has, even on the common zero-hits case.)
...
that field. Fire-state for these 3 is instead the SAME evidence Step 0b.2 above already computed THIS
cycle (extend, not duplicate — Step 0b.1–0b.3 run unconditionally before any tier-specific work, so by
the time this section runs the tier-1/2/3 gap checks already ran): fold the `[SCHEDULE-GAP]` result (or
its absence) for tier-1/2/3 into this check's own N/M line below; do not re-emit a second signal for the
same gap under a different check_id — Step 0b.2's own `D-CYCLE-2` emit (or Step 0b.1's `D-CYCLE-1`) is
the signal of record for these three.
