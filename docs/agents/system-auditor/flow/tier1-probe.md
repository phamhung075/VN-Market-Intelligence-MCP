...
**NEVER write `docs/data/auditor-tier1-last-healthy.json` from any step in this file, on any verdict**
(FIX-AUDITOR-TIER1-FLOW-HAND-WRITES-HEARTBEAT-FILE, 2026-08-11 — 2nd confirmed hand-write recurrence). Sole
writer is `scripts/agents-flow/auditor-tier1-probe.sh`'s `_write_heartbeat()`, reachable only from that
pre-gate script's own `run_probe()` ALL_GREEN branch — a branch that never runs while this subagent is
executing (this subagent is spawned only when the pre-gate did NOT reach ALL_GREEN). This file's own output
is limited to the notebook cycle-log entry + `scripts/emit-audit-signal.sh` signal rows (§Emit per failure
below) — never a direct write to that filename. Full contract: `docs/policies/dev-standards.md`
...
2. For each ENGAGEd container: `[A-30] <container>: deep-probe subprocess FAILED` present →
   ...
   unhealthy).
2b. **`"verdict":"INCONCLUSIVE","truncated":true` present in a container's JSON block**
   (FIX-AUDITOR-A30-DEEPPROBE-TRUNCATES-ON-SECOND-CONTAINER-RAG-SERVICE, 2026-08-12) →
   `probe.sh`'s own per-container watchdog killed that container's deep-probe subprocess
   after it exceeded its `window.deadline_sec` budget without producing a verdict. This is
   the MACHINE-READABLE replacement for the old unstructured prose "Probe output incomplete
   / raw JSON verdict block not present" DEFER (recurring 3x, c40/c41/c42) — treat it as a
   structured DEFER: log `[A-30] <container> DEFER — deep-probe watchdog-terminated after
   <deadline_sec>s (structured marker per its own JSON, not a hand-typed guess)`, cite the
   block's own `reason` field verbatim, NO signal emit (a bounded/incomplete window is not
   evidence of anything, positive or negative — same rule as A-30's own header contract for
   a script-level failure), and expect a fresh attempt next Tier-1 cycle (ENGAGEd containers'
   deep-probes now run CONCURRENTLY — see `probe.sh`'s own `_a30_run_investigate_gate`
   header comment — so this should now be rare; a `truncated:true` marker recurring across
   consecutive cycles for the SAME container is itself worth a WARN-severity signal that the
   watchdog deadline or that container's own probe window may need recalibration, distinct
   from an A-30 memory-pressure finding).
