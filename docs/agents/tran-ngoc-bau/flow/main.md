...
**Authoring invariant (FIX-TNB-AUDIT-STEPS-ASSUME-NONEXISTENT-CHANNEL-PARAM, 2026-08-23):** an audit step must be written against a verified tool schema, not an assumed one. `./audit-chef-coverage.md` Step 0.5 and `./audit-market.md` Steps 1a/1b once called `read_telegram_reports(channel=...)` on the assumption the tool had a `channel` filter — it never did (zod: `status`/`limit`/`unclaimed_only` only, `apps/mcp-server/src/interface/mcp/tools/briefings/telegramReportTools.ts`); the argument was silently ignored for 20+ cycles rather than erroring, same defect class as `feedback_wrong_arg_type_silently_disables_a_verification_predicate`. Before writing a NEW audit step against any MCP tool, read that tool's live zod input schema directly — never assume a parameter exists because a docs page or a prior step implies it.