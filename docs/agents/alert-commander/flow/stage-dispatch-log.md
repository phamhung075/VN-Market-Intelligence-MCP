...
> **Concurrent-slot NNN-collision note (alert-commander, discovered live 2026-08-24T08:07Z):** this agent runs ≥2 independent slots (`alert-commander-market` every 15min, `alert-commander-critical` every 4h) that can genuinely overlap within the same wall-clock minute, each reading the notebook's highest `c<NNN>` independently before either has written — both then compute the SAME next `<NNN>` (live instance: two `c225` headings landed seconds apart, one per slot, plus a stale `c222` retention-cap breach from the same race). No CAS/mutex guards this read-then-write step (unlike `orch-apply.sh` for `orch-state.json`). Self-correct on detection: keep the chronologically-EARLIER same-`<NNN>` section as-is, renumber the LATER one to the next free `<NNN>`, then re-apply AC-2 retention (drop oldest until ≤3 sections) across the reconciled set — never drop content, never silently overwrite a peer's section. This is a live gap, not yet a mechanical fix (candidate: route notebook `<NNN>` assignment through `commit-mutex` critical section rather than a bare pre-commit file read).
