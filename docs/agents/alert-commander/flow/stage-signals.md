...
> **Field-opacity note (alert-commander, discovered live 2026-08-07T04:12Z):** same opacity as the chain_catalyst `confidence`/`regime_adj_score` gap documented below — `get_agent_signals(signal_type="urgent_news")`'s text render carries NO literal `conviction`/`confidence_score`/`regime_adj_score` token at all (only `Mức độ ảnh hưởng: N/10` impact score and the `pillars=...` tags), so this table's "conviction ≥ regime threshold" check has no numeric input to compare for a bullish per-ticker urgent_news signal. In practice — and consistent with the pre-existing tribal pattern already visible in `docs/agent-memory/notebooks/archive/alert-commander-archive-20260729.md` (VNM #9199 evaluated as "THẬN TRỌNG/50%" against the watchlist-opportunity gate, not against a Signal Matrix conviction number) — skip trying to derive a Step 3 conviction for `urgent_news` and instead evaluate the named ticker directly against the cycle.md Firing Gate (watchlist-opportunity's `kinhDichConfidence`/`kinhDichSignal`/`agentSignalsMajority`, via `get_kinhdich_reading` + bus majority check). Live instance: signal #10433 (VNM +5.93% SOE-inflow, impact 8/10) — no conviction field present; VNM kinhDich confidence 38% (GIU, not BUY) failed the gate directly, correctly suppressed without ever needing a Step 3 conviction number.
...
> - **Market-wide vs sector-specific (alert-commander, discovered live 2026-07-23T12:10Z):** naming a real external catalyst (per the clarification above) is necessary but not sufficient for the carve-out — the carve-out is additionally reserved for advisories that plausibly affect the whole portfolio, not a single-sector tailwind. Live instance: chain_catalyst id9062 "Oil prices rise 5 days on US-Iran tensions" cleared confidence (0.80 ≥ 0.75) and named a genuine external catalyst (US-Iran tensions/oil supply, unlike the domestic recaps above), but its content was explicitly sector-specific ("VN oil_gas sector bullish positioning"), not market-wide — and live `get_market_snapshot` that cycle showed a broadly positive session (VN-Index +1.85%, breadth 189 up/122 down), contradicting the signal's own POL:bearish pillar and confirming the impact was narrow, not broad. Firing a "market-wide advisory/opportunity" message for a single-sector signal would misrepresent it as portfolio-wide relevant — suppressed under the standard 2026-07-17 no-ticker-bullish-suppression default instead.
