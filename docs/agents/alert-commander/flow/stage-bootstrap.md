...
> **Clarification (alert-commander, discovered live 2026-08-07T03:58Z):** the live `docs/data/cycle-snapshot-<HH:MM>.json` shape carries only `tick`, `created_at`, `market_context`, `macro_snapshot` — it never carries an `agent_signals`/`cycle_bootstrap` field, on a fresh-hit tick-snapshot as much as a stale/missing one. A `CYCLE_SNAPSHOT` hit therefore only ever substitutes for the `market_context`+`macro_snapshot` portion of Step 0/1 — it does NOT supply Step 3's Signal Matrix input. Regardless of `CYCLE_SNAPSHOT` hit or miss, call `get_agent_signals(agent="alert-commander", status="all", hours_back=2)` directly to populate the bus signals `stage-signals.md` § 3 evaluates.
