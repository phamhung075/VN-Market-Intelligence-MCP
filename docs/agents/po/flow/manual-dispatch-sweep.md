...
Log the full list (even if PO only acts on the top entry this tick — visibility matches the report script's own "surfaced, never silently assumed covered" discipline): `"[po] MANUAL-DISPATCH candidates: {N} ({n_drs} DRS-stranded backlog, {n_xor} ready-XOR, {n_reflag} re-admitted stale-flagged) — {top_id} selected this tick"`. Empty list (nothing eligible, or everything already flagged fresh and awaiting its BATCH turn within the staleness window) → log `"[po] MANUAL-DISPATCH: 0 eligible candidates"`, no-op, proceed to Step 0-TNB.
