...
| Bash | READ-ONLY health probes ONLY, plus 2 named write-adjacent exceptions below. Permitted: docker ps, docker inspect, docker stats --no-stream, docker logs --since, docker events (read), docker exec mcp-server <sqlite3/curl/ls/which/tesseract>, curl -sf (health endpoints), df -h, free -h, `grep`/`wc`/`comm` against the notebook path (§Notebook write's `c<NNN>` derivation, `docs/agents/system-auditor/flow/main.md` — FIX-AUDITOR-NOTEBOOK-COMPOSE-ACTUATOR-BUILT-TESTED-NEVER-WIRED, 2026-08-14: superseded the old Step 1a/2a snapshot+corruption-check, which no longer exist), `bash scripts/notebook-compose.sh <notebook-path> <new-section-body-file> [max-sections] [section-cap]` (the ONE authorized compose actuator — §Notebook write, `main.md`; writes the notebook atomically, never a Claude Write/Edit tool call), `bash scripts/auditor-notebook-commit.sh <msg> <notebook-path>` (the ONE authorized git write — commits the notebook; FIX-AUDITOR-COMMIT-MUTEX-SKIP). FORBIDDEN: docker stop, docker kill, docker rm, docker restart, docker compose down, docker compose up, kill, pkill, killall, rm -rf <any live dir>, and — FIX-SYSAUDITOR-NOTEBOOK-COMPOSE-ACTUATOR 2026-08-06 — any raw/narrated `git add`/`git commit` targeting the notebook path outside `scripts/auditor-notebook-commit.sh` (a bare `git commit` bypassing this script tripped sweep-guard 3s before the `0fcc6a5d2` corrupting commit landed), and — for Tier-1/2/3/5 cycles ONLY (the fire-election tiers rewired 2026-08-14; Tier-4/D-FLEET's separate `handlers.md` §FA-6 pilot notebook-append is untouched by this change and still uses its own narrated Write, out of this pilot's PO-closed scope) — any raw/narrated `Write`/`Edit` tool call targeting the notebook path outside `scripts/notebook-compose.sh` (the notebook write path for those tiers is now that one script — a narrated Write bypassing it reintroduces the exact reproduction-burden defect class the script exists to remove). Violation = abort cycle, send_telegram(bug, "PLAN-ONLY violation aborted: <command>"). |
...
**Reconciled 2026-08-14 (FIX-AUDITOR-NOTEBOOK-COMPOSE-ACTUATOR-BUILT-TESTED-NEVER-WIRED AC-2 item 3):** this section previously stated "read-only, never writes locks" — that had already drifted from the live flow, which uses `task_claim`/`task_heartbeat`/`task_release` for §Step 0d fire-election (pre-existing, since before this task existed) and now also for §Notebook write's compose-mutex (new, this task). Two legitimate, narrowly-scoped write uses — not a general lock-write grant:

| Tool | Purpose | Key Params |
|------|---------|-----------|
| `task_list_held` | Audit current lock state (expired=true to surface orphans) | `kind?, owner_agent?, expired?` |
| `task_claim` | (a) §Step 0d fire-election — win the current tier's tick boundary before doing any tier work. (b) §Notebook write AC-4 — claim the compose mutex before deriving `c<NNN>`/touching the notebook | (a) `task_id=FIRE_TASK_ID ("cron:auditor-t<N>:<FIRE_TICK>"), task_kind="sprint-task"`. (b) `task_id="commit-mutex:system-auditor-notebook", task_kind="commit-mutex"` |
| `task_heartbeat` | Re-entrant renewal on either lock above (same-session restart mid-tick / mid-compose) | `task_id` (as above), `owner_client_session` |
| `task_release` | Explicit release of either lock above at end of cycle / immediately after the compose call | `task_id` (as above), `owner_client_session` |
