...
No `ISSUE` set AND all checks pass AND (if OOM-class) the Durability Gate D1-D5 satisfied with D5 fields written → JUMP TO `vc-approved`. Any `ISSUE`, failing check, or unmet Durability Gate condition → JUMP TO `vc-changes`.
