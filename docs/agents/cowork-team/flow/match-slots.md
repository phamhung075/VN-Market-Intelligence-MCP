<!-- size-justification: 107L — cohesive slot-matching block: resolve UTC, run matcher script (+UC-CDC-P7 Phase 2a metadata capture), drift guard, silent-exit if no matches, collision-detection guard. Child of main.md. FIX-CHEF-MARKER-KEY-ANCHOR-2 2026-08-23 (agent-father): +9L (98→107) — `scheduled_utc_time` documented on `slots[]`, previously documented only on `catchup_raw[]` even though ANCHOR-1 now emits it on both; the null-degradation contract and the deliberate `scheduled_key_part` asymmetry are the two things a consumer gets wrong without it. In-place growth in the existing Script SSOT paragraph, no new section. TASK-COWORK-DOC-TRUTH-LAYER-INVENTORY AC-2 2026-08-23 (agent-father): +8L (107→115) — the `catchup_raw` paragraph's scope corrected (guaranteed-only = 8 of 21 enabled slots) and its REACH limit recorded: `catchup-check.md` does not exist AND `catchup_max_lateness_minutes` caps recovery at 1 VN day (live values 60/120/180/360/1440), so wiring the consumer alone would not have recovered the multi-day outage that motivated the parent row. Eligibility counts are stamped with their measurement time because they move within a single day (8 records/0 eligible early, 8 records/2 eligible later, same date). -->
...
`catchup_raw` (FR-9a, TASK-COWORK-CATCHUP-2): catch-up candidates computed by `cowork-catchup-predicate.js` — array, empty on the common no-catch-up tick, one record per considered slot not already in `.slots[]` this tick: `{slot_id, dish_type, agent, flow_path, trigger_prompt, guaranteed:true, scheduled_utc_time, scheduled_key_part, expected_publish_task_id, catchup_eligible, reason}`.

**Scope correction + reach limit** (TASK-COWORK-DOC-TRUTH-LAYER-INVENTORY, agent-father 2026-08-23 — figures re-measured that date):

- **Scoped to `guaranteed:true` slots only** — 8 of the 21 enabled slots. The other 13 have no catch-up path at all, in any layer. This is a deliberate scope, not an oversight, but it is the reason a miss on a non-guaranteed slot is structurally invisible to this array.
- **Step 4.55 is not merely unwired — as specced it would also recover little.** `docs/agents/cowork-team/flow/catchup-check.md` does not exist (`ls`-confirmed), so nothing consumes `catchup_raw` today. But eligibility is separately capped by `_dish_type_catchup_config.catchup_max_lateness_minutes`, live values **60 / 120 / 180 / 360 / 1440 minutes** — a **1-day ceiling at the most generous** — plus a `rolled_past_vn_date` check. Against the measured multi-day outage that motivated this row, every candidate ages out. Wiring the consumer without also revisiting these bounds recovers a single VN day at best.
- **Measured, with its timestamp, because it MOVES:** a run at 2026-08-23T15:1xZ returned **8 records, 2 eligible** (5 `rolled_past_vn_date`, 1 `freshness_window_exceeded`). An earlier same-day run returned 8 records, **0** eligible. Do not restate either number as a standing property of the array — re-run `node scripts/agents-flow/cowork-match-slots.js | jq .catchup_raw` before quoting a count in an incident note.

Design ref: `docs/architecture-briefs/2026-07-22-cowork-guaranteed-slot-catchup-design.md` §2.1/§2.3; layer inventory: `docs/architecture-briefs/2026-08-23-cowork-slot-durability-layer-inventory-and-miss-detection.md`.
