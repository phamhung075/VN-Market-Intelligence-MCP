...
MULTI-VIOLATION-MSG / STRUCTURAL-FIELDS-EXCLUDED / usage-error cases) — verified read-only against the
real live `orch-state.json` (zero regression risk: 42 pre-existing over-ceiling rows all resolve to
non-blocking WARN, exit 0) before wiring into `orch-apply.sh`.
...
Lazy-load full detail for one id (any of the 3 lanes): `jq '.items["<id>"]' docs/data/orch/archive/backlog-detail.json`
