...
#   FIX-ORCH-COLD-EVICT-VALIDATION-EXIT1 (2026-08-29): the CAS re-check now runs BEFORE the
#   Stage 2 conservation check — a candidate built from a stale snapshot (the exact condition
#   the CAS guard detects) is indistinguishable from a genuine conservation violation against
#   the fresher live file, so pre-reorder it aborted exit 1 fatally instead of retryably;
#   orch-cold-evict.sh hit this live (telegram 5209, 2026-08-26). See the CAS-baseline entry.
...
BEFORE the CAS re-check — which itself now runs BEFORE Stage 2 conservation check
(FIX-ORCH-COLD-EVICT-VALIDATION-EXIT1, 2026-08-29: the CAS check must precede every live-relative
gate so a stale candidate aborts retryable exit-2, not fatal conservation exit-1).
...
Wired into `scripts/orch-apply.sh` as Stage 2.5, after the conservation check (Stage 2). The CAS
re-check previously sat after this guard too; it now runs BEFORE Stage 2 (FIX-ORCH-COLD-EVICT-
VALIDATION-EXIT1, 2026-08-29 — a stale candidate must abort retryable exit-2 via CAS, not fatal
exit-1 via a false conservation violation against the fresher live file). Owning brief:
