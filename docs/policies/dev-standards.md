...
**Extension — durable pending-triage-inbox dimension (FIX-DEVTEAM-IDLE-CHAIN-TEST-DURABLE, 2026-08-09,
brief §3.4):** `signal_total(doc)` now sums `length(signal_queue.rows)` **+**
`length(dev_team_idle_chain.pending_triage_inbox)` — the durable inbox
(`FIX-DEVTEAM-IDLE-CHAIN-P2A-DURABLE-DRAIN`, 2026-08-08) is a second not-yet-triaged-signal
holding area, structurally identical in kind to `signal_queue.rows[]`, that was invisible to the
pre-2026-08-09 formula: a bug in the inbox's own append/clear logic (`docs/agents/dev-team/flow/
main.md` § Step 1 — PO Triage "Durable-inbox CLEAR", or `drain-signals.js`'s
`appendDurableBatch()`) that silently wiped the whole array would sail through this circuit-breaker
undetected as long as `signal_queue.rows[]` itself stayed intact — the same blind spot the original
row-identity/magnitude guards above close for `signal_queue.rows[]`, now closed for its durable-inbox
sibling. Summed into ONE metric (not a third independent one) because both arrays are the same
conceptual "signals not yet delivered/consumed" quantity. Test: `scripts/agents-flow/
drain-signals-durable.test.js` § Conservation Guard Extension (silent 10→0 inbox wipe with
`signal_queue.rows` unchanged is now REJECTED — regression-proof this wipe class was previously
invisible; single-entry append/consume both remain unblocked) — plus regression re-run of
`bash scripts/test/orch-apply-wrapper-tests.sh` (75/75 unaffected) and `bun test
...
46/46 assertions passing. Isolation: every scenario builds its own `mkdtemp` harness (real
...
line, AC-1 isolation, no-claim no-op, bundled-commit regression); `auditor-notebook-commit.sh`'s AC-4
path has no MCP-dependent test harness (script has none — verified via disposable scratch-repo +
stubbed `mcp_call` manual smoke run, not a persisted `.test.sh`, per the owning board row's
`baseline_pass` scope). Owning flow: `docs/protocols/fail-loud-protocol.md` § Analysis-Only Exit
Guard, `docs/agents/system-auditor/flow/main.md` § Commit / § ANALYSIS-ONLY-EXIT GUARD.
