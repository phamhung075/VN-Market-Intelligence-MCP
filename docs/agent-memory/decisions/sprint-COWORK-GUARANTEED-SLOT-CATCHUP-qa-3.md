...
### STEP qa-S51 · qa · 2026-08-06T17:43:13Z
**task-id:** FIX-CHEF-EVENING-DUP-DATE-MISLABEL-INVESTIGATE
**what-done:** Direct-commit verify. Derived commit 8cb767471 via git log on chef.md/cowork-schedule.json (no `.commit` field on row), matches review_note prose exactly: chef.md Step 0.5 pins ONE `CYCLE_DATE_UTC`, reused verbatim at Step 7.6 filepath+date_vn, Step 8b header, marker key; cowork-schedule.json publish_date_basis vn_date->utc_date for morning/eod/evening. AC(1)/(2) date-consistency CONFIRMED: sampled all 9 evening synthesis JSONs 07-29->08-06 post-fix — zero VN-local leaks, filepath/date_vn/marker-key always mutually consistent = UTC date of execution, incl. today's 08-06 06:37Z dish.
**what-considered:**
- AC(3) "exactly one published:chef-evening:<date> marker per real calendar day" — NOT held. Found unified-agent-synthesis-2026-07-30-evening.json (cycle_id evening-2026-07-30T19:52:43Z) + -2026-07-30-chef-evening.json (cycle_id evening-2026-07-30T19:59:12Z): two independently-computed full evening dishes 6m29s apart, SAME correct UTC date (no date-mislabel — different mechanism), divergent regime_state/convictions — chef.md gates Step 0 GATHER behind the marker claim, so both reaching Step 7.6 JSON persist means both claims succeeded; SLOT_ID is not canonicalized at Step 0.5 (only CYCLE_DATE_UTC was fixed), so this row's own mutex-key derivation still lets a same-day double-dispatch through.
- Fresh mechanism-linked evidence found live in orch-state.json signal_queue (id cowork-20260806T064440Z-chef-evening-retry-date-drift, filed today 06:44Z, HIGH, to po, RAW-verified via get_market_message_digest): pinning CYCLE_DATE_UTC to wall-clock-at-execution (this row's exact design) broke retry/backfill semantics — a ~10h43m-delayed retry targeted TODAY instead of the missed day, permanently orphaning 2026-08-05 evening + prematurely tombstoning 2026-08-06's marker 13h early.
