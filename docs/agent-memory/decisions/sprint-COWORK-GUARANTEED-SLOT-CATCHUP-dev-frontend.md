...
### STEP dev-frontend-S5 · dev-frontend · 2026-08-24T19:16:00Z
**task-id:** FIX-DASH-CRON-LAYERB-NEVERFIRED-FALSE-LABEL
**what-done:** Layer-B last-fire cell checks `row.layer==="cli-session"` first, renders new named exports `CRON_LAYER_B_LAST_FIRE_LABEL`/`_HINT` instead of falling into the Layer-A "Chưa từng chạy" ternary; dropped redundant per-row `Lớp` column; fixed brief doc L127; made test predicate layer-aware.
**what-considered:**
- AC-5 `Lớp` column removal: verified the two pinned tests (test:287 CRON_STATUS_LABELS, test:337 cronLayerLabel) are pure-function tests, not DOM assertions on the column — removal is conflict-free since AC-18's two-sub-table split already exists live.
- AC-3 copy placement: tooltip-only (existing `<tr title>`) vs always-visible inline hint — chose always-visible `<span>` block under the label per AC-3's explicit "not tooltip-only" requirement.
**why-decision:** Both AC-1 (primary) and the non-blocking AC-5 were cleanly resolvable without touching the Layer-A branch or the normalizer (AC-4 guard) — did both since AC-5's precondition (AC-18 split) was already shipped.
**why-change:** No change from task brief. Full vitest 2188 pass/2 fail (same pre-existing unrelated QUE_DESCRIPTIONS fails, +5 net new tests all green); tsc clean; render-check.spec.ts (G12) 3/3 PASS; full `playwright test` also surfaced 2 pre-existing unrelated live-data-content fails in quality-audit-lastverified.spec.ts (zero file overlap, confirmed via direct code read not stash — notebook flags `git stash` unsafe in this shared-main tree).
