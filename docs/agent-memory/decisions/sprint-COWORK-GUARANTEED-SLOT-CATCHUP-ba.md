...
### STEP ba-S16 · ba · 2026-08-22T18:52:53Z
**task-id:** FIX-GHOSTZONE-CONVICTION-ASC-LIMIT-TRUNCATES-NEWEST
**what-done:** Decomposed po's ghost-zone dispatch into 2 FRs + 2 NFRs: newest-N row-selection replacing the `ORDER BY date ASC LIMIT 2000` truncation, WITH a preserved ASC return-contract (buildSnapshot/buildSeries in convictionHistoryHandler.ts silently depend on it — a naive DESC flip would corrupt every symbol's snapshot to its OLDEST score, not just fix staleness). Spec: `docs/handoffs/FIX-GHOSTZONE-CONVICTION-ASC-LIMIT-TRUNCATES-NEWEST-BA-spec.md`. Row updated in place (`backlog[]`, no lane move — matches BA-ANALYSIS-QUALITY-CONVERGENCE precedent): `ba_spec_complete`, `ba_handoff`, `ba_completed_at`, `next_agent=agents-architect` via `orch-apply.sh` (conservation 715↔715).
**what-considered:**
- Ship the ticket's literal "order DESC" phrasing vs verify caller assumptions first: verified `TASK17-CONVICTION-conviction-history-endpoint.test.ts` AC-2/AC-5 — both encode an ASC-input assumption the ticket text didn't name.
- Block on PO for the calendar-window-vs-row-count freshness durability question (NFR-2) vs route as architect-owned: routed to architect — it's an engineering trade-off, not a priority call, and PO's own ticket already ratifies the general fix direction.
**why-decision:** A store-only, ASC-contract-preserving fix keeps this a one-file change; flipping order without checking the caller would ship a WORSE bug (wrong-direction data) that passes existing tests silently, since no test asserts input order into buildSnapshot directly.
**why-change:** No change from po's dispatch scope. Dispatched together with the sibling foreign-flow row (shared "served MAX(date) must match table MAX(date)" regression shape) per router instruction; specs written independently, no code overlap (different files). Session had no MCP tool binding (gateway/vn-market absent) — Read/Edit/Write/Bash only, same known limitation as prior 2026-08 cycles.

### STEP ba-S17 · ba · 2026-08-22T18:52:53Z
**task-id:** FIX-GHOSTZONE-FOREIGN-FLOW-MAXDATE-MISSING-NONNULL-GUARD
**what-done:** Decomposed po's ghost-zone dispatch into 1 FR + 2 NFRs: move the `foreign_volume IS NOT NULL` guard into the `MAX(date)` subquery in `foreignFlowHandler.ts`, matching the handler's own docstring contract (line 8) that the implementation already contradicted. Spec: `docs/handoffs/FIX-GHOSTZONE-FOREIGN-FLOW-MAXDATE-MISSING-NONNULL-GUARD-BA-spec.md`. Row updated in place (`backlog[]`, no lane move): `ba_spec_complete`, `ba_handoff`, `ba_completed_at`, `next_agent=agents-architect` via `orch-apply.sh`, same write as ba-S16 (conservation 715↔715).
**what-considered:**
- Open new backlog rows now for the two cascade symptoms (momentum-indicators foreign_accum null, money-radar coverage_pct 0.83) named in po's own ticket note vs defer: deferred as NFR-2, a re-measure-first verification step — they may self-resolve once this handler stops serving an empty day, ticket text explicitly says re-measure before opening separate rows.
- Recommend extracting the embedded SQL into a separate infrastructure/db store (matching convictionHistoryStore's layering) vs leave as-is: left as-is, flagged as an observation only — a one-line predicate fix does not justify a layering refactor in the same commit.
**why-decision:** The docstring already states the exact intended contract verbatim, so this is a one-line implementation correction with zero open design question, not something needing PO or architect design input beyond ratifying the one-line change.
**why-change:** No change from po's dispatch scope. Session had no MCP tool binding (gateway/vn-market absent) — Read/Edit/Write/Bash only, same known limitation as prior 2026-08 cycles.

### STEP ba-S18 · ba · 2026-08-23T12:49:05Z
**task-id:** FEAT-BCTC-INSPECT-QUARTER-TICKER-FILTER
**what-done:** Decomposed po's 10-AC feature note into 8 FRs + 4 NFRs, all landing in the interface layer (no domain/application/infrastructure touch — pure client-side facet filter + one cosmetic server-side label fix). Live-verified: `DocListItem.period_quarter` (typed `number|null`) is the STRING `"Q1"` on 2/257 rows (both HUT) — closes AC9's landmine at both the client normalizer and `buildLabel()`. Flagged FR-2 (module-level `allDocs` cache does not exist today, needed for AC2's zero-network-call constraint) and FR-7 (selection-preservation must not route through the existing change-handler, which unconditionally refetches PDF/OCR/table/MD). Spec: `docs/handoffs/FEAT-BCTC-INSPECT-QUARTER-TICKER-FILTER-BA-spec.md`. Row updated in place (`backlog[]`, no lane move): `ba_spec_complete`, `ba_handoff`, `ba_completed_at`, `next_agent=architect` via `orch-apply.sh` (conservation 767↔767).
**what-considered:**
- Trust PO's note that the quarter landmine only affects the label (AC9) vs re-derive the facet count independently: re-verified live against `bctcInspectHandler.ts` — confirmed the SAME string-vs-number split also inflates the quarter-facet option count (13 vs 11) if unnormalized, so FR-3/FR-4 explicitly forbid deriving quarter facets from raw `period_quarter`.
- Recommend a shared client/server module to de-duplicate the quarter-coercion logic vs accept duplication: accepted duplication (NFR-4) — the page has no build step / import mechanism to share code with the TS backend; building one is out of scope for an S-size ticket.
**why-decision:** FR-7's selection-preservation guard is the one real implementation-risk item (AC6) — flagged explicitly so the filter-triggered doc-select rebuild is not implemented by re-firing the existing change handler, which would violate "no refetch, no flicker" on every filter change even when the resolved doc is unchanged.
**why-change:** No change from po's dispatch scope; PO's own note already forecloses backend work, deep-linking, and the WRITE-path fix as out of scope. Spec adds FR-2 and FR-7's implementation detail at the code level. Zero PO blockers — chain is ba → architect → pm → dev-mcp-server → qa per po's own note.
