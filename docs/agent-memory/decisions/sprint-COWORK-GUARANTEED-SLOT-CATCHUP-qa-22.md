...
### STEP qa-S153 · qa · 2026-08-14T17:33:51Z
**task-id:** FU-CTG-DISCOVERY-FILENAME-FILTER
**what-done:** Direct-Commit Verify (`qa[]` row, `.commit_sha`=`1c9959cdd1` present, no `.files[]` — derived via `git show --stat`). Commit confirmed real, on `main` ancestry (`git merge-base --is-ancestor` true), touches exactly the two files described (`vps-scripts/discover-bctc-urls-browser.py` + new `test_discover_bctc_filename_classifier.py`).
**what-considered:**
- Non-bun zone (vps-scripts): script has stdlib-only imports (no 3rd-party deps), so no docker image / dependency-drift risk applies — host `python3` == VPS runtime for this file; ran directly, not blocked by the pytest/mypy-in-image rule (that rule targets dockerized microservices like rag-service/pdf-extractor).
- Re-ran REAL verification, not trusted from row prose: new suite 15/15 pass, full `vps-scripts/` suite 57/57 pass (matches claim exactly), `py_compile` clean on both touched files, `mock-guard.sh --files vps-scripts/discover-bctc-urls-browser.py` exit 0 PASS, no `process.env`/hardcoded-secret patterns.
- Read the diff itself (not just the summary): `is_cover_letter_filename()` correctly checks only the final path segment post query-strip+URL-decode, case-insensitive, for `cv_cbtt`/`cong_van_cbtt`; wired into `_fetch_pdf_url()`'s href loop so a rejected cover-letter attachment does not short-circuit — scanning continues for a later legit href in the same ArticlesFileAttach response. Matches AC and the article-613699 repro described in the commit message.
**why-decision:** vc-approved, DONE_VERIFIED. All checks green, logic sound on independent read, no apps/mcp-server change needed (scope claim confirmed — fix fully contained in vps-scripts/). VPS deploy correctly withheld per standing user-gate policy; not a QA blocker for a main-commit verify (no merge/push/deploy step in this JUMP-TO).

### STEP qa-S154 · qa · 2026-08-14T17:35:45Z
**task-id:** FIX-COWORK-SUPERSEDE-MUTEX-SCRIPT-AND-MATCHSLOTS-WIRING
**what-done:** Direct-Commit Verify (`qa[]` row, `.commit`=`662d1fcc3`+`.files[]` present, router-flagged as already RAW-verified — re-verified independently, not trusted). Commits `662d1fcc3` (feat) + `87e0554db` (dev journal) confirmed real, on `main` ancestry; `git show --stat` matches all 5 claimed files exactly (new `cowork-supersede-mutex.js`+`.test.js`, modified `cowork-match-slots.js`+`.test.js`+`cowork-schedule-consistency.test.js`).
**what-considered:**
- Read the actual diff for AC-1/AC-2/AC-5, not the commit message: `applySupersedeMutex` reads only `.supersedes` off scheduleSlots, drops named victims present in `matches`, S self-ref guarded (TC-6), mirrors `cowork-chef-mutex.js`'s exact `fail()` envelope shape/exit-1 contract (byte-diffed the two files side by side). AC-4: `grep -n "finish("` shows all 3 `matchSlots()` return points (legacy, legacy-fallback, adaptive) route through `finish()`, which now chains `applySupersedeMutex` after `applyChefMutex` unconditionally. AC-5: wiring diff shows `require('./cowork-supersede-mutex.js')`, no shell pipe at the live call site.
- Re-ran independently (not trusted from router note): `cowork-supersede-mutex.test.js` 32/32, `cowork-match-slots.test.js` 69/69, `cowork-schedule-consistency.test.js` 13/13 (incl. new AC-6 typo-guard TC-8..11, passes against live `docs/data/cowork-schedule.json` which the sibling row already wired `market-watcher-eod.supersedes=["market-watcher-offhours"]` into — confirmed via `jq` on live file), `cowork-chef-mutex.test.js` 25/25 confirmed byte-identical/untouched (`git log -1` on that file predates this commit). `mock-guard.sh` PASS on both touched production files. `tsc --noEmit` N/A — `scripts/agents-flow/` outside `tsconfig.json`'s `include` (plain Node zone, confirmed by reading tsconfig).
- Scope check: `git show --name-only` on both commits confirms zero touch to `cowork-chef-mutex.js`/`.test.js`, `docs/data/cowork-schedule.json`, `docs/agents/**`, `orch-state.json` — non_goals honored; those live in the sibling row's already-merged commit `5918c55fe`.
**why-decision:** vc-approved, DONE_VERIFIED. AC-1 through AC-6 all independently confirmed against source, not prose. Zero blocking issue.
**why-change:** none — router's RAW-verify note held up under independent re-derivation.

### STEP qa-S155 · qa · 2026-08-14T17:36:00Z
**task-id:** FIX-LANCEDB-INSERT-SEGFAULT
**what-done:** Direct-Commit Verify (`qa[]` row, no `.files[]`/`.owner` drain fields — both derived from `review_note` prose per fallback path). Commit `41f522117` confirmed real, on `main` ancestry; `git show --stat` matches all 4 review_note-claimed files exactly (`insertBctcAnalysis.ts`, `types.ts`, `fetchParseAndStoreBctc.ts`, new test `FIX-LANCEDB-INSERT-SEGFAULT-step4-guard.test.ts`). Companion memory commit `32279f9a0` also on `main` ancestry.
**what-considered:**
- Independently re-derived the root-cause reversal (not trusted from prose): live `grep -rn "@lancedb/lancedb" apps/mcp-server/src` returns only 2 comment-only hits (both inside this task's own new test file / infra comments), zero production import statements — confirms a native segfault on this path is categorically dead today, matching the claimed d29da3a8d (G5b HTTP rewire) + 456851797 (CI-RED-da847805-FIX) supersession chain.
- Read the diff itself, not the summary: `insertAnalysisFn/insertAnalysisResolverFn` resolution line genuinely moved from OUTSIDE to INSIDE the try block in `insertBctcAnalysis.ts`, new `insertAnalysisResolverFn` injection seam added to both `types.ts` and threaded through `fetchParseAndStoreBctc.ts`'s Step-4 call — matches the "everything injectable" claim exactly, no scope creep.
- Re-ran REAL verification: new test file 2/2 pass (matches AC1/AC2 RED→GREEN claim); AC3 regression subset (1181/1352a/1352b/p2-f-rag-http-rewire + new file) 33/33 pass, byte-identical count to claim; `bun tsc --noEmit` clean (0 output); `mock-guard.sh --files <3 touched prod files>` exit 0 PASS; secrets/`process.env` grep clean on all 4 touched files; DDD scan N/A — all touched files are `application/usecases/` (which the file's own header documents may import both `domain/` and `infrastructure/`), zero `domain/` layer files in scope. Did not re-run the full 14974-test suite (575s, pre-existing flake band already QA'd elsewhere) — targeted regression matches the JUMP-TO's own "touched test file(s), else targeted zone suite" instruction.
**why-decision:** vc-approved, DONE_VERIFIED. Root-cause correction is sound and independently reproducible, real live guard-gap genuinely fixed, all gates green on re-derivation not prose-trust.
**why-change:** none — verdict matches the row's own review_note claim.

### STEP qa-S155 · qa · 2026-08-14T17:36:30Z
**task-id:** FDA-7
**what-done:** Direct-Commit Verify (`qa[]` row, no `.commit`/`.files[]` drain fields — commit derived from `.commit_sha`=`24022a53a`, files derived from `backlog-detail.json#FDA-7`'s own `files[]` claim, single file). Commit `24022a53a` confirmed real, on `main` ancestry; `git show --stat` matches the claimed file exactly (`macroTools.ts`), plus a 2nd file (`macroSnapshotGuard.ts`) + test + doc in the same commit, all consistent with the FIX scope.
**what-considered:**
- Read the diff itself, not the row's `return_summary` prose: `source_tier` fallback genuinely flipped `?? 2` → `?? 4` (conservative/unknown) when no present `signals.*` component carries a tier; `fetchedAt` genuinely flipped from `?? new Date().toISOString()` (fresh-stamp) to `?? null` (fail-loud absence) — exact match to the backlog note's own stated FIX. `macroSnapshotGuard.ts` JSDoc updated in step (guard intentionally still only gates on `text`, doc comment only — no logic change there).
- Re-ran REAL verification: `089-tool-macro.test.ts` 21/21 pass (incl. the 4 new FDA-7 RED→GREEN tests: null fetchedAt on omission, tier=4 when signals present-but-tierless, tier=4 when signals entirely absent, no-regression real fetchedAt passthrough — read each assertion, not just the pass count). `bun tsc --noEmit` clean (0 output). `mock-guard.sh --files macroTools.ts macroSnapshotGuard.ts` exit 0 PASS. DDD: `interface/mcp/tools/macro/` importing `infrastructure/logger.js`+`fetchDeadline.js` is pre-existing/untouched by this diff's hunks and not a domain/ file — no violation. `process.env`/secrets grep clean on both files.
**why-decision:** vc-approved, DONE_VERIFIED. Zero blocking ISSUE — fix matches AC exactly, tests are real and meaningful (not trivially-true), no regression on the existing-fetchedAt-present path.
**why-change:** none — verdict matches the row's own `return_summary` claim.

### STEP qa-S154 · qa · 2026-08-14T17:35:00Z
**task-id:** FDA-10
**what-done:** Direct-Commit Verify (`qa[]` row, `.commit_sha`=`ec27c69d3108` present, no `.files[]` on board row — derived via `backlog-detail.json#FDA-10.files[]`, single file `shippingIndex.ts`).
**what-considered:**
- Commit real + on main ancestry (`git merge-base --is-ancestor` true), `git show --stat` touches exactly the claimed file, no other files.
- Diff content re-read directly (not trusted from prose): deletes stale "SCFI proxied via TE or BDI-related symbol" line (L7) + corrects SHIPPING_SYMBOLS JSDoc (L73-77) to state SCFI has no free ticker and is NOT fetched/proxied — matches status_note verbatim. Current-file grep confirms only the corrected line remains, no residual misleading claim.
- Re-ran REAL verification: `bun test src/__tests__/252-shipping-index.test.ts` 8/8 pass (matches claim), `bun tsc --noEmit` clean, `mock-guard.sh --files shippingIndex.ts` PASS. Comment-only edit, SHIPPING_SYMBOLS/fetch logic untouched — confirmed by diff (no logic lines changed).
**why-decision:** vc-approved, DONE_VERIFIED. All checks green, diff matches self-report exactly, no arch/domain concern (comment-only cleanup).
**why-change:** none — verdict matches the row's own status_note claim.

### CAP-REACHED · 2026-08-14T17:37:05Z
