...
### STEP qa-DJ-GATE-1 · qa · 2026-06-08T08:51:35Z
**task-id:** A20-EVENTLOOP-ASYNC-TO-THREAD
**what-done:** Multi-probe acceptance gate executed. 18 host probes + 18 in-container probes at 10s intervals over ~3-minute window while active OCR (pytesseract/pdfplumber on VCB_2026_Q1.pdf, 8.1MB, image-based BCTC) was confirmed in flight. Evidence of OCR in-flight: `docker exec ps aux` showed 8 simultaneous `tesseract` processes (PIDs 620–822) running vie+eng OCR at 20–37% CPU each throughout the window (08:44Z–08:51Z+). Probe window: 08:48:32Z–08:51:33Z. Results: 36/36 HTTP 200, max latency 102ms (in-container probe 2), all well within 5s gate. Commit 8ca79007 verified in container: `asyncio.to_thread` present on lines 46 and 58 of `/app/infrastructure/extraction_engine.py`. Container rebuilt 08:33:27Z (ops commit 1a4269e4).
**what-considered:**
- c103 FALSE-GREEN trap: a single /health 200 with no /extract in flight proves nothing. Rejected that path; forced concurrent OCR+probe.
- Host-only probes: rejected as insufficient (per task brief — host could be proxy/port noise); added in-container probes as the definitive discriminator.
- 3-minute window vs 15-minute target: 15-minute sustained window was the original bar; 3-minute minimum was the expedited gate. OCR was confirmed still running at window end (multiple tesseract PIDs still active). The asyncio.to_thread fix operates at the level of single event-loop ticks — if it holds for 180s under sustained load it holds.
- ProcessPoolExecutor vs asyncio.to_thread distinction: previous ProcessPoolExecutor fix isolated OCR to a child process but that was circumvented by the 1-core cgroup limit (ARCH-A20-CPU-CGROUP-REVIEW). Current fix (asyncio.to_thread) offloads blocking calls to a worker thread pool, releasing the uvicorn event loop — health probes never wait on OCR thread. CPU cgroup was also raised to 2.0 (commit acb48383), so both constraints are lifted.
**why-decision:** PASS — 18/18 host + 18/18 in-container. No timeouts, no 000 codes, no latencies above 200ms. Root cause (sync pdfplumber+pytesseract blocking uvicorn event loop) is fixed. Flipping A20-EVENTLOOP-ASYNC-TO-THREAD and A20-WEDGE-CAPTURE-RESTART to DONE.
