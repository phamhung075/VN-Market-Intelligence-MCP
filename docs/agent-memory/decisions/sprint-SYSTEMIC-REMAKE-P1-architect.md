...
### STEP architect-S7 · architect · 2026-07-10T19:20:00Z
**task-id:** BACKLOG-HYGIENE-VERIFY-PRUNE-SWEEP
**what-done:** Re-verified PO's root cause live (read `orch-cold-evict.sh` full 553L — confirmed it never scans `backlog[]`); ran `bun scripts/orch-validate.mjs` live (129 warnings, exact match); recomputed the full 7-lane status histogram to independently confirm PO's 55/12/1/58 split (exact match, +3 review-BLOCKED PO didn't enumerate). Spot-checked 6 of the 55+12 rows: 2 confirmed mislabels found in opposite directions (`FIX-BCTC-BANK-SUMMARY-MAPPING` falsely DONE — live-DB-probed the exact defect, still reproducing today; `FACTORY-INTERFACE-split-server-ts` falsely BLOCKED — all 4 shipped commits present in git log).
**what-considered:**
- Trust PO's D1 target (`archive/backlog-detail.json`) as-is — REJECTED: that file is the live-task detail-prose split target (361/384 backlog rows point there for OPEN work), not a terminal-eviction sink; found the correct dormant target (`.backlog_detail[]` in monthly cold archive, already schema-present, always empty).
- Treat D2 "relocate mislaned active rows" as safe mechanical moves — REJECTED after Exception 2 proved a 3rd status-direction (BLOCKED but actually done) exists inside that exact 12-row bucket — folded into D0's per-row rigor instead of a separate blind relocate.
- BLOCKED-status lane-coherence gap (7 rows, no lane accepts BLOCKED anywhere) — new finding, not in PO's scope; designed as D2.5 (extend `LANE_ALLOWED_STATUSES` orthogonal-to-lane) vs rejected new `blocked[]` lane (blast radius against `.strict()` schema + every lane-enumerating consumer) — flagged PO-ratification-required, mirrors ADD-1 precedent.
- Execute the sweep single-shot vs decompose — CHOSEN decompose (PM multi-task sprint, D3/D2.5/D4/D0 parallel-safe, D1/D5 sequenced after): size (385 rows, 2 script/schema changes, 1 PO-gated decision) plus the empirically-proven non-trivial error rate in D0 make single-shot unsafe and out of architect's own-execution mandate.
**why-decision:** Live re-verification changed 2 concrete deliverable targets (archive path, D2 scope) and added 1 blocking sub-task (D2.5) PO's plan had not surfaced — designing off PO's word alone would have shipped a working script writing to the wrong file and left BLOCKED rows permanently un-closeable under D5's hard-fail flip.
**why-change:** Corrects PO's D1 archive-target and D2 scope; adds D2.5 as a new required step; everything else (root cause, counts, D3/D4/D5 shape) confirmed as PO scoped it.
**board-mutation:** none by architect (per role boundary — see brief §7); board row update (next_agent→pm) applied via `scripts/orch-apply.sh` in same cycle as this entry.
**next-agent:** pm — decompose D3/D2.5/D4/D0 (parallel) → D1 (after D0+D4) → D5 (after D3+D2.5+D1, 0 warnings confirmed live). Brief: docs/architecture-briefs/2026-07-10-backlog-hygiene-verify-prune-sweep.md; handoff: docs/handoffs/TASK_BACKLOG-HYGIENE-VERIFY-PRUNE-SWEEP.md
