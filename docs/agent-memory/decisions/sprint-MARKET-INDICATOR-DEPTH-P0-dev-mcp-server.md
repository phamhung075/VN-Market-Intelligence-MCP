...
### STEP dev-mcp-server-S10 · dev-mcp-server · 2026-06-30T22:00:00Z
**task-id:** OHLCV-DEPTH-SUBTASK-D
**what-done:** Added `SPARSE_THRESHOLD=8` and `DEPTH_FLOOR=252` exported constants to `ohlcvStartupProbe.ts`. Extended `OhlcvStartupProbeResult` with `shallowTickers[]` and `shallowSent` fields. Restructured classification loop to bucket tickers into sparse (<8), shallow (8–251), healthy (≥252). Phase 3+4 (queue/backfill) wrapped in `if (sparseTickers.length > 0)` — shallow path never triggers backfill (observability only). Added ONE aggregated shallow WORK alert after the sparse alert path: single `sendWorkFn` call listing all shallow tickers as `CODE(cnt)` pairs mentioning `DEPTH_FLOOR=252`. All 3 early-return paths updated with new fields. Existing sparse path is line-identical in behavior. Updated 3 existing TCs (TC-1 VNM 20→300 healthy, TC-2 15/20→300, TC-5 8→252 boundary) to reflect correct healthy threshold. Added 5 SUBTASK-D TCs (TD-1 sparse unchanged, TD-2 shallow 3-ticker ONE-call, TD-3 healthy no-call, TD-4 5-ticker→calls===1, TD-5 8/252 boundary). Impl commit `45fff717`. tsc EXIT 0. 10 pass / 0 fail.
**what-considered:**
- Early-return restructure: changed `if (sparseTickers.length === 0)` to `if (sparseTickers.length === 0 && shallowTickers.length === 0)` — must not short-circuit when only shallow tickers exist.
- Backfill guard: Phase 3+4 is meaningful only for sparse tickers (shallow tickers have enough data to ingest future bars normally; real depth fill comes from VPS push SUBTASK-A+B). Wrapping in `if (sparseTickers.length > 0)` prevents accidental queue-row insertion for shallow-only startups.
- TC-1/TC-2/TC-5 updates: the original tests used 15–20 row counts which, after adding the shallow bucket, would have triggered a shallow alert and broken `calls.toHaveLength(0/1)` expectations. Updated to 300 rows (healthy) so test intent (no alert when healthy) is preserved. TC-5 boundary shifted from 8→silent to 252→silent to match new DEPTH_FLOOR semantic.
- Alert format: `[ohlcv-probe] daily_ohlcv shallow on boot — tickers need ≥252 bars for momentum: CODE(cnt), ...` — matches spec §2.2-E single-message requirement.
