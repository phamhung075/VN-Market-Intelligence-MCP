...
### STEP developer-S9 · developer · 2026-07-23T00:00:00Z
**task-id:** TE-T17
**what-done:** (1) Pruned `docs/agent-memory/notebooks/ops.md` 1197L→29L: 23 `## ` sections moved verbatim to new `docs/incidents/<date>-<slug>.md` files, replaced with one-line pointers under a single archive-index heading. (2) Root-cause fix: new `scripts/agents-flow/notebook-linecap-sweep.sh` (+ paired test) wired into `code-janitor/flow/main.md` § Notebook Line-Cap Sweep (existing 6h cron) — sweeps every `docs/agent-memory/notebooks/*.md` write-path-agnostically, delegating over-cap files to the SAME drop-oldest logic as `notebook-auto-prune.sh` via synthetic PostToolUse JSON (no duplicated pruning code). (3) Added a blocking `wc -l` pre-commit gate to ops's notebook-commit step in `ops/flow/main.md`. (4) `.test-notebook-prune-debug/` already absent from the tree — no-op, verified via unrestricted `find`.
**what-considered:**
- Python migration script (one-off, scratchpad-only, not committed) doing exact `## `-boundary line-slicing vs manual per-section Read/Edit — chose script: 24 sections, mechanical extraction, verified boundaries against `grep -n "^## "` before running, spot-checked output + total-line conservation (1366 incident-file lines + 29 pointer lines vs 1197 original — consistent given added headings/pointers).
- New PostToolUse `Bash` matcher in `.claude/settings.local.json` (mirroring `orch-state-hook-bash-backstop.sh`'s precedent for the identical write-path-bypass class) vs a code-janitor cron sweep — chose the cron sweep: it is the explicit deliverable named in the task brief, avoids adding per-Bash-call latency fleet-wide (global hot-path cost, disproportionate to a P1/S task), and a periodic write-path-agnostic sweep closes the same gap. Left a KNOWN-GAP comment in `notebook-auto-prune.sh` cross-referencing the sweep for discoverability.
...
### STEP developer-S13 · developer · 2026-07-23T14:33:08Z
**task-id:** TE-T33
**what-done:** Built `scripts/agents-flow/cold-archive-sweep.sh` (monthly-guarded, `COLD_ARCHIVE_FORCE=1` override) with 2 legs — handoffs (`>30d` + not referenced anywhere in the 5 OPEN task_board lanes, jq recursive-descent string scan, computed live) → `archive/YYYY-MM/`; sessions non-`.md` leftovers `>30d` → `archive/YYYY-MM/`. Wired into `code-janitor/flow/main.md` § Cold Archive Sweep (6h cron). Fixed the 2 unscoped DJ-GATE-1 grep comments in `dev-team/main.md` (`sprint-*-*.md` → `sprint-${SPRINT_ID}-*.md`, matching the already-correct canonical pattern in `agent-chaining-protocol.md`/qa/pm). CANONICAL pointer + changelog in `dev-standards.md`.
**what-considered:**
- Decisions-leg + po-decisions.md rotation, as literally speced in the board note, vs verifying live overlap first — verify-first: found `decision-journal-archive.sh` (UC-MDH-P4, shipped same day) already SUPERSEDES the decisions/ mtime-leg (board row's own `audit_ref` note confirms) and `memory-prune-sweep.sh` (UC-MDH-P3) already sweeps `sessions/*.md` at 14d flat — implementing a duplicate/conflicting mtime sweep over the same files would be a correctness bug, not diligence. Scoped my sessions leg to the exact gap memory-prune-sweep.sh leaves (non-`.md` types) instead.
- po-decisions.md 200L rotation: invent a standalone prune vs reuse — reused `notebook-auto-prune.sh`'s drop-oldest-`##` algorithm verbatim via a new opt-in `NOTEBOOK_PRUNE_EXTRA_GOVERNED_PATH` env guard (default empty = zero behavior change on the hot PostToolUse path); rejected widening the hook's guard to a bare `docs/agent-memory/decisions/*.md` wildcard (blast radius on every future file in that dir) in favor of one explicit opt-in path set only by this sweep's caller.
- Handoff-reference predicate: exact `.handoff`/`.detail_ref` field match vs broad substring scan — chose the broad `jq .. | strings? | test("docs/handoffs/")` scan (deliberately conservative: false-positive "referenced" only causes a skip, never a wrong archive) after confirming both field names appear live and neither alone would catch all reference shapes.
**why-decision:** Live dry-check: 839 candidate `.md` files (`mtime +30`), 27 referenced-anywhere in open lanes, exactly 2 of those 27 are also `>30d` — both correctly held back by the referenced-set check (`comm -12` verified before writing any code). `cold-archive-sweep.test.sh` 14/14 PASS incl. idempotent re-run + explicit "live po-decisions.md untouched" (`git diff --quiet`) assertion; re-ran the 3 sibling suites (`notebook-linecap-sweep`, `memory-prune-sweep`, `decision-journal-archive`) after the shared-hook edit — 7/7, 12/12, 26/26 PASS, zero regression.
**why-change:** Decisions-leg dropped entirely (superseded, confirmed via board row `audit_ref` + `dev-standards.md` CANONICAL entry + the shipping script's own header comment naming TE-T33) — task's own escape hatch covers exactly this ("predicates prove ambiguous... do NOT force it"); this isn't ambiguity, it's a confirmed prior-shipped supersession, so I skipped the leg rather than duplicate/conflict with it, and left the rest of the CLEAN scope (handoffs, sessions-gap, po-decisions rotation, DJ-GATE-1 grep, CANONICAL pointer) mechanical as speced.
