...
### STEP dev-mcp-server-S14 · dev-mcp-server · 2026-08-23T13:35:00Z
**task-id:** TASK-BCTC-INSPECT-UI-FILTERS
**what-done:** Added `#quarter-filter`/`#ticker-filter` to bctc-inspector.html's `.controls` bar (FR-1), module-scope `allDocs` cache (D-2), extracted `renderDocOptions()` from `loadDocList()`, added `normalizeQuarter()` (client-side mirror, same semantics as the sibling task's server-side one — different file/layer, no collision), `populateFilterOptions()`, `applyFilters()` (D-3 pseudocode verbatim: AND-compose, `select.value` mutation with NO synthetic change event per FR-7). Live-verified against 257→268-row growing dataset: 11 quarter options, 50 ticker options (AC3/AC4 match exactly). New test file FEAT-BCTC-INSPECT-QUARTER-TICKER-FILTER.test.ts: 18 pure-fn assertions GREEN. All 5 named regression files + new file: 145 pass/0 fail. Full `bun test`: 15359 pass/40 skip/50 fail — every failing test named+grepped (backtest_runs, vps_push_log, insider_transactions, task_heartbeat Zod, foreign_flow, VPS-proxy-health, MCP SSE) is outside this task's files and unrelated to filter/quarter/ticker/bctc-inspector; pre-existing environmental red (many 5000ms timeouts = shared-DB/network contention from concurrent agents), not introduced by this change.
**what-considered:**
- dispatching a synthetic `change` event after `select.value = currentDocId` so the existing handler's side effects "just work" — REJECTED: AC6/AC8/FR-7 explicitly forbid it (would refetch PDF/OCR/table/MD on every filter tweak, defeating the zero-network-call design and risking flicker).
