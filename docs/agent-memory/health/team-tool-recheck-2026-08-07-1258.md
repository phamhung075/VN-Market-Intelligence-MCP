# Team Tool-Grant vs Declared-Boundary Recheck — 2026-08-07T12:58Z

**Run by:** agent-father `keep` flow, Step 5b (`docs/agents/agent-father/flow/team-tool-recheck.md`)
**Trigger:** manual (router-spawned `keep` cycle — same check, off the `23 14 * * *` cron cadence)
**Prior report compared:** `docs/agent-memory/health/team-tool-recheck-2026-08-06-1318.md` (prior day)
...
**Mechanical-enforcement status:** PROSE-ONLY, zero mechanical enforcement — `docs/data/system-map.json` has 0 `write_boundary` keys (`jq '[.. | objects | select(has("write_boundary"))] | length'` = 0); `.claude/settings.json` has a `PreToolUse` block but its sole matcher is `Glob|Grep` (graphify context injection) — no `Write|Edit` matcher, no `agent-write-boundary-guard` string anywhere in `.claude/settings.json` or `.claude/settings.local.json`. Unchanged since the prior run.
...
| Origin | `Bash` added by commit `610110e16` (2026-07-31, "fix(claude/agents): grant Bash to alert-commander, news-scout, market-watcher") — deliberate, justified grant (commit-mutex/task_claim session-id derivation), but description text never updated to reflect it. Unresolved for 7 days as of this run. |
...
The pre-2026-06-23 writer additionally ran live MCP probes: tool param-name drift vs live schema (e.g. `ticker` vs `code`), `get_cron_health`, `get_vps_proxy_health`/`get_vps_service_health`, `get_system_status`. `agent-father` holds no `mcp__gateway__call_tool` grant and cannot reproduce this subset. Handoff already filed: `docs/signals/2026-08-06-chore-team-tool-recheck-livescope-handoff.json` — po to assign a gateway-bound owner (still pending, not re-filed this cycle — no new information to add).

## RESOLVED THIS CYCLE

N/A — all 3 CRITICAL findings persist unchanged from the 2026-08-06T13:18Z run. No description-text fix or grant-narrowing landed for alert-commander / market-watcher / news-scout between the two runs.
