...
- Files modified: 1 (docs/agents/fb-market-poster/flow/main.md — 434L → 475L)
- Cascade: none — no name/routing/inter_agent/roster/dispatch/tool-package change; init.md version already 2026-05-29
- Validation: 7/7 passed (frontmatter line 1 ✓, flow has error boundary + RETURN ✓, tool package unchanged ✓, no new always_load paths ✓, hashtag block rule is in composition section + check 16 is in validation section ✓, 16-check count consistent across STEP 4 + STEP 8 notebook format ✓, evergreen set is 8 tags verified by listing them in both rule and check ✓)
- Decision: User request — discoverability on Facebook and YouTube Community posts. Hashtag block follows disclaimer as last element; English tags (#StockMarket etc.) are exempted from no-jargon-English rule. Diacritics rule: FB/YouTube hashtags do not handle Vietnamese accents.
