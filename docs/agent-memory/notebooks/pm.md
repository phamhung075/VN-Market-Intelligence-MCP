...
- **T1-ARCH-CRON-T4-DEDUP-GUARDS** (M, P1, no deps, READY) — Lever 2: add recency checks to 26 T4 jobs (ohlcvDailyAggregatorJob, vnstockFundamentalsRefresh, reputationComputeJob, baseRateComputation, predictionResolution, calibrationReport, weeklyPortfolioReport, devTeamHeartbeat, verdictResolutionJob, signalOutcomeJob, alertOutcomeJob, dataAuditDaily, dataAuditWeekly, etc.). Each uses `jobRunRepo.getLastRuns('jobName', 1)[0]` + 90% cadence window check. JSDoc @idempotency T4 tagged. Blocks T2. AC: all 26 jobs guarded, no double-Telegram on recovery replay, bun test green, tsc clean.
- **T2-ARCH-CRON-RECOVER-JITTER** (M, P1, depends=[T1], READY) — Lever 1 + Lever 3: (1b) uniform `recoverMissedExecutions: true` on all 50+ cron.schedule() calls in startScheduler.ts except foreignFlowFetch (every-minute by design). (1c) deterministic jitter shifts in cronConfig.ts: ohlcvDailyAggregatorJob 0→3min, vnstockFundamentalsRefresh 0→5min, reputationComputeJob 30→33min, baseRateComputation 0→7min, predictionResolution 30→35min, calibrationReport 0→4min, cascadeBacktest 30→37min, dailyDashboard 30→38min. Env-overrides intact. Blocks T3. AC: all 50+ sites have recovery flag, 8 jitter shifts applied, timing conflicts verified safe (per architect R-3 review), bun test green, tsc clean.
- **T3-ARCH-CRON-WATCHDOG** (M, P1, depends=[T2], READY) — Lever 4: new schedulerWatchdogJob.ts with WATCHDOG_MANIFEST (16 critical jobs: ohlcvDailyAggregatorJob=self-heal, vnstockFundamentalsRefresh=alert-only, etc.), per-job last_run age check vs cron_job_runs, 1.5× cadence threshold, alert-only or self-heal dispatch, 2h in-process rate limit. Registered at */10 * * * * with recoverMissedExecutions: true. Creates 3 test files: ARCH-CRON-watchdog.test.ts (5 watchdog unit tests), ARCH-CRON-idempotency.test.ts (T4 dedup under replay), ARCH-CRON-recovery.test.ts (loop-saturation simulation). AC: watchdog fires every 10 min, self-heal calls wrapRun, dedup guard prevents double-execution on self-heal tick, rate-limit 2h enforced, bun test all green, tsc clean.

IMPL GATE (CRITICAL): **FIX-MCP-CRASH-LOOP-WRITEWAL VERIFIED done_verified (commit 09e2586b)**. Crash-looping server drops ticks at process level; this fix landing + ops-verified is mandatory precondition. Developer FIX-MCP verified live post-rebuild + uptime >4h — gate sound. Next: dispatch T1 to dev-mcp-server (zone apps/mcp-server/). T2 and T3 READY but sequenced via depends[]: T2 cannot start until T1 ships, T3 cannot start until T2 ships. WIP constraint: dev-mcp-server zone serialized (one apps/mcp-server/ task in-flight at a time). SSOT atomic: orch-state.json updated with ARCH-CRON next_agent=dev-mcp-server, next_task=T1-ARCH-CRON-T4-DEDUP-GUARDS, 3 new ready[] tasks with depends[] ordering locked.
