# dev-rag-service — Notebook Archive (entries up to 2026-06-08)

Split out of `docs/agent-memory/notebooks/dev-rag-service.md` on 2026-08-05 by po triage:
the live notebook had breached its 12000B byte cap (13912B at 185L) with only one section
left to prune, and dev-rag-service was being dispatched for FIX-RAG-SERVICE-CLEAN-EXIT-RESTART-LOOP
(an APPEND-mode agent, so the next cycle would have pushed it past the 200L cap too).
The two most recent entries (2026-07-24, 2026-07-15) stay in the live notebook.
...
<!-- Entries 2026-08-05 and 2026-07-24 moved here 2026-08-12 (self-prune, main notebook byte cap 12000B breached after the FIX-RAG-EMBEDDER-IDLE-UNLOAD-ALLOCATOR-PAGES-NOT-RETURNED-TO-OS entry landed). Nothing deleted; full record preserved here and in git history. -->
