...
**Amendment 12:25Z:** router falsified my "never had a periodic writer" claim. Verified and accepted — writer is **intermittent, not absent**. `.live_path_evidence_20260725` rewritten in place: retraction marked, observation log added, mechanism recorded **UNRESOLVED** (3 candidates), OH-3.5 risk corrected permanent→**flapping**. Ruling itself untouched — still FOLD, 0 rows minted, no lane/status/owner/priority change.

## Lessons

- **⚠️ `git log` on a DIRTY file is not an observation log — I got falsified on exactly this.** I concluded "the heartbeat has never had a periodic writer, WU-3 dormant 23d" from `git log` of `auditor-tier1-last-healthy.json`. The file is uncommitted-dirty (` M`): `HEAD`=`07-25T07:38:02Z` while live was `11:37:59Z` → **history is structurally blind to precisely the writes that decide the question.** The writer is *intermittent*, not absent (fired 3 of the last 4 tick windows; I watched it advance to `12:20:51Z` mid-triage, ~5 min after my own read showed `11:37:59Z`). **Before reasoning from `git log` on any state file, run `git status --porcelain` on it first; if dirty, the live file is the only admissible evidence.**
...
- **I inherited a bad inference and then built a worse one on top.** Router's "Job 2 force-spawns forever" rested on conflating two scripts; I correctly split them, then over-read the timestamp pattern into "no writer ever". Every write lands +8..+21 min after a `*/30` boundary and *never* near it — which looks like a discriminator until you notice session dispatch lag is ~7 min (12:00Z tick → notebook header 12:07:37Z) and ad-hoc runs land in the same band. **The offsets are consistent with all three candidate mechanisms; recorded UNRESOLVED rather than resolved.**
