...
Triaged the 5 router-assigned NEW signal_queue rows across 2 orch-apply writes — both clean (conservation held task_total 560→561, signal 158; 12-key SSOT intact; no payload_ref dangle hit this cycle). Full trail → sprint-FLOW-PRICE-ALPHA-LOOP-po.md po-S151.
...
## Carry-over
- **BLOCKING — FIX-DRAIN-PAYLOADREF-DANGLE-ON-MOVE:** dangling payload_ref on drain has threatened orch writes 3x in ~1h (router). Unshipped code fix now treated BLOCKING; recurring_bug_count=3 stamped. HONEST: my own 2 writes did NOT block (live file clean, validator PASS at drain start) — escalation reflects the standing defect + router's cross-cycle assertion, not a block I observed.
