...
### Task 1338a: Write failing validation tests for Sprint 1338 docs
- **Files**: `apps/mcp-server/src/__tests__/1338-sprint-goal-retrospective.test.ts` (new, 4 tests), `TASKS.md` (1338a → In Progress)
- **Finding**: RED phase confirmed — 2 tests fail (SPRINT_GOAL.md first H2 = "Sprint 1327" < 1338; no retrospective block mentioning 1330/1337). 2 tests already pass because project-stats.json was pre-updated to currentSprint=1338 and sprintGoal mentions "1338". Task 1338b (GREEN phase) must update SPRINT_GOAL.md to fix the remaining 2 failures.
- **Status**: Ready for QA
...
### Fix 1290/1305: morningBriefing no stale fallback + review_market_message id coercion
- **Files**: `apps/mcp-server/src/scheduler/briefings/morningBriefingJob.ts` (catch block: logger.error → sendTelegramWork + early return, no stale market send), `apps/mcp-server/src/interface/mcp/tools/briefings/marketMessageTools.ts` (review_market_message schema: z.number() → z.coerce.number()), `apps/mcp-server/src/__tests__/FIX-1290-briefing-no-stale.test.ts` (5 new tests), `apps/mcp-server/src/__tests__/FIX-1305-review-id-coerce.test.ts` (6 new tests)
- **Finding**: 1290 — error path only called logger.error, never notified WORK channel; stale briefing was never actually sent by the catch block (the market send happens before catch), but the notice was missing. 1305 — MCP framework passes numeric tool args as strings; z.number() rejected "88" with "Expected number, received string". z.coerce.number() coerces transparently.
- **Status**: Ready for QA. Commit `7e2b4668`. Branch `fix/briefing-type-fixes`.

### Fix 1298/1299: BSR false cascade
- **Files**: `apps/mcp-server/src/domain/services/cascadeEngine.ts` (SectorRule interface: +`excludeKeywords?`, +`requireAnyKeyword?`; matching loop updated; geopolitical oil_gas rule: +requireAnyKeyword oil/energy terms, +excludeKeywords Fed/FOMC/monetary policy; coal/minerals rules: domain `oil_gas` → `utilities`, removed `"giá than"` from keywords to avoid first-match-wins conflict with Task 1315a rules; COMMODITY_TRIGGER_DOMAINS: added `"utilities"`), `apps/mcp-server/src/__tests__/FIX-1298-bsr-false-cascade.test.ts` (new — 7 tests)
- **Finding**: Two separate root causes. (1) FIX-1298: Geopolitical escalation oil_gas rule (line ~1550) had `"geopolitical"` as bare keyword — Fed/monetary policy articles mentioning "geopolitical uncertainty" fired it. Fixed with `requireAnyKeyword` (oil/energy terms required) + `excludeKeywords` (Fed/FOMC/central bank/rate cut blocks it). (2) FIX-1299: Coal/minerals → oil_gas SECTOR_RULE caused Masan khoáng sản articles to alert BSR via two paths: domain rule direct match AND market-wide broadcast fallback. Fix: changed domain to `utilities` (correct — coal is thermal power input, not oil refinery input). Added `utilities` to COMMODITY_TRIGGER_DOMAINS so market-wide broadcast is restricted on coal/minerals articles, skipping BSR (oil_gas domain not covered). Also removed `"giá than"` / `"giá than giảm"` from new utilities coal rule to avoid first-match-wins conflict with Task 1315a's more-specific directional rules.
- **Status**: Ready for QA. Commit `30650873`. Branch `fix/bsr-false-cascade`.

### Fix VPS-HEALTH-POLLER: data-freshness health checks (BUG 1 + BUG 2)
- **Files**: `apps/mcp-server/src/domain/services/vpsHealthPoller.ts` (full rewrite: HTTP poll → data-freshness; added FreshnessConfig, checkServiceFreshness, checkAllVpsServiceFreshness, DEFAULT_FRESHNESS_CONFIGS; kept FetchFn/DEFAULT_VPS_SERVICES/pollVpsServiceHealth as deprecated compat stubs), `apps/mcp-server/src/scheduler/system/vpsServiceHealthJob.ts` (removed breakers.polymarket + circuitBreakerRegistry import; now calls checkAllVpsServiceFreshness directly — fail-open), `apps/mcp-server/src/__tests__/FIX-VPS-HEALTH-FRESHN.test.ts` (16 new tests), `apps/mcp-server/src/__tests__/234-vps-health-sla.test.ts` (AC-10 updated: validates fail-open design instead of polymarket breaker presence)
- **Finding**: BUG 1 — DEFAULT_VPS_SERVICES pointed to localhost:5001-5005 (Docker microservice ports). VPS at $VINAHOST_IP is a push service with no HTTP health endpoint. BUG 2 — polymarket circuit breaker (5 fails → 10 min lockout) was reused for all 5 VPS health checks; one polymarket trip silently blocked all health monitoring. Fix: freshness = MAX(timestamp) per table vs. per-service threshold (price/foreign-flow: 5 min, news: 20 min, sbv: 35 min, bctc: passive). Health checks are fail-open — no breaker needed since all queries are local SQLite reads.
- **Status**: Ready for QA. Branch `fix/vps-health-poller`.
