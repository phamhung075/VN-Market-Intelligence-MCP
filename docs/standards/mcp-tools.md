...
- **News Analysis** — Cascade, sentiment, RAG search
- **Alerts** — Firing, accuracy, digests, mutes, health
- **Portfolio** — Positions, P&L, risk, rebalancing
- **Briefings** — Summaries, Telegram, market messages
- **Macro** — Indicators, policy, calibration, Kinh Dich
- **Sector** — Sector comparison, rotation, thematic
- **Backtesting** — Backtest lifecycle and comparison
- **System** — Coordination, VPS, memory, watchlist

**To find a tool:** `docs/agents/tools/list/INDEX.md`
...
| Alert Commander | `get_cycle_bootstrap`, `get_macro_snapshot`, `get_macro_calendar`, `get_market_context`, `get_alerts`, `get_legal_risk_signals`, `get_crisis_early_warning`, `get_market_snapshot`, `get_volatility_indicators`, `get_vn_liquidity_state`, `get_foreign_room`, `get_kinhdich_reading`, `get_watchlist`, `get_agent_signals`, `send_telegram`, `send_alert_digest`, `mark_alert_read`, `record_signal_outcome`, `write_alert_verdict`, `post_agent_signal`, `log_agent_work`, `submit_feedback` |
