...
| `bctcExtractReconcileJob` | `5,35 * * * *` | Resolves `pek_triggered` rows to `done`/`enrich_failed` by checking `bctc_layout_units`/`bctc_table_rows`/`bctc_md_tables` row counts once PEK has had runway; bounded re-fire via `MAX_RECONCILE_ATTEMPTS` (FIX-BCTC-D3C-RECONCILE-JOB). Emission circuit breaker (FIX-BCTC-RECONCILE-EMISSION-CIRCUIT-BREAKER): when a run exhausts `>= RECONCILE_DORMANCY_ROW_THRESHOLD` (default 3) rows OR the shared PEK producer (`bctc_layout_units`) hasn't landed a row in `RECONCILE_PRODUCER_STALE_DAYS` (default 2), all of that run's exhausted rows are folded into ONE run-summary Telegram BUG instead of one-per-row; below threshold with a healthy producer, per-row fail-loud emission is unchanged. Never gates the `enrich_failed` status write itself. |
