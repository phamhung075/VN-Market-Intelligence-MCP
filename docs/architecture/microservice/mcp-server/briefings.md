...
5. **Foreign-flow ("Khối ngoại") section never silently vanishes** (`eveningSummaryJob`/`franceSummaryJob`, FIX-FFLOW-BULLETIN-OUTAGE-SILENT-OMISSION): `formatForeignFlowSectionOrUnavailable()` (`src/scheduler/briefings/format/foreignFlowSection.ts`) renders the canonical `Khối ngoại: Dữ liệu không khả dụng (pipeline tạm dừng)` line whenever the resolved `foreignFlowMovers` array is empty (outage, DB error, or the default SQL's `foreign_net_vol <> 0` filter returning zero rows) instead of omitting the section. `formatForeignFlowSection()` — the raw formatter — is untouched (still returns `[]` for empty input; its own all-zero-but-nonempty branch stays reachable via an injected `getForeignFlowMoversFn` supplying unfiltered rows). `franceSummaryJob`'s `formatFranceSummaryVI` treats the `foreignFlowMovers` param as "omit" only when truly `undefined` (caller never attempted the query); an explicit empty array always renders (unavailable line or real data).
