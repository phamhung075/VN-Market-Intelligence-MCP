...
5. `search_similar_context` graceful-degrade (FIX-SEARCH-SIMILAR-CONTEXT-TIMEOUT-RECURRING, `analysis.ts:576-605`): the `ragSearch()` call is wrapped in its own try/catch — on rag-service timeout (bounded by `AbortSignal.timeout(8_000)` in `ragHttpClient.ts:148`), ECONNREFUSED, or non-200, it logs `logger.warn` and returns an empty raw-result set (→ `"No similar context found."`) instead of surfacing a hard `"Error searching context: …"` block. Mirrors `appendStockHexagramHttp`'s service-down omit-block pattern (`analysis.ts:84`). Root cause of rag-service instability tracked separately in FIX-RAG-SERVICE-CLEAN-EXIT-RESTART-LOOP — this is client-side fail-soft only.
