...
| `get_foreign_room` | Foreign room utilization/saturation suite; `tickers[]` trimmed to `top_n` (default 10, capped at actual ticker count) when `code` omitted, with `tickers_rollup` + `fetch_more.omitted_codes` (FIX-GET-FOREIGN-ROOM-TOOL-RESULT-TOKEN-BUDGET) | code?, top_n? | market.db (vnstock_trading_stats, foreign_room_events) |
...
12. `get_foreign_room` with no `code` filter reads `getAllTickersHistory` (`infrastructure/db/foreignRoomStore.ts`), which is NOT watchlist-scoped — it selects every distinct `code` present in `vnstock_trading_stats` (100+ live, the full traded universe the fundamentals job has touched, not the ~33-ticker watchlist). Serving that entire array inline exceeded the caller's tool-result token budget 3 consecutive alert-commander cycles (FIX-GET-FOREIGN-ROOM-TOOL-RESULT-TOKEN-BUDGET, 2026-07-29). Fix lives entirely in the interface layer (`interface/mcp/tools/market-data/foreignRoomTools.ts` calling the new pure `domain/services/market-data/foreignRoomAnalyzer.ts::summarizeForeignRoomTickers`) — the `application/usecases/getForeignRoom.ts` use case itself is UNCHANGED and still computes over the full universe internally (required: `computeMarketSaturation`/`computeForeignOutflowZ5d` are cap-weighted/z-score aggregates that need every ticker, and `getMoneyRadarComposite.ts` calls the use case function directly for `.market.foreign_outflow_z_5d` only, never the MCP tool). Only the MCP tool-result JSON trims `tickers[]` to `top_n` (default 10, effective limit always `min(top_n, actual count)` — never a hardcoded universe-size ceiling), ranking ROOM_LOCKED/FULL_ROOM_SELL-flagged tickers first, then `|depletion_velocity_5d|` descending. `tickers_rollup` (counts + `avg_utilization_pct`) is computed over the FULL universe regardless of trimming, and `fetch_more.omitted_codes` lists every dropped ticker for individual re-fetch via `code=<ticker>`. The pre-existing `/api/indicator-gauges` REST projection (`interface/mcp/routes/indicatorGaugesHandler.ts::buildForeignRoomSection`) was already unaffected — it has never forwarded `.tickers[]` at all (`.market` scalars only).
