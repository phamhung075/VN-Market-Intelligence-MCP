...
| DedupHit short-circuits | Duplicate fingerprint → fired=false, no route, no store |
...
## Test Counts (FACTORY-ALERT-consolidate-dual-engines, 2026-07-04)
application: 5 | infrastructure: 17 | interface/http: 8 | module/alert_pipeline: 10 |
primitive/cooldown-gate: 10 | primitive/dedup-key-builder: 7 | primitive/signal-classifier: 1 (7 subtests)
= **58 top-level tests (68 incl. subtests), 0 failing.** `go build ./...` clean, `golangci-lint run ./...` 0 issues, sandbox 11/11 scenarios green.
