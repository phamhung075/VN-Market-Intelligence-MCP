# TASK 1512a — RED: evening global snapshot test

sprint: 196
phase: RED (TDD)
file: src/__tests__/1512-evening-global-snapshot.test.ts (NEW)

## What to write

5 failing assertions. ALL must fail before 1512b starts.

| AC | What | Fails because |
|----|------|---------------|
| AC-1 | `EveningSummary` interface has `globalSnapshot?: GlobalSnapshot` | field not declared yet |
| AC-2 | `assembleEveningSummary` populates `globalSnapshot` from `commodity_prices` row | step not implemented |
| AC-3 | `assembleEveningSummary` sets `globalSnapshot=undefined` when table empty | step not implemented |
| AC-4 | `formatGlobalSnapshotSection` renders 4 metric lines | already passes — import from `morningBriefingJob.js` to verify reuse |
| AC-5 | `eveningSummaryJob` (formatter path) includes "Thị trường toàn cầu" when `globalSnapshot` present | formatter block not added yet |

## Minimal DDL for in-memory DB
...
describe("1512 evening-global-snapshot", () => {
  let db: Database;
  beforeEach(() => { db = new Database(":memory:"); setupDb(db); });
...
- AC-4 may already pass (formatter exists from 1511). Keep it — documents reuse contract.
- AC-5 requires `formatEveningSummaryLines` exported from `eveningSummaryJob.ts`. The inline `lines` array in the job's Telegram block (lines 252-327) must be extracted into this fn. It does NOT exist yet — this is the RED trigger.
- Do NOT import real DB (`getDb()`). Inject `{ db }` only.
- Branch: `task/1512-evening-global-snapshot`
...
- /Users/admin/Documents/Hung/__works__/__PROJET/__labo/VN-Market-Intelligence-MCP/src/__tests__/1512-evening-global-snapshot.test.ts   # created: 5 RED assertions

tests_written:
- src/__tests__/1512-evening-global-snapshot.test.ts   # 5 assertions: 4 FAIL (RED), 1 PASS (AC-4 reuse)

tests_skipped: []

tsc_clean: n/a (RED phase)
full_suite_pass: n/a (RED phase)

RED confirmation:
- AC-1: FAIL — `globalSnapshot` key absent in assembleEveningSummary return
- AC-2: FAIL — no globalSnapshot populated from commodity_prices
- AC-3: FAIL — throws SQLiteError (schema mismatch) before returning
- AC-4: PASS — formatGlobalSnapshotSection already exported from morningBriefingJob (1511)
- AC-5: FAIL — formatEveningSummaryLines not exported from eveningSummaryJob
