# TASK 1505a — RED: failing test assertions for cascade-backtest
...
Create `src/__tests__/1505-cascade-backtest.test.ts` with ≥ 7 failing assertions.
ALL tests MUST FAIL before 1505b starts.

## File to create

`src/__tests__/1505-cascade-backtest.test.ts`
...
`initDatabase(db)` provides all required tables including `cascade_rule_hits` (with `price_impact_3d`, `price_impact_7d`, `outcome_correct`, `hit_at`, `affected_stocks`) and `daily_ohlcv` (with `code`, `date`, `close`).

Insert `daily_ohlcv` rows directly via `db.prepare(...).run(...)` — no store helper exists for this table.

`cascade_rule_hits` insert via `recordHit(db, ruleKey, matchedText, sector?, stocks?)` from `cascadeHitStore.js`. To set `hit_at` in the past, UPDATE the row after insert:
```typescript
db.prepare("UPDATE cascade_rule_hits SET hit_at = ? WHERE id = ?").run(pastDate, id);
```
...
describe("1505 cascade-backtest", () => {
  ...
  // AC-1 ... AC-7 tests below
});
```

## 7 Required failing assertions
...
After writing, run:
```bash
cd /Users/admin/Documents/Hung/__works__/__PROJET/__labo/VN-Market-Intelligence-MCP && bun test src/__tests__/1505-cascade-backtest.test.ts 2>&1 | tail -20
```

All 7 tests MUST FAIL (module not found or assertion errors). If any pass before 1505b, something is wrong.
