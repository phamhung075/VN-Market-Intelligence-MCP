...
Direct-Commit Verify (adapted lane — row arrived `review[]`/`REVIEW`, not `qa[]`/`QA`), 2026-08-23T14:08:53Z. Independently re-ran: 18/18 new unit tests, 109/109 five named regression files, `bun tsc --noEmit` clean, DDD/security grep clean, `mock-guard.sh` N/A (`.html` outside its `.ts/.tsx/.py/.go` scan scope — hand-verified the diff instead, zero fabricated data). Commits `2e66153fd`/`cab92d7c5` confirmed ancestor of local `main`.

Live dual-origin verification with a real headless Chromium browser (Playwright), both `:3000` and `:3001`, identical results: quarter facet = 11 options (not 13 — HUT's 2 string-`"Q1"` rows correctly normalized), ticker facet = 50; AC5 AND-compose confirmed (HUT + Q1-2024 → exactly 1 doc, zero network calls); AC7 zero-match confirmed (disabled+selected placeholder, correct Vietnamese status text, zero console errors, screenshots pixel-identical both origins); AC9 label "HUT Q1 2024" not duplicated (paired commit `237fa6e26`); FR-7 selection-preservation confirmed — `select.value` mutation on a still-matching filter fired **zero** network requests (no synthetic `change` dispatch), total `/docs` fetch count = 1 across the whole multi-filter interaction (AC2). Blast-radius: `/health` toolCount=183 matches; `/api/bctc-inspect` 200 both origins; `/dashboards/news-fetch/` 200 on `:3000` (no such route exists on `:3001` at all — structural, pre-existing, not a regression). BCTC-Eval/OOM gates N/A.

VERDICT: `DONE_VERIFIED`, zero blocking issues. Row moved `review[]` → `done_verified[]` in the same `orch-apply.sh` write, `verification.raw_probe` attached. Task Report: `reports/TASK_REPORT_TASK-BCTC-INSPECT-UI-FILTERS.md`. Decision journal: `docs/agent-memory/decisions/sprint-COWORK-GUARANTEED-SLOT-CATCHUP-qa-26.md` STEP qa-S161.
