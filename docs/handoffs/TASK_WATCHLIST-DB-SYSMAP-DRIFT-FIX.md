...
## [QA] Review Record (2026-07-11T12:10:00Z) — round 1

**VERDICT: CHANGES_REQUESTED.**

RAW-verified independently of the dev/ops claims, not trusted from status_note:
- `pnpm check` (tsc): clean. Targeted tests: `WATCHLIST-DB-SYSMAP-DRIFT-FIX.test.ts` 16/16 pass, `scripts/migrations/__tests__/resync-watchlist-sysmap-2026-07-11.test.ts` 10/10 pass, the 5 updated seed-dependent tests (1343a/1343e/1787/1876a-A6/VNH-sector-fix) 36 pass/1 skip/0 fail. `gh run view 29151558506`: headSha=edd1ec31e (exact HEAD), conclusion=success, `bun test` job green among 8/8. DDD scan (domain→infra/application imports on the 2 modified production files): clean. Security scan (`process.env`, secrets): clean. `mock-guard.sh`: PASS.
- Direct fresh-connection DB probe against the SERVING named volume (`docker exec ... bun -e 'bun:sqlite'`): 33 rows, ticker set diffs to **zero** against `jq '.project.watchlist[]|select(.active==true)|.ticker'` from system-map.json. VEA confirmed `active:false` in SSOT (correctly absent from live). VNH/GVR confirmed zero entries in system-map.json (correctly absent). No `REFERENCES watchlist` FK anywhere in `apps/mcp-server/src/infrastructure/db/` — AC-6 clean. mcp-server container image `sha256:1c5845d64406...` confirmed live via `docker inspect`, health `toolCount=183`.

**Real, reproduced, blocking defect found — AC-5 unmet on live serving** (`- [ ] All TA endpoints return corrected universe (verify /ta/money-flow-oscillators + /ta/roc-momentum)` — this AC was never exercised by the [Developer] record; no evidence either endpoint was curled). `apps/technical-analysis` is a **separate Go container**, shares the same `market_data` named volume (`DB_PATH=/app/data/market.db`, `DB_READONLY=true`, no `WATCHLIST_TICKERS` env override) but resolves its **in-memory watchlist slice exactly once at process startup** — `apps/technical-analysis/cmd/server/main.go:54-64` (`readWatchlistFromDB`), injected into `NewComputeROCMomentumUseCase`/`NewComputeMoneyFlowUseCase`/etc at `main.go:77-96`. `docker ps` shows `vn-market-intelligence-mcp-technical-analysis-1` **Up 3 days** — never restarted, neither by the DB resync (13:41Z) nor the mcp-server swap (13:48Z, a different container). Live-curled just now:
```
curl -sX POST :5003/ta/roc-momentum -d '{}'         → 41 tickers
curl -sX POST :5003/ta/money-flow-oscillators -d '{}' → 41 tickers
```
Both return the exact OLD stale set verbatim — including the 5 garbage orphans commit `91ef0ac74`'s own message calls out as pre-dating even the old hardcoded seed's "removed" comment (`VDC, BDI, DLC, JSH, SIS` — e.g. `BDI` = Baltic Dry Index, not a VN stock), plus `GAS, GVR, MBB, ACB, CTG, VPB, HSG, NKG, HVN, ACV, HCM, DHG, POW, PPC, TCH, VNH, REE, MWG, VEA`. Zero overlap with the corrected 33-ticker SSOT set (`diff` against `ssot-active.txt` = 33 lines of pure divergence both directions). This is exactly the task's own stated purpose ("Restores correctness for /ta/money-flow-oscillators, /ta/roc-momentum, and sibling TA endpoints" — TLDR line 11) and it is **not delivered** on the live system right now. mcp-server's `get_watchlist` (33, correct) is a *different* serving surface from these two TA endpoints — the two-layer freshness gap here is a second, independent instance of the "same-DB tools diverge" class, on a downstream service nobody checked.
