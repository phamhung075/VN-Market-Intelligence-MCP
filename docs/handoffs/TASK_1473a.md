# TASK_1473a — RED: test(diacritics): TDD RED — failing assertions for batch-3 diacritics

sprint: 178
phase: RED
test_file: src/__tests__/1473-tool-diacritics-batch3.test.ts

## Goal

Create failing test file. All assertions must FAIL before GREEN phase (strings currently missing diacritics).

## File to create

`src/__tests__/1473-tool-diacritics-batch3.test.ts`
...
`bun test src/__tests__/1473-tool-diacritics-batch3.test.ts` → ALL tests FAIL (strings absent). Commit as `test(1473): RED — batch-3 diacritics assertions`.
