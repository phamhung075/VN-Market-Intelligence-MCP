# TASK_1465a — RED: ohlcv-staleness-check TDD

task: 1465_a
phase: RED
sprint: 175
test_file: src/__tests__/1465-ohlcv-staleness-check.test.ts

---

## Goal

Write failing test file. Import `runOhlcvStalenessCheck` from `../scheduler/ohlcvStalenessCheckJob.js` — file does not exist yet. All 5 tests must compile but FAIL (import error or assertion failure).
...
## Function Stub (for compile only — no logic)

Create `src/scheduler/ohlcvStalenessCheckJob.ts` with stub:

```typescript
// src/scheduler/ohlcvStalenessCheckJob.ts
// STUB — RED phase only. Replace with full impl in TASK_1465b.
...
- `bun test src/__tests__/1465-ohlcv-staleness-check.test.ts` → 5 tests FAIL (NOT_IMPLEMENTED error)
...
- /Users/admin/Documents/Hung/__works__/__PROJET/__labo/VN-Market-Intelligence-MCP/src/__tests__/1465-ohlcv-staleness-check.test.ts   # created: 5 RED test cases
- /Users/admin/Documents/Hung/__works__/__PROJET/__labo/VN-Market-Intelligence-MCP/src/scheduler/ohlcvStalenessCheckJob.ts   # created: stub throwing NOT_IMPLEMENTED

tests_written:
- src/__tests__/1465-ohlcv-staleness-check.test.ts   # 5 assertions, all RED (NOT_IMPLEMENTED)

tests_skipped: []

tsc_clean: true
full_suite_pass: n/a (RED phase — 5 intentional failures)
