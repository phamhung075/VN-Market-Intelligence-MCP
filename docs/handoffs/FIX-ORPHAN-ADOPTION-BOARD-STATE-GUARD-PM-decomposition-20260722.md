...
#### [Developer] Implementation Record — 2026-08-25 (dev-team Incident-Lane Consumer dispatch, session 036ceaf1)

- **Board-row state at claim time:** this row was `po_expedited_at` P0, dispatched via the ILC's own independent `INCIDENT_CAP` budget (not the shared WIP<=2 slot). Its board copy carries a richer `subtasks[]` array + 4 dated po annotations (`po_triage_20260806T2205_dispatch`, `po_triage_20260806T2230_rootcause_correction`, `po_corroboration_20260808`, `po_triage_20260825T0107Z_subtask4_lineref_drift`, `po_expedite_20260825T0727Z`) than this handoff doc's original Task 4 spec above — implemented against the LIVE board row's subtasks, not the (now partially stale) prose here, per the row's own line-drift corrections.
- **Path deviation (evidence-based, not the literal brief text):** `scripts/lib/resolve-task-lane-by-id.jq`, not `scripts/agents-flow/resolve-task-lane-by-id.jq` as the 2026-07-22 architecture brief §2 literally names. Confirmed via `find` before writing: `scripts/agents-flow/` holds zero `.jq` files repo-wide (only `.sh`/`.js`/`.mjs` standalone hook/probe scripts); `scripts/lib/` is where this repo's shared `include`-based multi-consumer jq predicate libraries already live (`devteam-eligibility.jq`, `po-manual-dispatch-eligibility.jq`) — this file is the same shape. Full rationale in the script's own header and the new `docs/policies/dev-standards.md` CANONICAL block.
- **Files created:** `scripts/lib/resolve-task-lane-by-id.jq` (`lane_map`/`bare_id`/`resolve_lane` — batch-resolves every `.task_board` id across all 7 flat lanes + nested `active_sprints[].tasks[]` in one pass, `{id, lane, status, supervised}` per row, additive `supervised` field beyond the brief's literal `{id,lane,status}` shape to feed the subtask-2(ii) widening below); `scripts/lib/resolve-task-lane-by-id.test.sh` (12/12 GREEN — lane coverage, prefix-strip idempotency, not-found-never-active, supervised surfacing, synthetic cold-archive query shape).
- **Files modified:** `docs/agents/dev-team/flow/orphan-adoption.md` (subtasks 2/3/4 — the actual per-signal adoption loop; this row's own `po_triage_20260825T0107Z_subtask4_lineref_drift` annotation already established that the loop body relocated here from `main.md` on 2026-08-05 (TE-T02), and the same drift applies identically to subtasks 2/3, whose titles still cite stale `main.md` line numbers): 112L→188L, new size-justification header (declared count = actual, verified via `size-lint-justification.sh --check`, 0 new offenders). `docs/agents/dev-team/flow/main.md` (Step 0a-B one-line sub-flow description updated to name the guard — the guard's own logic lives in the delegated sub-flow per the same relocation, this file only points to it). `docs/policies/dev-standards.md` (new CANONICAL block + header entry, Script Persistence rule).
- **Subtask 2 (FR-4 read-guard) — implemented + WIDENED per `po_corroboration_20260808`:** new guard block in `orphan-adoption.md`, positioned BEFORE both the `redispatch_count >= N_MAX` branch (EC-3, matches the row's literal acceptance line) AND the adopt claim (stronger than that literal line — never locks a lock on an already-finished task at all; produces the identical externally-observable skip/log/release behavior the po's "after task_claim succeeds" phrasing describes, without the wasted claim+release round-trip — documented as a deliberate, evidence-based deviation in-file, same class as Task 3's FR-1 deviation above). Classification reuses `resolve-task-lane-by-id.jq`'s `lane_map`, identical active/terminal rule to `dispatch-claim/SKILL.md` § Orphan-Adoption Probe (cited in-comment as SSOT — the lookup MECHANICS are genuinely shared via the one script; the classification branches themselves still exist as inline pseudocode in both files, since a markdown flow-doc has no runtime `include` across files — flagged, not something either file's own instruction text can structurally close). **(i) Cold-archive fallback (widening):** a hot-lane miss (`lane_map[$id] // null`) now queries `docs/data/orch/archive/{2026-06,2026-07,2026-08}.json` `.done_tasks[]`/`.closed_sprints[]?.tasks[]` before defaulting to terminal — live-verified against the real archive: `TASK-DEV-MCP-SIGNAL-TYPE-REGISTRY` (the exact row from this ticket's own `docs/handoffs/2026-08-25-orphan-adoption-terminal-guard-live-reproduction.md`) resolves to `{lane:"archive", status:"DONE_VERIFIED"}`. **(ii) Supervised-skip (widening):** a hot row otherwise classified `active` is re-classified `terminal` when `.supervised == true`, per the annotation's WF-2/Supervised-Lane-Sweep rationale. **ZONE GUARD (out-of-subtask, in-scope per this row's own dispatch instruction):** added an explicit `task_zone` empty/unresolved refusal immediately before the pre-existing `git status --porcelain -- {task_zone}` tree-hygiene revert — an empty pathspec there widens the revert to repo-wide, the exact live-recorded hazard (`feedback_orphan_adoption_treehygiene_unscoped_revert`) this ticket's dispatch explicitly named; the fix refuses the revert (logs + continues adoption without tree-hygiene) rather than falling back to an unscoped `git status --porcelain` call.
- **Subtask 3 (FR-5 board-flip) — implemented:** the `main.md:383-388`-cited (now `orphan-adoption.md`) board-flip jq is replaced with a lane-aware write reusing the SAME `hit`/`bare_id` the guard above already resolved (architect brief §2 — no second `ltrimstr`, no second lookup). `hit.lane == "active_sprints"` keeps the original nested-path jq; any other lane (guaranteed to be one of the 8 real board lanes, never `"archive"` — that class always `continue`s in the guard above) now targets `.task_board[$lane][] | select(.id == $tid)`. Both EC-1 (prefix strip — the old jq used raw `{original_task_id}`, never stripped) and EC-2 (flat-lane blindness — old jq only ever matched `active_sprints`) are closed. Verified live-mechanics-only (not against the real board) against two scratch fixtures: a flat `ready[]` row and a nested `active_sprints[].tasks[]` row both flip `assigned_to`/`adopted_at`/`tree_hygiene_note` correctly.
- **Subtask 4 (FR-2 Rung-B release params) — implemented, literal `po_triage_20260825T0107Z_subtask4_lineref_drift` line refs applied to the file as it exists today:** all 3 `task_release` call sites in `orphan-adoption.md` now pass `owner_agent: "dev-team"` + `original_owner_client_session: dead_session` (the `signal.payload.original_owner_client_session` value already in scope as `dead_session` throughout the loop) alongside the pre-existing `owner_client_session`: the new FR-4 guard's own terminal-skip release, the git_sha-invalid branch's orphan-signal release, and the final post-adoption cleanup release. **Deliberate over-inclusion, flagged not silently narrowed:** the po's annotation counts "3" release calls but one of the three (`task_release(task_id: original_task_id, ...)`, releasing the just-claimed SPRINT-TASK lock in the git_sha-invalid branch) is not itself `"orphan-signal:*"`-prefixed and already matches via Rung A (its `owner_client_session` is this session's own, non-null) — added the same params there too for literal-acceptance compliance; confirmed inert/safe per NFR-1 (the null-session ladder gates on the ROW's own `owner_client_session IS NULL`, so extra caller params on a live-session row change nothing).
- **Tests:** `bash scripts/lib/resolve-task-lane-by-id.test.sh` — 12/12 GREEN (new). No `apps/` TS/Go touched — pure jq/flow-doc/markdown, `bun test`/`tsc` N/A. Ran `bash scripts/agents-flow/dev-team-tick-preflight.test.sh` (154/154, unrelated sibling suite) as a smoke check that nothing else in this dispatch path regressed — unchanged, as expected (this task touches neither file that suite covers).
- **Verification (not asserted from prose):** every jq snippet embedded in the flow-doc pseudocode was executed for real against the live `docs/data/orch/orch-state.json` (read-only) and against throwaway scratch fixtures (writes) before being written into the doc — see the 4 `jq`/scratch-fixture runs in this session's own tool-call history (hot lookup on the real board resolves this ticket's own row correctly: `{"lane":"in_progress","status":"IN_PROGRESS","supervised":false}`; cold-archive lookup on the real archive resolves the real incident row correctly). `bash scripts/audits/size-lint-justification.sh --check` and `bash scripts/audits/task-claim-owner-session-lint.sh --check` both re-run post-edit: 0 new offenders in either (pre-existing unrelated `bctcScalarAggregator.ts` baseline-tolerance offender only).
- **Not done here (out of this row's scope, flagged not fixed):** `.claude/skills/dispatch-claim/SKILL.md` § Orphan-Adoption Probe's own FR-3 guard still carries its original Task-3-authored inline lane-lookup, not yet retrofitted to consume `scripts/lib/resolve-task-lane-by-id.jq` — the architecture brief's "3 call sites, 1 script" vision is 2/3 realized (this row's FR-4 guard + FR-5 flip); migrating SKILL.md's own copy is a separate row, not implied by this row's subtasks. `.claude/skills/dispatch-claim/SKILL.md`'s own escalation-heartbeat call site is still missing `original_owner_client_session` (a residual Task 3 flagged in its own Implementation Record, 2026-08-07) — unrelated file, out of scope here.
- **Board-write landmine hit and worked around, not fixed (this row is NOT the owner):** the `in_progress[] -> review[]` lane-move for THIS row hit `scripts/orch-row-prose-ceiling-check.mjs`'s documented "D3" false-positive (`docs/architecture-briefs/2026-08-23-orch-row-prose-ceiling-value-shape-measure-and-frozen-cohort-paydown.md` — `PROSE_CEILING_LANES` excludes `in_progress[]`, so ANY row leaving it reads `live=0B` and its full serialized size counts as "net new growth"; this row's accumulated `po_triage_*`/`po_corroboration_*` annotations put it at 14266B, over the 12000B ceiling). Same class already reproduced live on `UC-CCA-P3`/`UC-CDC-P1`/`FIX-SYSTEM-MAP-WATCHLIST-STALE-34-OF-58` per that brief §0 — this row is simply the newest instance, not a defect this ticket introduced. Worked around WITHOUT touching the shared `orch-backlog-stub.sh`/its stale `STUB_FIELDS` list (that script's default field set is itself flagged HIGH-risk in the same brief for silently dropping `next_agent`/`owner`/`supervised`/`plan_only` with no safe landing for consumers that read them inline-only): manually stubbed THIS ONE row using the ceiling-check's own `STRUCTURAL_FIELDS` set (id/title/status/owner/next_agent/supervised/dispatch_lane/claimed_at/claimed_by/etc. all stay hot — zero dispatch-relevant data loss, confirmed live via `effective_next_agent()` resolving `"qa"` correctly and a real dry-run of `scripts/devteam-review-claim-qa-drain.jq` claiming the row into `qa[]`), full row (incl. all `po_*` prose) preserved at `docs/data/orch/archive/backlog-detail.json#FIX-ORPHAN-FR4-FR5-FLOW-DEVTEAM-ADOPTION-GUARD` (cold-first-hot-second write order, matching `orch-backlog-stub.sh`'s own safety ordering). The underlying D3 bug and the STUB_FIELDS-alignment prerequisite remain unfixed and are NOT this row's to fix — separate, already-architected initiative.
...
- **Pre-work finding:** `coordinationStore.test.ts` already existed — Task 1's own commit (`d6c4e6006`) TDD-drove 13 tests (H1-H5/L1-L3/N1/R1-R2/N2/combined) as part of implementing heartbeatTask/releaseTask, and its own file header names THIS task by id ("Comprehensive edge-case + regression + live-integration-round-trip coverage is the separate, dependent task FIX-ORPHAN-FR8-TEST-COORDINATION-STORE (Task 5) — not duplicated here"). Mapped every one of subtasks 1-4's acceptance lines against the 13 existing tests before writing anything new — all satisfied except two residual gaps (below); subtasks 1-4 were therefore de facto already GREEN, not a rewrite.
- **Files modified:** `apps/mcp-server/src/infrastructure/__tests__/coordinationStore.test.ts` — added H6/H7 (ttl_seconds clamp-floor(60)/ceiling(691200) boundary — clamped inside `coordinationStore.ts`'s own `heartbeatTask()` via `Math.min(Math.max(...))`, a real coordinationStore.ts behavior distinct from Task 2's separate Zod-layer bound, previously untested at the boundary) and R3 (releaseTask null-session ladder wrong-`owner_agent` failure branch — heartbeat side already covered this via L2; release side only had the wrong-echo branch, R2 — subtask 3's own wording, "fails on wrong params" plural, calls for both failure modes on both functions). Updated the file's own header comment to document the closed gaps and record subtask 5's disposition (below) inline, so a future reader doesn't have to cross-reference this handoff to learn why no live-round-trip test exists in the file.
- **Tests written:** 3 new (H6, H7, R3) — 16/16 total pass, 33 `expect()` calls (was 13/27).
- **Git commits:** `8af71e8c2` — test(mcp-server): FR-8 close residual gaps in coordinationStore.test.ts
- **tsc status:** clean ✓
- **Targeted suite:** same 7-file coordination/lock scope Task 1/2 used (`task-lock-coordination-store.test.ts` + `task-lock-reaper-timer.test.ts` + `task-lock-coordination-tools.test.ts` + `FIX-ORPHAN-FR2-FR6-FR7-INTERFACE-COORDINATION-TOOLS.test.ts` + `commit-mutex-coordination.test.ts` + `DWF-coordination-phase2.test.ts` + `coordinationStore.test.ts`) 143/143 pass, 0 regressions. Repo-wide `bun test` not re-run per pinned CANONICAL reading (`dev-standards.md:1435`) — targeted suite governs push-gate.
- **Subtask 5 — NOT attempted this cycle (documented as blocked, not fabricated):** the live integration round-trip needs Task 2's (`FIX-ORPHAN-FR2-FR6-FR7-INTERFACE-COORDINATION-TOOLS`) Zod-schema exposure reachable through an actually-running container. Verified, not assumed: (1) Task 2's board status is `REVIEW`, not `DONE_VERIFIED` — QA has not signed off; (2) `docker inspect vn-market-intelligence-mcp-mcp-server-1` reports `StartedAt: 2026-08-08T19:06:16Z` (UTC), roughly 5 hours BEFORE Task 2's commit `fb5207746` (`2026-08-09T02:09:43+02:00` = `2026-08-09T00:09:43Z` UTC) — the running image predates and therefore does not contain Task 2's code, so `ttl_seconds`/`payload_patch`/`owner_agent`/`original_owner_client_session` are not live on the MCP tool surface today regardless of QA status; (3) this session's granted tool set is `Read/Edit/Write/Bash` only (no `mcp__gateway__call_tool`, confirmed by the absence of any MCP tool function in this session's tool schema) — structurally cannot place a `task_heartbeat`/`task_release` call even against an up-to-date container. Per this repo's fabrication-lesson set (fb-poster/BCTC "no fake data", "PARTIAL_EXIT invented"), the honest action is to leave this subtask un-executed and say so explicitly, not to simulate or assert a pass. **FR-3/FR-4/FR-6 prose MUST NOT be described as "available" until this round-trip is actually run (post Task 2 DONE_VERIFIED + container rebuild, by a session holding gateway tool access) and passes.**
- **`depends` array data-quality note (flagged, not silently fixed):** this row's own machine-readable `depends: ["FIX-ORPHAN-FR1-FR2-INFRA-HEARTBEAT-LADDER"]` omits Task 2, even though this same PM decomposition doc's § Ordering Guidance states "Task 5 depends on Task 1 + Task 2 (tests cover FR-1/FR-2 impl)." The omission is why dev-team's RLC could legitimately claim this row today (Task 1 alone is DONE_VERIFIED) despite Task 2 still being in `REVIEW` — a genuine board/prose mismatch, surfaced here for PM/PO rather than silently patched by this developer session (board-schema edits are outside this task's assigned file scope).
- **Docs updated:** `docs/WORK.md` (one-liner) | this Implementation Record | `coordinationStore.test.ts`'s own header comment (in-file documentation of the gap-closure + subtask-5 disposition, see above).
- **Graphify:** NOT run — same structural tool-grant gap as Task 1/2/3 above (`Read/Edit/Write/Bash` only, no Skill-invocation tool). Doc changes this cycle are factual "what was done" entries (WORK.md one-liner + this record + a test-file header note), not new agent-facing API/schema prose.
- **Simplicity gate:** PASS — Q1 scope clean (every added line maps to subtask 1's ttl_seconds behavior or subtask 3's release-side failure-branch symmetry, no unrelated edits); Q2 N/A (test file, no abstractions added); Q3 N/A (test file, no indirection); Q4 N/A per skill's own carve-out ("do not apply Q2/Q4 to test files").
- **Zone-dispatch note (same deliberate, evidence-based deviation as Task 1/2/3 above):** `apps/mcp-server/` nominally routes to `dev-mcp-server` per `system-map.json`'s Tier-1 explicit-zone rule, but this session (Task-tool subagent, `Read/Edit/Write/Bash` only, no `Agent` tool) structurally cannot nest-spawn a zone specialist. The board row's own `next_agent`/`dispatch_lane` already read `developer` at claim time. Implemented directly rather than stalling at an unreachable dispatch wall.
...
#### [Developer] Implementation Record — 2026-08-11 (dev-team RLC dispatch, session 165f4245)

- **Finding: NO EDIT NEEDED — confirmed, not assumed.** `docs/data/tool-registry.json`'s `task_heartbeat`/`task_release` entries are bare name strings inside the `system` group's `tools[]` array (41 tools). The file's own top-level shape is `{_maintained_by, lastUpdated, totalCount, groups[{name, count, tools[]}]}` — there is no per-tool object anywhere in the file, so a "per-tool schema payload" (params, `.describe()` prose, Zod bounds) cannot structurally be duplicated here in the first place.
- **Root-cause verification (read the generator, not just the output):** `scripts/gen-tool-registry.ts` (file header: `_maintained_by: "generator (do not hand-edit)"`) extracts tool names via regex `server\.tool\(\s*["']([^"']+)["']` / `server\.registerTool\(...)` — captures ONLY the string-literal tool name, never the Zod schema block that follows. FR-1/FR-2's new `ttl_seconds`/`payload_patch`/`owner_agent`/`original_owner_client_session` params (Task 1 `coordinationStore.ts` + Task 2 Zod schemas) live entirely inside the 3rd-argument object passed to `server.tool(...)`, which the generator's regex never reaches — this file is therefore structurally incapable of drifting on a param-schema change, only on a tool being added/removed/renamed.
- **Freshness cross-check (live, not trusted-from-prose):** since Task 2 landed, `task_heartbeat`/`task_release` were further split out of `coordinationTools.ts` into standalone `apps/mcp-server/src/interface/mcp/tools/system/coordination/taskHeartbeatTool.ts`/`taskReleaseTool.ts` (a size-lint refactor, `FIX-CI-SIZELINT-COORDINATIONTOOLS-TS-457L`, unrelated to this task). Confirmed this move did not silently drift the registry: `bun scripts/gen-tool-registry.ts --dry-run` → `totalCount=183`, `groups=12`, `system: 41 tools` — byte-identical to the committed file's own `totalCount:183` and `system` group count (nested subdir files are correctly attributed to their top-level `system/` category by `getCategoryForFile()`, per its own first-path-segment rule).
- **CI drift guard already exists and is green:** `bun test src/__tests__/tool-registry-parity.test.ts` → 17/17 pass, 39 `expect()` calls — this suite asserts registry/source parity (including a documented fake-tool-injection negative-control recipe in its own header) and would fail loudly on exactly the class of drift this subtask worries about (per-tool duplication or staleness). Nothing to add — the regression gate this subtask might otherwise have recommended already ships.
- **Docs updated:** this Implementation Record | `docs/WORK.md` (one-liner). `docs/data/tool-registry.json` itself NOT touched (verified current, generator-owned, "do not hand-edit").
- **Tests:** N/A new tests — pre-existing `tool-registry-parity.test.ts` (17/17) is the applicable regression gate and already covers this exact drift class; no `apps/` source changed this task.
- **Git commits:** none to `tool-registry.json`/source (verification-only task, no edit needed) — Implementation Record + WORK.md commit only.
- **Simplicity gate:** N/A — no code change (verification-only subtask, explicitly permits "confirm no edit needed" as the terminal outcome per its own acceptance line).
- **Zone-dispatch note:** `docs/` carries no entry in `system-map.json`'s zone list → falls to `developer`'s own documented fallback ("no matching zone → developer handles it directly") — no dispatch deviation needed this task (unlike sibling Tasks 1/2/3/5 which needed the `apps/mcp-server/`/`flow-docs/` structural-gap deviation note).
