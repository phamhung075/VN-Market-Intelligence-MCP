...
## [PO] SIG-IMPL-GATE Spec-Gate Verdict — APPROVED (2 hardening notes) — 2026-05-27T21:16:12Z

I red-teamed the blueprint against the live repo before deciding — this is a deliberation gate, not a rubber-stamp (user standing mandate: "all automate, need thinking before approve anything"). I verified the GREENFIELD claim by hand (all 4 target files ABSENT; `improve_check_log`=0 in schema-system.ts; `SELF_IMPROVE_AUTO_DISPATCH`/`DEGRADATION_CAUSE_MAP`=zero matches in `apps/mcp-server/src`), confirmed the reused infra actually exists (`getAccuracyStats` + `SignalAccuracyStats` in signalOutcomeStore.ts, `jobRunRepo.wrapRun()` in startScheduler.ts, `accuracyDigestJob.ts` `_running`+`AccuracyDigestDeps` reference pattern), confirmed `infrastructure/signals/` + `docs/improvement-proposals/` are absent (R-3/R-4 honest), and read the live `cronConfig.ts` 09:00-slot block. Concrete judgements per the 7 mandated axes below.

### Finding 1 — C-1 honored (structural target_agent derivation, never free-prose parse)? PASS

The blueprint replaces the SIG-PO-GATE EDIT-4 break-risk (agent-father parsing the free-text "Proposed Change" paragraph) with a **typed `FIX_AREA_TO_AGENT` constant** (blueprint §0-ii, lines 297-323). The orchestrator reads `fix_area` straight from `DEGRADATION_CAUSE_MAP` and looks up `target_agent` in that map — it never reads prose. The map is total: every `fix_area` key resolves to one of `{'dev-mcp-server','dev-technical-analysis','UNRESOLVED'}`, and `_default`→`fix_area:'manual'`→`UNRESOLVED`, target_files `[]` (AC-T4-7). Critically, an **unmapped** `fix_area` falls to `{target_agent:'UNRESOLVED', target_files:[]}` — fail-SAFE (human triage), so a mis-mapped area can NOT silently route a proposal to the wrong agent; it routes to UNRESOLVED, which agent-father's C-1 fail-loud reject (wired in SIG-IMPL-MD 062a6569) catches downstream. AC-T4-1..3 assert non-empty kebab-case target_agent + JSON-parseable target_files for a known fix_area; AC-T4-7 asserts the UNRESOLVED fallback. C-1 is satisfied at both the emit (this phase) and consume (Phase-1 already shipped) ends.

### Finding 2 — C-2 honored (lifecycle stays agent-father's; this phase does NOT flip status to DONE)? PASS

The proposal doc is written `status: 'DRAFT'` only (§1e `ImprovementProposalFields.status: 'DRAFT'`, DASHBOARD row `status=NEW`). REQ §0 scope explicitly excludes the status flip to DONE ("that is agent-father's job per C-2"), excludes the WIP-cap check, and excludes the recheck loop. The orchestrator emits and stops; the IMPLEMENTING→DONE lifecycle lives in agent-father's `improvement_approved_md` route (already shipped 062a6569). This phase cannot reach `DONE`. C-2 satisfied.

### Finding 3 — C-4 honored (per-path default-false ENFORCED in code, global-flag a test-asserted REJECT)? PASS — strongest axis

This is structurally — not by convention — enforced. `DISPATCH_PATHS as const` + `type DispatchPath = typeof DISPATCH_PATHS[number]` makes `isAutoDispatchEnabled(path: DispatchPath)` reject freeform strings at COMPILE time (AC-T5-5). The env key is `SELF_IMPROVE_AUTO_DISPATCH_{PATH_UPPER}` — there is no key pattern a bare global `SELF_IMPROVE_AUTO_DISPATCH=true` can match, so the global flag cannot enable any path. AC-T5-4 is the test-asserted REJECT: set `SELF_IMPROVE_AUTO_DISPATCH=true` (no suffix) → assert every path still returns false. Default-false is from `Bun.env[key] === 'true'` (anything-but-'true' → false), NOT from env-absence alone (AC-T5-1 asserts all-false with NO env set). I asked: is there any code path where an operator re-introduces a global enable? No — adding a path requires editing the `DISPATCH_PATHS` array (a code change, code-reviewed), and the call site type-guards `DISPATCH_PATHS.includes(...)` before the lookup so unknown signal_types get false without exception. The single-flag-blesses-all regression is structurally prevented, not merely discouraged. C-4 satisfied; QA owns the live GATE-PROOF that flips any one path.

### Finding 4 — C-5 honored (D-IMPROVE doc-write failure cannot abort improve_check_log insert or WORK Telegram)? PASS

R-6 (blueprint §5) specifies the doc-write step runs AFTER the `improve_check_log` insert and the WORK Telegram, each wrapped independently; the doc-write is in a try/catch that logs-and-continues, never rethrows (§1e docstring: "Throws on file-write failure (caller wraps in try/catch, non-fatal per C-5)"). AC-T4-6 is the proof: inject a `writeProposalFn` that throws → assert `cron_job_runs.status='success'` AND the WORK Telegram still fired. The add-on is subordinate to the primary pipeline exactly as C-5 demands. Satisfied.

### Finding 5 — GATE-PROOF is real (TASK-6 injects into degradationRules.ts subject code, RED-then-GREEN with evidence, false-green→lane-A demotion)? PASS

TASK-6 (REQ lines 365-393) names the gate = the `detectDegradedSignalTypes()` unit suite, and the injection target is **the subject code, not the test/gate config**: AC-T6-2 changes the DEGRADED threshold constant in `degradationRules.ts` so a 15pp delta no longer trips, then asserts AC-T2-1 goes RED; AC-T6-3 restores → GREEN; AC-T6-4 records "Gate proven red on [date] by [method]. Evidence: [output]" into the first real proposal doc. AC-T6-5 is the false-green firewall: if the gate does NOT go red, QA records `GATE-PROOF-FAILED` and the path is demoted to LANE-A (PO manual approval, no automated backstop) — "an assertion that a test suite passes without demonstrating red is a REJECT (feedback_fence_false_green)." This matches brief §5 GATE-PROOF-1..5 verbatim and honors `feedback_silent_swallow_serial_bugs` (per-path proof, not one-proof-blesses-all — Finding 3). The proof is QA's acceptance criterion, not an assertion. Satisfied.

### Finding 6 — Lane-C-in-disguise check (does the blueprint EDIT existing gate/audit/classification logic vs only ADD)? PASS — no lane-C leak

I grep-confirmed the GREENFIELD: `DEGRADATION_CAUSE_MAP` does not exist in the codebase, so "modify an existing entry / threshold / detection policy" is **structurally impossible this phase** — you cannot mutate a constant that does not yet exist. Every file is NEW or a pure-ADD (schema `CREATE TABLE IF NOT EXISTS` + new DDL to `initSystemTables()`; `cronConfig.ts` adds one key; `startScheduler.ts` adds one import+register; `docker-compose.yml` adds commented-out default-false env vars). R-2 explicitly forbids merging `FIX_AREA_TO_AGENT` into `DEGRADATION_CAUSE_MAP`; hard-constraint table line 749 confirms "DEGRADATION_CAUSE_MAP entries are ADDED only; no existing entry modified." Nothing edits the gate/audit/classification logic, nothing is irreversible (git-tracked, no prod-data deletion, shadow-mode), nothing touches a user-facing-comprehensibility surface (the lane-C-forever anchor is untouched). No STOP, no reclassification, no user escalation required. Lane assignment at emit is LANE-B for DEGRADED/PERSISTENTLY_LOW, LANE-A for COVERAGE_GAP — correctly classified, and agents-architect's IP-review (shipped 062a6569) re-validates it. PASS.

### Finding 7 — Shadow-mode + host budget (ship genuinely inert, no new Docker/agent, one cron slot, no regression-floor breach)? PASS

Ship is inert: all `DISPATCH_PATHS` default-false (AC-T5-1), nothing auto-dispatches (REQ §0 + kickoff). §6 hard-constraint table confirms: zero new Docker service (all 6 new files inside the mcp-server process), zero new cowork agent (it is a scheduler job), ONE new cron slot. New tests = 36 (TASK-1..5) + TASK-6 QA evidence, no existing test modified — does not threaten the 9408-PASS floor / 348-FAIL ceiling. Single-writer DB honored (runs inside mcp-server, Option C). Within the already-budgeted Sprint-1948 footprint (~0 MB incremental RAM, ~0.5 KB/day to improve_check_log, ~2-5 KB/proposal). No kernel-panic / 8GB-cap aggravation (`project_host_memory_panic`). PASS.

### Two hardening notes (NOT blockers — pm/dev resolve; neither touches a lane-C axis, a hard constraint, or a product call)

**HN-1 (cron rationale FACTUAL ERROR — decision is nonetheless correct).** The blueprint's cron-slot rationale (§0 Cron-Slot Collision, lines 329-334) states `bctcOverdueCheck = '0 9 * * 1-5'` (weekdays). I read live `cronConfig.ts` line 26: `bctcOverdueCheck = '0 9 * * *'` — **DAILY, not weekday-only**. (The REQ §TASK-3 line 203 carries the same wrong premise. The `0 9 * * 1-5` slot is actually `marketOpen`, line 11.) So the real `0 9 * * *` collision is with a DAILY job, not a weekday one, AND there is also a weekday `marketOpen` collision. The blueprint's DECISION — offset to `2 9 * * *` — is therefore not just zero-cost-nicety but the **correct** call: it cleanly avoids BOTH the daily `bctcOverdueCheck` and the weekday `marketOpen` same-minute pileup at 09:00 UTC. The decision stands; the supporting sentence is factually wrong and pm/dev should not propagate "bctcOverdueCheck is weekdays" anywhere. The hard constraint (ONE new slot, `2 9 * * *`) holds — verified against the live 09:xx block (`marketOpen 0 9 * * 1-5`, `bctcOverdueCheck 0 9 * * *`, `bctcReparseJob 30 9 * * *`, `marketEarningYield 30 9 * * 1-5`; `2 9` is free). No second slot.

**HN-2 (anti-runaway severity ordering — silent spec↔blueprint divergence).** REQ TASK-3 step 8 (line 226) orders the top-2 anti-runaway selection `DEGRADED > COVERAGE_GAP > PERSISTENTLY_LOW`. The blueprint (§1b line 412, R-7 line 732) orders it `DEGRADED > PERSISTENTLY_LOW > COVERAGE_GAP`. These conflict and the blueprint did not flag the change. The blueprint's ordering is the more defensible one (a measured accuracy regression is more actionable than a missing-signal gap), but the divergence must be reconciled explicitly so the AC-T3-8 test fixture asserts ONE agreed order, not a guess. pm: make the architect's `DEGRADED > PERSISTENTLY_LOW > COVERAGE_GAP` the canonical order and have ba/dev correct REQ TASK-3 step 8 to match (or, if ba defends the REQ order, route back one hop) — do NOT let dev pick silently. This is a 1-line ordering decision, fully reversible, machine-testable; not lane-C.

### Summary judgement

All 7 mandated axes PASS, several verified structurally against live code (C-4 compile-time type enforcement; greenfield lane-C impossibility; reused-infra existence). The blueprint faithfully translates the BA spec, honors all five SIG-PO-GATE conditions C-1..C-5, and breaks no hard constraint. The two findings are implementation-hardening (one factual-error-in-prose with a sound decision; one spec↔blueprint ordering reconciliation) — neither is a lane-C leak, an irreversible/comprehensibility surface, or a missing condition. Therefore: **APPROVED**, with HN-1/HN-2 recorded for the pm/dev chain to honor. No return to architect; no user escalation.

---

## [PO] VERDICT — APPROVED — 2026-05-27T21:16:12Z

The SIG-IMPL-GATE blueprint is sound on all 7 critique axes and is APPROVED to enter the dev chain. C-1 (typed `FIX_AREA_TO_AGENT`, no prose parse, UNRESOLVED fallback total), C-2 (DRAFT-only, no DONE flip), C-3 (lane-C-in-disguise critique field already shipped in Phase-1 flows), C-4 (per-path default-false enforced at compile + AC-T5-4 global-flag REJECT), C-5 (doc-write isolated, AC-T4-6) are all honored. GATE-PROOF is QA-owned, subject-code-injected, RED-then-GREEN with evidence + lane-A demotion on false-green (AC-T6-5). Greenfield = zero lane-C leak. Shadow-mode inert at ship, one cron slot, within Sprint-1948 host budget, no regression-floor breach.

### Routing — SIG-IMPL-GATE → dev chain

- **NEXT: pm** — decompose REQ_SIG-IMPL-GATE.md (TASK-1..6) into `TASK_NNN` dev tasks in docs/TASKS.md and sequence the chain. Honor WIP=2.
- **Sequence (serial by dependency, single zone `apps/mcp-server/`):** dev-mcp-server TASK-1 (schema+store) → TASK-2 (domain detect) → TASK-3 (orchestrator+cron+wiring) → TASK-4 (D-IMPROVE bridge) → TASK-5 (per-path kill-switch) → **ops REBUILD** (force-recreate mcp-server, not restart — `feedback_rebuild_after_dev_change`) → **qa TASK-6** (GATE-PROOF live against the running orchestrator's first proposal doc).
- **pm MUST carry into the dev tasks:** HN-1 (cron is `2 9 * * *`; the "bctcOverdueCheck is weekdays" premise is FALSE — live is daily; do not propagate it) and HN-2 (anti-runaway order = architect's `DEGRADED > PERSISTENTLY_LOW > COVERAGE_GAP`; reconcile REQ TASK-3 step 8 to match before AC-T3-8 is written). Make C-4 per-path-default-false an explicit AC; make C-1 structured target_agent/target_files an explicit AC.
- **Commit safety:** all on `main`, explicit-path `git add` only, files UNSTAGED for main-terminal serialization, no `pilot-status-*.json` touch.
