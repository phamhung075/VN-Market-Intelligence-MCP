...
## NF-LD-4-dev-A FIX — QA round 1 remediation (dev-mcp-server) — 2026-05-24

**Root cause diagnosed:** The two sed substitutions in `sync-news-fetch-dashboard.sh` were ordered shortest-match-first. The generic pattern `check that mcp-server is running at localhost:3000.` consumed the longer `Could not reach the server — check that mcp-server is running at localhost:3000.` substring before the second sed ran, producing a third variant the committed copy didn't match.

**Fixes applied (2 files, `apps/mcp-server/` zone only):**

1. `apps/mcp-server/sync-news-fetch-dashboard.sh` — reversed sed substitution order: more-specific (longer) pattern runs first so the shorter generic pattern never consumes it. Added comment explaining the ordering requirement.
2. `apps/mcp-server/src/interface/news-fetch-dashboard/index.html` — regenerated by running the fixed script. Changes vs committed:
   - Header `To re-sync` path: `scripts/sync-news-fetch-dashboard.sh` → `sync-news-fetch-dashboard.sh` (stale `scripts/` prefix removed)
   - Hand-added 7-line `IMPORTANT: This is the SERVED COPY…` JS comment block removed (it was not in source; script does not inject it; the HTML-level GENERATED header already carries all that information)
   - Error message: third variant replaced by script-consistent `Could not reach the server — the live endpoint returned an error.`

**Idempotency proof:** two consecutive `bash apps/mcp-server/sync-news-fetch-dashboard.sh` runs on the same working tree produce byte-identical index.html (md5 `ace65115a5e4345a31b217eb5dfc5c23`). After second run `git diff apps/mcp-server/src/interface/news-fetch-dashboard/` = 0.

**Verification results:**
| Check | Result |
|---|---|
| Idempotency: 2nd script run → git diff clean | PASS |
| ENDPOINT relative (`/api/news-fetch/live…`) | PASS |
| file:// degrade branch kept (`window.location.protocol === 'file:'`) | PASS |
| Creds grep served dir: 0 matches | PASS |
| NF-LD-4 tests: 11/11 | PASS |
| NF-LD-2 tests: 9/9 (regression) | PASS |
| tsc --noEmit: exit 0 | PASS |
| Header path in generated copy: no `scripts/` | PASS |

**Commit:** `6b012fc8`

**NEXT: back to qa for NF-LD-4-QA round 2.**
...
## NF-LD-5-dev-A — dev-mcp-server (regenerate served copy) — DONE

### [dev-mcp-server] Implementation Record — NF-LD-5-dev-A — 2026-05-24

**Files shipped (apps/mcp-server/ zone only):**
- `apps/mcp-server/sync-news-fetch-dashboard.sh` (MODIFIED) — updated to handle NF-LD-5 variable rename (`var ENDPOINT` → `var BASE_ENDPOINT`): added `BASE_ENDPOINT` sed guard; updated verification greps to accept `var (BASE_)?ENDPOINT`; updated header comment. Reason: dev-B (commit `12600a1f`) refactored `initLivePanel()` IIFE into named `loadLiveData()` callable and renamed the URL constant from `var ENDPOINT` to `var BASE_ENDPOINT` (already relative — no rewrite needed, guard kept for safety).
- `apps/mcp-server/src/interface/news-fetch-dashboard/index.html` — REGENERATED by running the fixed script. Now contains the Refresh button (`id="live-refresh-btn"`), source selector (`id="live-source-select"`), `loadLiveData()` callable, `var BASE_ENDPOINT = '/api/news-fetch/live?limit=20'` (relative). Zero localhost:3000 in fetch path. file:// degrade branch kept.
- `apps/mcp-server/src/__tests__/NF-LD-4-news-fetch-dashboard.test.ts` — tests (h) and (i) updated to match `var (BASE_)?ENDPOINT` (regex) so assertions pass with either old or new variable name. All 20 tests (NF-LD-4 11 + NF-LD-2 9) green.

**Root cause of script fix needed:** NF-LD-5-dev-B changed the ENDPOINT variable name from `var ENDPOINT` to `var BASE_ENDPOINT`. The sync script's verification check looked for `var ENDPOINT = '/api/news-fetch/live` — which no longer exists — causing exit 1. Fix: broaden the grep pattern to `var (BASE_)?ENDPOINT`.

**Idempotency proof:**
- Run 1 md5: `b1d8806f7e8ae8b7de26a78962b6550b`
- Run 2 md5: `b1d8806f7e8ae8b7de26a78962b6550b` (identical)
- Run 3 md5: `b1d8806f7e8ae8b7de26a78962b6550b` (identical)
- Git diff after run 2 (relative to pre-NF-LD-5 committed copy): 329 lines — stable, does not grow between runs. After commit, git diff will be 0.

**AC evidence:**
- AC-1 PASS: `bash apps/mcp-server/sync-news-fetch-dashboard.sh` → exit 0, all 4 PASS lines; served copy is script output, not hand-edited
- AC-2 PASS: after commit, `git diff -- apps/mcp-server/src/interface/news-fetch-dashboard/` = 0 (committed == generated)
- AC-3 PASS: md5 `b1d8806f7e8ae8b7de26a78962b6550b` identical across 3 consecutive runs; 2nd-run git diff line count = 329 (same as run 1, stable)
- AC-4 PASS: `grep -n "live-refresh-btn"` → line 239 (`id="live-refresh-btn"`); `grep -n "BASE_ENDPOINT"` → line 370 `var BASE_ENDPOINT = '/api/news-fetch/live?limit=20'` (relative, no localhost:3000); `grep -n "window.location.protocol === 'file:'"` → line 377 (degrade kept)
- AC-5 PASS: `grep -rn "VPS_PUSH_API_KEY|x-api-key|Authorization|Bearer|DB_PATH|process.env|Bun.env" apps/mcp-server/src/interface/news-fetch-dashboard/` → 0 matches
- AC-6 PASS: `diff apps/news-fetch/dashboard/data.js apps/mcp-server/src/interface/news-fetch-dashboard/data.js` → exits 0 (byte-identical); `git diff HEAD -- apps/mcp-server/Dockerfile` → empty (no Dockerfile change needed)
- AC-7 PASS: `bun test NF-LD-4 NF-LD-2` → **20 pass / 0 fail** (59 expect() calls); `bun tsc --noEmit` → exit 0
- AC-8 PASS: `docs/data/pilot-status-news-fetch.json` not touched
