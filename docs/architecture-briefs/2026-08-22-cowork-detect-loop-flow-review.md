...
## D) Resilience under extended host downtime (vacation scenario — 2026-08-22 addendum)

**New operational fact:** the entire fleet (MCP server, gateway, every cron-driven agent) runs on the user's personal PC — no cloud/server hosting. The user is going on ~1 week vacation; the machine will sleep and/or fully shut down for that span. Two prior confirmed sleep-outages (2026-08-15T22:48→08-18, ~2.5d; 2026-08-18T10:53→08-22T12:38, ~4d) self-healed cleanly — but a full week exceeds both, and may involve an actual shutdown rather than sleep, which is a structurally different, untested failure mode per `CronCreate`'s own documented contract ("Jobs live only in this Claude session — nothing is written to disk, and the job is gone when Claude exits"). This section re-examines the same two loops against that specific scenario. Same verification standard as §C: CONFIRMED = independently checked this cycle: SUSPECTED = plausible, not independently checked.

**F9 — CONFIRMED: exactly one component in either loop survives a full shutdown+reboot automatically; everything else is session-only `CronCreate` and needs manual re-arm.**
Checked `~/Library/LaunchAgents/` directly (not just the repo's `launchd/` source folder) and cross-referenced against `launchctl list`. Only `com.vn-market.cowork-guaranteed-slot-firer` is both (a) a real OS-level LaunchAgent (symlinked into `~/Library/LaunchAgents/`, `StartInterval=900s`) and (b) actually relevant to these two loops — it fires only the 8 `guaranteed:true` cowork-schedule slots by invoking `claude -p 'slot=<id>'` directly (per F4, already known to bypass `cowork-team/flow/main.md` and never stamp `last_fired`). LaunchAgents in that directory are auto-loaded by launchd at every user login, with no manual `launchctl load` needed after a reboot — so this one path resumes itself once the user's Mac is powered on and logged in, no terminal/session required. Everything else in both loops has zero boot-survival path and is 100% dependent on a human opening a terminal and the 3 re-arm skills running again:
- cowork-team's own `*/15` master dispatcher (the only path that drains `.signal_queue.rows[]`, runs pressure/cadence logic, and covers the other 15 non-guaranteed cowork slots — alert-commander, market-watcher-offhours/eod, news-scout, bctc-analyst, refine_bctc_md) — `CronCreate` only.
- All 4 `cron-detect-loop` jobs (dev-team hourly, system-auditor Tier-1/2/3) — `CronCreate` only.
- `cron-standalone-team`'s 6 jobs (agent-father, claude-manager-helper, code-janitor, the 2 `AUDIT_TIER=DATA` CADRAT-2 jobs, the journal-mode verify job) — technically outside this brief's 2-loop scope, but the same "`CronCreate`-only, needs manual re-arm" fact applies, worth naming since the user's downtime fact is fleet-wide.

**F10 — CONFIRMED, new finding (not previously surfaced in §C): the designed guaranteed-slot catch-up mechanism exists in code but is wired into zero live consumers — directly relevant to what happens on resume.**
`scripts/agents-flow/cowork-catchup-predicate.js` is a real, built module (per `docs/architecture-briefs/2026-07-22-cowork-guaranteed-slot-catchup-design.md`) and it IS `require`'d by `scripts/agents-flow/cowork-match-slots.js`, which correctly computes and returns a `catchup_raw[]` array (bounded per-dish-type freshness windows, 60–1440 minutes depending on dish type, `docs/data/cowork-schedule.json._dish_type_catchup_config`). But grepped all 3 documented consumer sites and found zero references to `catchup`/`catchup_raw` in any of them: `docs/agents/cowork-team/flow/main.md` (the design's own Step 4.55 sub-flow, `catchup-check.md`, was never created — file does not exist), `scripts/agents-flow/cowork-tick-preflight.sh` (Step 6.5 never added), and `scripts/agents-flow/cowork-guaranteed-slot-firer.sh` (still reads only `.slots[]`, filtered to `guaranteed==true` — the exact launchd plane the design's own §2.3 called "the plane most likely to actually deliver the catch-up during a session-down window," never wired). The plain slot-match window (`cowork-match-slots.js`, confirmed by comment) is **±2 minutes** around the cron's exact scheduled minute. Consequence: any guaranteed dish/post whose exact scheduled minute falls while the machine is off or asleep (every one of them, across a full week) is silently and permanently skipped — no late-fire, no structured miss record (that logic was part of the same never-wired design), no dedicated alert. More consequentially for the *return day itself*: since re-arming is a manual, un-scheduled human action ("walk over and open a terminal"), if that moment doesn't happen to land within ±2 minutes of one of that day's own guaranteed cron minutes (chef morning 05:15 UTC / EOD 08:45 / evening 19:45, digest-daily 17:30, tnb-audit 20:13, fb-daily 09:15, etc.) — quite likely, since nothing about vacation-return is itself scheduled — that day's guaranteed dish(es) can *also* be silently missed, with no automatic recovery. This is a materially different, more actionable finding than anything in §C and is being routed to agent-father/developer as a follow-up (see signal below), not fixed here.

**F11 — CONFIRMED: no duplicate-publish or stale-data-as-fresh risk on resume — by a combination of two *different*, both-working mechanisms, neither of which is the (unwired) catch-up predicate from F10.**
Two genuinely distinct recovery paths exist, and it matters which one is in play:
- **Sleep, not shutdown (the only case tested so far, 2x confirmed live):** the CLI process itself never exits, so `CronCreate`'s own internal scheduler queues every missed fire and replays them back-to-back on wake — through the *full, real* dispatcher logic each time (signal_queue drain, pressure/cadence, published-marker gate). The bulk of replayed ticks correctly no-op (`SILENT`/`TOMBSTONED` verdicts, nothing newly due); any tick that *is* due for a guaranteed slot goes through the same live-verified published-marker gate documented in §B (Phase-1 probe + Phase-2 claim, period-keyed) — so even a large stacked replay cannot double-publish. This matches the 2 already-observed clean self-heals.
- **Shutdown (untested, the more likely case for a full week):** registrations are simply gone. Nothing "replays" at all — there is no backlog of queued ticks to flush, because there is no process that was holding them. Once a human re-arms manually, the fleet just resumes from whatever "now" is; per F10, anything from the outage window (including possibly the resume day) is quietly skipped, never republished. So: no duplicate-post risk either way, and no stale-data-treated-as-fresh risk from a burst-replay (there is no burst in the shutdown case, and the sleep case's replay is protected by the marker gate) — but the *reason* is "conservative to the point of silently losing coverage," not "correctly catches up," which is the opposite of reassuring for anyone expecting the missed dishes to eventually appear.

**F12 — CONFIRMED, stated plainly per the task's own instruction not to soften it: there is no automated "long gap since last activity → re-arm" detection anywhere in this codebase. Re-arming is 100% dependent on a human or the router remembering to do it.**
Checked `.claude/settings.json` and `.claude/settings.local.json` — the only hooks registered are `PreToolUse` (Glob/Grep graphify hint; Write/Edit orch-state pre-write guard; Bash tmux status), `PostToolUse` (Write/Edit notebook-prune + context-bloat backstop), `Stop` (branch-hygiene), and `SubagentStart` (tmux status) — none reference cron registration, none reference the 3 re-arm skills, and there is no `SessionStart` hook at all. The 3 re-arm skills (`/cron-cowork-team`, `/cron-detect-loop`, `/cron-standalone-team`) are documented in the project's root `CLAUDE.md` as "re-arm ... after every session restart" — this is a **prose instruction** the router is expected to read and comply with, not a code-enforced hook. Nothing measures elapsed time since the last successful tick and automatically invokes these skills; nothing pages the user; there is no fallback if the first post-vacation session is compacted, interrupted, or opened for an unrelated quick question before the router's own CLAUDE.md preamble runs. If that happens, the entire fleet — including `system-auditor`, the one component that would normally *notice* and alert on prolonged silence — stays completely dark with no self-alerting mechanism, indefinitely.

**Pre-vacation / post-vacation checklist:**

| When | Action | Why |
|---|---|---|
| Pre-vacation | No lock/marker cleanup needed | Confirmed self-healing: per-tick locks (`cron:cowork:<TICK>` etc., TTL 600s) and published-marker gates (TTL 24h–8d, period-keyed) naturally expire or become moot within the outage window; the one longer-lived class (`cron-registration:*` markers, TTL 8d) is reclaimed independent of its own TTL via the dead-peer check (`session-presence` absent → `task_force_release_orphan` after only 2h) — see §C F3's mechanism, same logic applies here. |
| Pre-vacation | Prefer sleep over full shutdown if there's a real choice | Per F11, sleep has a *proven* (2x live), safe self-heal path through the real dispatcher logic; shutdown does not — it requires manual re-arm and, per F10, has no working catch-up for anything missed. |
| Post-vacation, first session | Run (or have the router run, per its own CLAUDE.md preamble) all 3 re-arm skills: `/cron-cowork-team`, `/cron-detect-loop`, `/cron-standalone-team` | Mandatory after any full shutdown; also needed after a sleep-outage if the specific terminal/CLI process itself was also closed at any point (sleep alone doesn't require it only if the exact same process survived). |
| Post-vacation | Do not expect a catch-up burst of the missed week's guaranteed dishes/posts | Per F10/F11, none will be republished — this is the current (if unintentional) behavior, not a bug to chase down. |
| Post-vacation, first dev-team tick | Expect a larger-than-usual `signal_queue`/idle-chain drain (days-to-a-week of accumulated rows, on top of the pre-existing backlog already found in F2) | Bounded and self-recovering (`signal_queue` has no TTL-based loss, only a 200-row soft cap the fleet is nowhere near) — a one-time bigger drain, not a failure. |
| Post-vacation | Manually sanity-check that *same-day* guaranteed dishes actually fired (chef morning/EOD/evening, tnb-audit, fb-daily or fb-weekend, digest-daily or digest-sunday) | Per F10, a same-day miss is possible if the re-arm session doesn't happen to start within ±2 min of that day's own cron minutes — there is currently no automatic alert for this. |

**D bottom line.** The two loops' *documented* mechanics (§B/§C) don't change under this scenario — but the vacation framing surfaces two new, concrete gaps that weren't visible from a same-day live-state read alone: (1) the guaranteed-slot catch-up mechanism is fully designed and half-built but wired into nothing (F10), meaning missed guaranteed dishes are quietly lost rather than caught up, including possibly on the return day itself; and (2) re-arming the whole fleet after any full shutdown has zero automated trigger — it depends entirely on a human/router remembering, with no self-alerting fallback (F12). Neither is fixed here, per scope; both are routed to agent-father below as concrete follow-up candidates.
