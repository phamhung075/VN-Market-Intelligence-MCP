...
- All 33 sandbox scenarios are GREEN (25 primitive + 5 module + 3 service). The real `cmd/sandbox` CLI runs ONE scenario per invocation — there is **no** `-module` flag and **no** `-scenario=all`. The actual signature is `go run ./cmd/sandbox -tier=<primitive|module|service> -scenario=<name-or-path>` (plus `go run ./cmd/sandbox -audit` for the env-credential gate). Each invocation exits 0 on GREEN, 1 on RED. To prove all 33 GREEN, the canonical AI/CI gate runs from the service root `apps/technical-analysis`:
  - `node dashboard/verify-render.mjs` — strict build gate: asserts exactly **33 dot-green** (25 primitive + 5 module + 3 service), 0 dot-red, 0 dot-pending, 0 JS errors; writes `dashboard/dist/render-check.png` as evidence. Any non-green = non-zero exit.
  - `node dashboard/dash-check.mjs` — AI inspector: loads the rendered dashboard headless via `file://`, emits one `DASH-CHECK-RESULT: {...}` JSON line to stdout with a `verdict` of `PASS | WARN | FAIL` (exit 0 on PASS/WARN, non-zero on FAIL). Convention defined in `08-sandbox-dashboards.md §2e`.
  These two scripts are the authoritative, browser-free way to prove all 33 scenarios GREEN. Env-credential audit: `go run ./cmd/sandbox -audit` exits 2 if any forbidden `DB_`/`API_KEY`/`SECRET`/`TOKEN`/`PASSWORD` key is present in the sandbox environment.
