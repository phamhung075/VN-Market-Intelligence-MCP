...
- Never-fired: `last_fire == null ? "Chưa từng chạy" : <VN-locale format>` (AC-20, **Layer-A only** — `layer === "server"` rows whose true `last_fire` is genuinely absent). **Layer-B (`layer === "cli-session"`) branch is separate and MUST be checked first**: these rows always render a dedicated not-tracked label (never "Chưa từng chạy") because `last_fire` is force-nulled by construction for every Layer-B row — there is no fire-telemetry pipeline for CLI-session crons, so `null` there does not mean "never fired," it means "not tracked." Conflating the two branches asserts a false fact for Layer-B (FIX-DASH-CRON-LAYERB-NEVERFIRED-FALSE-LABEL, 2026-08-24 — see BA-DASH-CRON-RECHECK-TABLE.md:364 AC-20's original "(Layer-A, never fired)" qualifier, dropped when this brief was authored).
