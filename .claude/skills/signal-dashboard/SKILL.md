...
> Shell/flow code MUST route through `scripts/orch-apply.sh` — the sole CAS-guard provider for
> shell/flow writes (see Write protocol above); TS code uses `appendSignalQueueRow()` (line 19).
> **Never use bare temp→rename without the CAS guard** — last-write-wins drops the first writer's rows.
...
- **Extended statuses (admitted, PO-only, decided 2026-08-01 per FIX-PO-TRIAGE-SIGNALS-TABLE-MATCHES-ZERO-LIVE-SIGNAL-TYPES AC-4):** `"triaged"` (disposition recorded — carries `disposition`/`triaged_at`/`triaged_by`; treat as equivalent to `RESOLVED` for downstream consumers) and `"RETRACTED"` (verified false positive, no action taken). Both were live on 127+3 of 132 hot `to=po` rows with zero prior doc backing before this fix — chosen over rewriting PO's writing flow because the richer audit trail is already load-bearing. **Eviction gap CLOSED 2026-08-08 (FIX-COLDEVICT-TERMINAL-SIGNAL-STATUSES-OMITS-TRIAGED-RETRACTED):** `scripts/orch-cold-evict.sh`'s `TERMINAL_SIGNAL_STATUSES` default now includes both case variants of `triaged` (matching is exact-string, no case fold) plus `RETRACTED` — see § PRUNE criteria below, which is the live SSOT for the current default value.
...
**Criteria:** status IN (`READ`, `RESOLVED`, `SUPERSEDED`, `ACUTE-RESOLVED-ROOT-TRACKED`, `triaged`, `TRIAGED`, `RETRACTED`) AND row `ts` older than 24h.
