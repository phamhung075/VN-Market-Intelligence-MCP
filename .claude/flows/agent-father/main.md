...
|---|---|---|
| `trigger=scheduled` (cron tick) | `.claude/flows/agent-father/keep.md` | Default for ambient invocations — orphan + roster sweep |
| `intent=create` or "create new agent" | `.claude/flows/agent-father/create.md` | Requires `agent_name`, `agent_type`, `purpose_description` |
| `intent=edit` or "edit agent X" | `.claude/flows/agent-father/edit.md` | Requires `agent_name`, `change_description` |
| `intent=review` or "audit agents" | `.claude/flows/agent-father/review.md` | Requires `agent_name(s) or "all"` |
| `intent=keep` or "maintenance" | `.claude/flows/agent-father/keep.md` | Default fallback |
