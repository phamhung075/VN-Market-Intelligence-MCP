#!/bin/bash
# VN Market board details fetcher — forever loop driver for systemd
#
# Runs under vn-board-details.service (Type=simple, Restart=always).
# DO NOT call from cron directly.
#
# Cadence: Board composition data is stable (changes only at AGM or mid-term
# resignation). Daily sweep at 02:00 UTC (after AGM plan sweep at 01:00).
...
FAILURE_COUNT=0
ALERT_SENT=0
MCP_BASE="${BOARD_DETAILS_MCP_BASE:-https://zenmidi.com}"
VPS_API_KEY="${API_KEY:-}"

log_backoff() {
  local failures=$1
  local delay=$2
  echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] fetch-board-details-loop: $failures consecutive failures → ${delay}s delay"
}
...
# Target: once daily near 02:00 UTC (after AGM plan sweep at 01:00 UTC)
TARGET_HOUR=2
...
while true; do
  ...
  if [ "$now_hour" -eq "$TARGET_HOUR" ] && [ "$now_day" != "$LAST_RUN_DAY" ]; then
    if /root/fetch-board-details.sh; then
      LAST_RUN_DAY="$now_day"
      if [ "$FAILURE_COUNT" -gt 0 ]; then
        echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] fetch-board-details-loop: recovered after $FAILURE_COUNT failures"
        FAILURE_COUNT=0
        ALERT_SENT=0
      fi
      ...
      if [ "$FAILURE_COUNT" -eq 10 ] && [ "$ALERT_SENT" -eq 0 ]; then
        ALERT_SENT=1
        send_alert "VN board details fetch: 10 consecutive failures at $(date -u +'%Y-%m-%d %H:%M:%S'). Check: ssh root@\$VINAHOST_IP journalctl -u vn-board-details.service -n 50"
      fi
