...
The task commit (`cc28c63`) includes both the test file and the implementation. 10 tests cover: happy-path document list, document shape, absolute URL resolution, empty-page edge case, network error graceful degradation, quarterly keyword filtering, annual keyword filtering, `reportType` tagging for both types, and `publishedAt` non-empty assertion. All HTTP calls are mocked — no real network traffic in the test suite.
