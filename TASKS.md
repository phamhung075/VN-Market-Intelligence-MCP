...
| 1003 | FPT/VEA Q4-2025 BCTC overdue 8 days, no PDFs in store. Verify SSC downloader queries FPT/VEA filings page; check if portal HTML changed or filings genuinely not published. Report 998. | `task/1003-ssc-overdue-investigation` | Developer | P2 | — | Backlog |
...
| 1025 | SSC BCTC PDF download — ADF portal migration full fix (Task 1035) | Report 1034: SSC portal migrated to Oracle ADF SPA. Minimum fix (8f95ca1) stops 568 error/week by fixing URL + UA. Full fix needed: ADF PPR does not expose direct PDF URLs in HTML. Design options: (A) ADF session simulation (POST PPR to get download URL), (B) route PDF fetches through Singapore VPS (already has SSC access for price data), (C) scrape HOSE/HNX company disclosure pages for PDF links. The bctcReparseJob already re-parses local PDFs — the gap is DISCOVERING new PDF URLs for HPG/MSN/MWG/SSI/VEA Q4-2025. financial_reports table is empty (0 rows). | Developer | P1 | — | Backlog |
