...
| 1038 | [janitor] Remove ensureBrokerSanctionsTable() from brokerSanctionStore.ts — broker_sanctions DDL duplicates schema.ts | code-janitor auto-detected: src/infrastructure/db/brokerSanctionStore.ts defines broker_sanctions DDL; identical table already created by initDatabase() in schema.ts:305. Remove the local helper and its call sites. | Developer | P3 | — | Done — initBrokerSanctionTable() removed from brokerSanctionStore.ts; test 915-broker-credibility.test.ts updated with inline createBrokerSanctionsSchema() helper; tsc clean; 22/22 tests pass. |
| 1039 | [janitor] Remove ensureTrackedIndicatorsTable() from commodityTracker.ts — tracked_indicators DDL duplicates schema.ts | code-janitor auto-detected: src/infrastructure/db/commodityTracker.ts defines tracked_indicators DDL; identical table already created by initDatabase() in schema.ts:693. Remove the local helper and its call sites. | Developer | P3 | — | Done — ensureTable()/_tableCreated removed; canonical DDL stays in schema.ts:715; regression test 1039-commodity-tracker-ddl-dedup added; 5/5 pass; full suite 3467 green; tsc clean. |
| 1040 | [janitor] Move briefing_log DDL from morningBriefingJob.ts into initDatabase() in schema.ts | code-janitor auto-detected: src/scheduler/morningBriefingJob.ts:82 creates briefing_log table inline at runtime; this table is not known to initDatabase(). Add DDL to schema.ts:initDatabase() and remove the inline CREATE TABLE from the scheduler. | Developer | P3 | — | Done — briefing_log DDL moved to schema.ts; inline CREATE TABLE removed from morningBriefingJob.ts; 7/7 tests pass. Commit 33a4946. |
| 1041 | [janitor] Move audit_state DDL from dataAuditJob.ts into initDatabase() in schema.ts | code-janitor auto-detected: src/scheduler/dataAuditJob.ts:124 creates audit_state table inline; this table is not known to initDatabase(). Add DDL to schema.ts:initDatabase() and remove the inline CREATE TABLE from the scheduler. | Developer | P3 | — | Done — audit_state DDL moved to schema.ts; ensureAuditDependencies() is now a no-op; 7/7 tests pass. Commit 33a4946. |
| 1042 | [janitor] Move vnstockStore.ts inline DDL (8 tables) into initDatabase() in schema.ts | code-janitor auto-detected: src/infrastructure/db/vnstockStore.ts defines 8 tables inline (vnstock_financials, vnstock_trading_stats, vnstock_events, vnstock_officers, vnstock_shareholders, vnstock_fetch_log, vnstock_balance_sheet, vnstock_cash_flow); none of these are in initDatabase(). Move all DDLs to schema.ts and remove local ensureVnstockTables() call. | Developer | P3 | — | Done — DDL moved to initDatabase() in schema.ts; initVnstockTables() kept for backward compat; 11 new tests; commit bfb06d0 2026-04-08 |
| 1043 | [janitor] Move tradeStore.ts inline DDL into initDatabase() in schema.ts | code-janitor auto-detected: src/infrastructure/db/tradeStore.ts defines trade_exposures table inline; not in initDatabase(). Add DDL to schema.ts and remove local CREATE TABLE. | Developer | P3 | — | Done — trade_exposures DDL moved to schema.ts; ensureTable() now a no-op; 9/9 tests pass. Commit 28c52ce. |
| 1044 | [janitor] Move cascadeHitStore.ts inline DDL into initDatabase() in schema.ts | code-janitor auto-detected: src/infrastructure/db/cascadeHitStore.ts defines cascade_rule_hits table inline; not in initDatabase(). Add DDL to schema.ts and remove local CREATE TABLE. | Developer | P3 | — | Done — cascade_rule_hits DDL moved to schema.ts; ensureCascadeHitsTable() remains exported for test compat; 9/9 tests pass. Commit 28c52ce. |
| 1045 | [janitor] Move bondMaturityStore.ts inline DDL into initDatabase() in schema.ts | code-janitor auto-detected: src/infrastructure/db/bondMaturityStore.ts defines bond_maturity table inline; not in initDatabase(). Add DDL to schema.ts and remove local CREATE TABLE. | Developer | P3 | — | Done — bond_maturity DDL moved to schema.ts; ensureBondMaturityTable() remains for backward compat; 9/9 tests pass. Commit 26a5746. |
| 1046 | [janitor] Move pharmaStore.ts inline DDL into initDatabase() in schema.ts | code-janitor auto-detected: src/infrastructure/db/pharmaStore.ts defines pharma_events table inline; not in initDatabase(). Add DDL to schema.ts and remove local CREATE TABLE. | Developer | P3 | — | Done — pharma_events DDL moved to schema.ts; initPharmaStore() remains for backward compat; 9/9 tests pass. Commit 26a5746. |
| 1047 | [janitor] Move hexagramStore.ts inline DDL into initDatabase() in schema.ts | code-janitor auto-detected: src/infrastructure/db/hexagramStore.ts defines kinhdich_readings and hexagram_transitions tables inline; neither is in initDatabase(). Add both DDLs to schema.ts and remove local helpers. | Developer | P3 | — | Done — both tables moved to schema.ts; initHexagramTables() remains exported as no-op; source column included in canonical DDL (eliminates ALTER TABLE migration for fresh installs); 8/8 tests pass. Commit 4a24803. |
