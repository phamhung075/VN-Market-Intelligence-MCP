...
|----|-------|--------|-------|------------|------|-----------|--------|
| 1070 | Position ledger: buyPosition + sellPosition + applyPositionCommand | `task/1070-position-ledger` | domain/infra | — | M | `src/__tests__/1070-position-ledger.test.ts` | Todo P0 |
| 1071 | Telegram /set_position + /check_position handlers | `task/1071-telegram-position-commands` | interface | 1070 | M | `src/__tests__/1071-telegram-position-commands.test.ts` | Backlog |
| 1072 | ask_queue DDL + askQueueStore CRUD helpers | `task/1072-ask-queue-store` | infrastructure | — | M | `src/__tests__/1072-ask-queue-store.test.ts` | Todo P0 |
| 1073 | Telegram /ask handler | `task/1073-telegram-ask-command` | interface | 1072 | S | `src/__tests__/1073-telegram-ask-command.test.ts` | Backlog |
| 1074 | askQueueCheckJob scheduler + cron registration | `task/1074-ask-queue-check-job` | scheduler | 1072 | S | `src/__tests__/1074-ask-queue-check-job.test.ts` | Backlog |
| 1075 | alertPolicyChecker + stopLossComputer + mcp.config.json alertPolicy | `task/1075-alert-policy-checker` | domain | — | M | `src/__tests__/1075-alert-policy-checker.test.ts` | Backlog |
| 1076 | marketScanJob noise retirement (remove direct MARKET sends) | `task/1076-market-scan-noise-retirement` | scheduler | 1075 | S | `src/__tests__/1076-market-scan-noise-retirement.test.ts` | Backlog |
...
| 1078 | askQueueTools: get_pending_ask_questions + answer_ask_question MCP tools | `task/1078-ask-queue-tools` | interface | 1072 | S | `src/__tests__/1078-ask-queue-tools.test.ts` | Backlog |
...
| 985 | Investigate macro alert HIGH/CRITICAL count = 0 despite Brent $108 — verify Step A2.5 actually inserts (check classifyDeviation z-score for current Brent stats; suspect rolling stdDev too high to trigger HIGH at z>=2, OR INSERT OR IGNORE dedup hiding insert) | `task/985-macro-alert-investigation` | Developer | P1 | — | Done — resolved by tasks 137 (schema fix) + 1024 (retry window 24h); verified live: Brent EXTREME (-3.02σ) + Gold HIGH (+2.17σ) alerts present in DB 2026-04-07 |
| 1001 | BCTC ingest regression: VNM PDF on disk 9 days, get_bctc_full returns "Chua co du lieu". Tasks 309/310 marked Done but pipeline not populating financial_reports table. Verify fetchParseAndStoreBctc actually called for stranded PDFs; check filename matcher (`BCTC VNM 31.12.2025 - HOP NHAT - VN.pdf`). Reports 996/997/998. | `task/1001-bctc-ingest-regression` | Architect | P1 | — | Backlog |
| 1002 | Anonymous SSC PDF attribution: filenames like `000000015802468_Bao_cao_tai_chinh_Rieng_nam_2025.pdf` carry no stock code. Add download-time normalisation that records stock_code from SSC portal metadata, OR extract from PDF first page on ingest. Report 997. | `task/1002-pdf-stock-attribution` | Architect | P1 | — | Backlog |
| 1003 | FPT/VEA Q4-2025 BCTC overdue 8 days, no PDFs in store. Verify SSC downloader queries FPT/VEA filings page; check if portal HTML changed or filings genuinely not published. Report 998. | `task/1003-ssc-overdue-investigation` | Developer | P2 | — | Done — 10 regression tests in 1003-ssc-fpt-vea-query.test.ts; FINDINGS: FPT=filing genuinely not published (portal JS-shell, HOSE fallback empty); VEA=UPCOM coverage gap in HOSE/HNX fallback (Task 1025 gap); x17f parser was correct. Commit af1393c 2026-04-08 |
| 1004 | Cascade gap: VN-market policy/macro news (e.g. "government-backed fund to stabilize stock market", "war-induced decline") scoring at base 10.0 — cascade engine missing rules for govt-stabilization signals. Add cascade rules + raise base score for systemic-stress + policy-intervention combos. Report 1001. | `task/1004-cascade-policy-gap` | Architect | P2 | — | Backlog |
| 1005 | Pre-fix phantom alert marker: log_fix now accepts `supersedes_alert_ids`; `markAlertsSuperseded` writes `resolution_notes`+`resolved_at`; `formatAlertRow` shows `[SUPERSEDED]` tag. 7 new tests in `1005-superseded-alert-marker.test.ts`. | `task/1005-superseded-alert-marker` | Developer | P3 | — | ✅ Done 2026-04-08 |
| 1006 | Sector peer PE/PB/ROE returns N/A for HAX/CTF/TMT/SMA (VEA peers), BID/CTG/TCB/MBB/VPB/ACB/STB/HDB (VCB peers), CMG/ELC/SAM (FPT peers). syncSectorPeers Step C2 wired but peers never get vnstock financials. Verify syncStockLight is called for all peer codes and not just watchlist. Report 1006. | `task/1006-sector-peer-financials` | Developer | P2 | — | Done — MAX_PEER_SYNCS_PER_CYCLE raised 5→30; automotive peers now reachable; 5 new tests in 1006-sector-peer-financials.test.ts |
| 1007 | Kinh Dich convergence: identical Que Du (16) for VCB/VNM/FPT in same cycle when BCTC absent + small price moves. Add jitter via stock-code hash, OR weight micro-features (open chains, sentiment delta) more aggressively. Report 1007. | `task/1007-kinhdich-convergence` | Developer | P3 | — | Done — tickerJitter() added to computeHaoScores; Hao 5 perturbed ±[0.05,0.09] when sector score=0.0; 8 tests in 1007-kinhdich-convergence.test.ts |
| 1018 | BCTC overdue Telegram alert: scheduler hook that emits an `alerts` row + chat message via Alert Commander when any watchlist stock crosses +3 days past deadline. **Slice 1 DONE** (`src/scheduler/bctcOverdueCheckJob.ts` + test 316, idempotent per UTC day, no Telegram, no cron). Slice 2 + Slice 3 tracked as 1018b/1018c below. Report 1013. | `task/1018-bctc-overdue-alert` | Developer | P1 | — | Done |
| 1018b | BCTC overdue Slice 2: wire `runBctcOverdueCheck` output into Alert Commander dedup pipeline. **DONE 2026-04-07** — severity bumped warning→high, readUnnotifiedAlerts assertion in test 316 (6 pass). Commit 4cbf6c4. | `task/1018b-bctc-overdue-wire` | Developer | P2 | 1018 | Done |
| 1018c | BCTC overdue Slice 3: register `bctcOverdueCheckJob` in `src/scheduler/jobs.ts` (daily 09:00 GMT+7). **DONE 2026-04-07** — `CRONS.bctcOverdueCheck` registered + header doc updated. Needs `launchctl kickstart` on next deploy. | `task/1018c-bctc-overdue-cron` | Developer | P2 | 1018b | Done |
| 1019 | SSC fetch network timeout not tripping circuit breaker (12:25 UTC 2026-04-07). Root cause: `downloadAndExtractPdf` caught all HTTP errors internally and returned empty, bypassing breakers.ssc on the PDF *download* path (listing path was already wrapped by 13db1f7). Fix 2026-04-08: optional CircuitBreaker param on `downloadAndExtractPdf` wired to breakers.ssc from fetchParseAndStoreBctc; OCR fallback axios also wrapped. 3 regression tests. Commit a8287c6. | `task/1019-ssc-breaker-bypass` | Developer | P3 | — | Done |
| 1020 | Kinh Dich identical-result repro confirmed (VCB/VNM/FPT all Que Du 16 same cycle 2026-04-07) — duplicate of 1007 but adds evidence: same -0.30 NguHanh score, same hao pattern. When working 1007, use these three readings as the failing fixture. Report 1014. | `task/1020-kinhdich-evidence` | Developer | P3 | 1007 | Done — resolved by task 1007 fix |
| 1021 | [janitor] Remove legacy src/db/schema.ts — dead schema file with diverged column definitions | code-janitor auto-detected: src/db/schema.ts duplicates core table DDL (watchlist, market_prices, alerts, rag_analyses) with diverged columns vs canonical src/infrastructure/db/schema.ts; not imported anywhere in production — safe to delete. | Developer | P2 | — | Done — src/db/schema.ts does not exist (already removed) |
| 1022 | [janitor] Consolidate inline schema guards in telegramCommands.ts into canonical initDatabase() | code-janitor auto-detected: src/infrastructure/notifiers/telegramCommands.ts defines user_requests and agent_feedback tables inline; user_requests is already in canonical schema.ts:606; agent_feedback is only defined in telegramCommands.ts and dataAuditJob.ts but not in initDatabase() — add it to schema.ts and remove both inline guards. | Developer | P2 | — | Done — agent_feedback canonical in schema.ts:622, telegramCommands.ts comment confirms (task 1022) |
| 1023 | [janitor] Move SUMMARY_CRONS into CRONS map in src/scheduler/jobs.ts | code-janitor auto-detected: src/scheduler/summaryJobs.ts defines its own SUMMARY_CRONS map with hardcoded cron strings not overridable via env vars; canonical pattern is CRONS in jobs.ts with Bun.env fallbacks — merge summary schedule keys into CRONS. | Developer | P3 | — | Done — CRONS.summary{Daily..Yearly} added with CRON_SUMMARY_* env overrides; SUMMARY_CRONS now reads same env vars; 8 tests pass. Commit 3e59f57. |
| 1024 | [janitor] Fix hardcoded WAL checkpoint cron in jobs.ts — wrong time and not in CRONS | code-janitor auto-detected: src/scheduler/jobs.ts:130 schedules runWalCheckpoint() with hardcoded '0 20 * * *' (20:00 daily) while the comment says "03:00 GMT+7" — wrong fire time AND not in CRONS map (cannot be overridden via env). Add CRONS.walCheckpoint entry with correct expression and replace the inline literal. | Developer | P2 | — | Done — CRONS.walCheckpoint entry already exists (line 71); cron.schedule call already uses CRONS.walCheckpoint (no inline literal). Task was stale — closed as already implemented. |
| 1025 | SSC BCTC PDF download — ADF portal migration full fix (Task 1035) | Report 1034: SSC portal migrated to Oracle ADF SPA. Minimum fix (8f95ca1) stops 568 error/week by fixing URL + UA. Full fix needed: ADF PPR does not expose direct PDF URLs in HTML. **DONE 2026-04-08** — fetchHoseDisclosures + fetchHnxDisclosures added to ssc.ts; wired as parallel fallback in listSscDocuments when portal_js_only triggers; titleMatchesReportType annual guard prevents quarterly docs leaking into annual filter; 10 tests pass. | Developer | P1 | — | Review |
| 1026 | Startup env self-check: fail fast + WORK alert if TELEGRAM_BOT_TOKEN missing | Reports 1035/1038/1039 (2026-04-08 00:01 UTC): a momentary env blip during one intelligence-cycle produced "TELEGRAM_BOT_TOKEN is not set" WARN in system_logs, causing analysis agents to report a prod regression. Root cause: the MCP server has no startup gate that verifies required env vars are present before the first cron fires. Fix: add a `checkRequiredEnv()` call in `src/index.ts` bootstrap that (a) logs a structured ERROR and (b) sends a plain-text WORK channel alert via a direct HTTP call (not via sendTelegram, to avoid circular dep) if TELEGRAM_BOT_TOKEN or any channel ID is blank. If a required var is absent, the server should still boot (degraded mode) but must emit a visible BUG channel notice. Priority P2 — prevents false-positive env-regression reports from analysis agents. | Developer | P2 | — | Done — src/infrastructure/envCheck.ts + wired in src/index.ts; 7 tests pass. |
| 1027 | [janitor] Remove inline market_prices_history DDL in hose.ts — missing exchange column vs schema.ts | code-janitor auto-detected: src/infrastructure/fetchers/hose.ts:ensureHistoryTable() defines market_prices_history without the exchange column; canonical schema.ts:135 includes exchange TEXT DEFAULT 'HOSE'. If hose.ts runs first on a fresh DB the table will be created without exchange, causing schema divergence. Delete ensureHistoryTable() and rely on initDatabase() from schema.ts. | Developer | P2 | — | Done — ensureHistoryTable() deleted, ALTER TABLE exchange guard removed, test 1027 added. Commit 3ebb468. |
| 1028 | [janitor] Remove inline market_prices_history DDL in correlationTools.ts — missing exchange column vs schema.ts | code-janitor auto-detected: src/interface/mcp/tools/correlationTools.ts:77 defines market_prices_history inline without the exchange column; canonical schema.ts:135 includes exchange TEXT DEFAULT 'HOSE'. Delete the inline CREATE TABLE block and rely on initDatabase(). | Developer | P2 | — | Done — inline CREATE TABLE block removed. Commit 3ebb468. |
| 1029 | [janitor] Remove inline telegram_reports + system_changelog DDL in devTeamHeartbeatJob.ts | code-janitor auto-detected: src/scheduler/devTeamHeartbeatJob.ts defines telegram_reports and system_changelog via local ensureReportsTable()/ensureChangelogTable() helpers; both tables are already created by initDatabase() in schema.ts (lines 568, 589). Delete the two local helpers and their call sites; initDatabase() is already called at server startup. | Developer | P3 | — | Done — ensureReportsTable() + ensureChangelogTable() deleted. Commit 3745f1f. |
| 1030 | [janitor] Remove inline tracked_indicators DDL in shippingIndex.ts | code-janitor auto-detected: src/infrastructure/fetchers/shippingIndex.ts:228 creates tracked_indicators inline; canonical definition is in schema.ts:674 (identical DDL). Delete the inline block and rely on initDatabase(). | Developer | P3 | — | Done — inline CREATE TABLE block removed. Commit 3745f1f. |
| 1033 | [janitor] Remove duplicate mention_velocity CREATE TABLE block inside schema.ts | code-janitor auto-detected: src/infrastructure/db/schema.ts defines mention_velocity twice (Task 265 ~line 271, Sprint 031 ~line 688). Identical DDLs, runtime no-op via IF NOT EXISTS but a footgun on future column changes. Delete the second block. | Developer | P3 | — | Done — second block removed, canonical definition kept at ~line 271, regression test 1033 asserts exactly one CREATE TABLE remains. Commit bde2b04. |
| 1034 | [janitor] Remove ensureChangelogTable() from changelogStore.ts — system_changelog DDL duplicates schema.ts | code-janitor auto-detected: src/infrastructure/db/changelogStore.ts:ensureChangelogTable() defines system_changelog DDL; identical table already created by initDatabase() in schema.ts:608. Remove the local helper and its call sites; rely on initDatabase() at server startup. | Developer | P3 | — | Done — helper removed from changelogStore.ts; changelogTools.ts no longer calls it; tests use a private DDL helper. tsc clean, 17/17 tests pass. |
| 1035 | [janitor] Remove ensureTelegramReportsTable() from telegramReportStore.ts — telegram_reports DDL duplicates schema.ts | code-janitor auto-detected: src/infrastructure/db/telegramReportStore.ts:ensureTelegramReportsTable() defines telegram_reports DDL; identical table already created by initDatabase() in schema.ts:587. Remove the local helper and its call sites; rely on initDatabase() at server startup. | Developer | P3 | — | Done |
| 1036 | [janitor] Remove inline user_requests DDL from userRequestStore.ts — duplicates schema.ts | code-janitor auto-detected: src/infrastructure/db/userRequestStore.ts defines user_requests DDL inline; identical table already created by initDatabase() in schema.ts:625. Remove the local CREATE TABLE and rely on initDatabase(). | Developer | P3 | — | Done — ensureUserRequestsTable() removed; test-only helper added at src/__tests__/helpers/userRequestsTestDdl.ts; 5 test files updated; 64/64 tests pass. Commit d0b2b53. |
| 1037 | [janitor] Remove ensureReputationTable() from reputationStore.ts — reputation_scores DDL duplicates schema.ts | code-janitor auto-detected: src/infrastructure/db/reputationStore.ts defines reputation_scores DDL; identical table already created by initDatabase() in schema.ts:286. Remove the local helper and its call sites. | Developer | P3 | — | Done — initReputationTable() removed; test-only helper at src/__tests__/helpers/reputationScoresTestDdl.ts; 3 test files updated; regression test 1037 added; 27/27 tests pass. |
| 1038 | [janitor] Remove ensureBrokerSanctionsTable() from brokerSanctionStore.ts — broker_sanctions DDL duplicates schema.ts | code-janitor auto-detected: src/infrastructure/db/brokerSanctionStore.ts defines broker_sanctions DDL; identical table already created by initDatabase() in schema.ts:305. Remove the local helper and its call sites. | Developer | P3 | — | Backlog |
| 1039 | [janitor] Remove ensureTrackedIndicatorsTable() from commodityTracker.ts — tracked_indicators DDL duplicates schema.ts | code-janitor auto-detected: src/infrastructure/db/commodityTracker.ts defines tracked_indicators DDL; identical table already created by initDatabase() in schema.ts:693. Remove the local helper and its call sites. | Developer | P3 | — | Backlog |
| 1040 | [janitor] Move briefing_log DDL from morningBriefingJob.ts into initDatabase() in schema.ts | code-janitor auto-detected: src/scheduler/morningBriefingJob.ts:82 creates briefing_log table inline at runtime; this table is not known to initDatabase(). Add DDL to schema.ts:initDatabase() and remove the inline CREATE TABLE from the scheduler. | Developer | P3 | — | Backlog |
| 1041 | [janitor] Move audit_state DDL from dataAuditJob.ts into initDatabase() in schema.ts | code-janitor auto-detected: src/scheduler/dataAuditJob.ts:124 creates audit_state table inline; this table is not known to initDatabase(). Add DDL to schema.ts:initDatabase() and remove the inline CREATE TABLE from the scheduler. | Developer | P3 | — | Backlog |
| 1042 | [janitor] Move vnstockStore.ts inline DDL (8 tables) into initDatabase() in schema.ts | code-janitor auto-detected: src/infrastructure/db/vnstockStore.ts defines 8 tables inline (vnstock_financials, vnstock_trading_stats, vnstock_events, vnstock_officers, vnstock_shareholders, vnstock_fetch_log, vnstock_balance_sheet, vnstock_cash_flow); none of these are in initDatabase(). Move all DDLs to schema.ts and remove local ensureVnstockTables() call. | Developer | P3 | — | Backlog |
| 1043 | [janitor] Move tradeStore.ts inline DDL into initDatabase() in schema.ts | code-janitor auto-detected: src/infrastructure/db/tradeStore.ts defines trade_exposures table inline; not in initDatabase(). Add DDL to schema.ts and remove local CREATE TABLE. | Developer | P3 | — | Backlog |
| 1044 | [janitor] Move cascadeHitStore.ts inline DDL into initDatabase() in schema.ts | code-janitor auto-detected: src/infrastructure/db/cascadeHitStore.ts defines cascade_rule_hits table inline; not in initDatabase(). Add DDL to schema.ts and remove local CREATE TABLE. | Developer | P3 | — | Backlog |
| 1045 | [janitor] Move bondMaturityStore.ts inline DDL into initDatabase() in schema.ts | code-janitor auto-detected: src/infrastructure/db/bondMaturityStore.ts defines bond_maturity table inline; not in initDatabase(). Add DDL to schema.ts and remove local CREATE TABLE. | Developer | P3 | — | Backlog |
| 1046 | [janitor] Move pharmaStore.ts inline DDL into initDatabase() in schema.ts | code-janitor auto-detected: src/infrastructure/db/pharmaStore.ts defines pharma_events table inline; not in initDatabase(). Add DDL to schema.ts and remove local CREATE TABLE. | Developer | P3 | — | Backlog |
| 1047 | [janitor] Move hexagramStore.ts inline DDL into initDatabase() in schema.ts | code-janitor auto-detected: src/infrastructure/db/hexagramStore.ts defines kinhdich_readings and hexagram_transitions tables inline; neither is in initDatabase(). Add both DDLs to schema.ts and remove local helpers. | Developer | P3 | — | Backlog |
| 1048 | [janitor] Consolidate scheduler cron defaults — config.ts duplicates CRONS fallbacks from jobs.ts | code-janitor auto-detected: src/infrastructure/config.ts:scheduler section defines 7 cron default strings (sscCheck, morningBriefing, marketOpen, marketClose, intelligenceCycle, eveningSummary, predictionMarketPoll) that duplicate the fallback defaults already in CRONS (jobs.ts). Any cron default change requires two edits. config.ts should import and derive from CRONS rather than redeclaring literals. | Developer | P3 | — | Backlog |
...
| # | Title | Branch | Agent | Priority | Depends on | Status |
|---|-------|--------|-------|----------|------------|--------|
| 192 | Fix flaky test: `164-polymarket-fetcher.test.ts` mock timing | `task/192-fix-polymarket-flaky` | Developer | P0 | — | Done ✅ (2026-04-02) |
| 193 | Dynamic tool registration: eliminate server.ts merge conflicts | `task/193-dynamic-tool-registry` | Developer | P0 | — | Done ✅ (implemented as Task 308; registry.ts + toolRegistry loop live on main) |
| 194 | CLAUDE.md sync through Sprint 026 | `main` (7f53108) | — | P1 | — | Done |
