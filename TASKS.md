...
| 1024 | [janitor] Fix hardcoded WAL checkpoint cron in jobs.ts — wrong time and not in CRONS | code-janitor auto-detected: src/scheduler/jobs.ts:130 schedules runWalCheckpoint() with hardcoded '0 20 * * *' (20:00 daily) while the comment says "03:00 GMT+7" — wrong fire time AND not in CRONS map (cannot be overridden via env). Add CRONS.walCheckpoint entry with correct expression and replace the inline literal. | Developer | P2 | — | Backlog |
