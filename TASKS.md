...
| 1018b | BCTC overdue Slice 2: wire `runBctcOverdueCheck` output into Alert Commander dedup pipeline so the inserted alert row flows to the Chat Channel through the existing readUnnotifiedAlerts/sendTelegram path (no new Telegram code). Verify cooldown + dedup respect the deterministic alert id. Depends on 1018 slice 1. | `task/1018b-bctc-overdue-wire` | Developer | P2 | 1018 | Backlog |
| 1018c | BCTC overdue Slice 3: register `bctcOverdueCheckJob` in `src/scheduler/jobs.ts` (daily 09:00 VN = 02:00 UTC), update CLAUDE.md cron count, restart launchd. Depends on 1018b. | `task/1018c-bctc-overdue-cron` | Developer | P2 | 1018b | Backlog |
