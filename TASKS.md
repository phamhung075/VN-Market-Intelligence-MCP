...
| 1018 | BCTC overdue Telegram alert: add scheduler hook (daily 09:00 VN) that calls earningsCalendar overdue logic and emits an `alerts` row + chat message via Alert Commander pipeline when any watchlist stock crosses +3 days past deadline. Slice 1: write `bctcOverdueCheckJob.ts` calling existing `getEarningsCalendar` and inserting an alert row (no Telegram). Slice 2: wire into Alert Commander dedup. Slice 3: register cron in jobs.ts. Report 1013. | `task/1018-bctc-overdue-alert` | Architect | P1 | — | Backlog |
