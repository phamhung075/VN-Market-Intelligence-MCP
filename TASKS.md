...
## Sprint 067 — Morning Briefing Intelligence Enrichment (ACTIVE)
...
| 1159 | TDD: write failing tests for all 3 new briefing sections (AC-1 through AC-6) in `src/__tests__/1159-morning-briefing-enrichment.test.ts` | Developer | tests | TECH-067 ✓ | task/1159-morning-briefing-enrichment | Review |
| 1160 | Extend `DailyBriefing` type + add 3 exported row types + `BEARISH_WARNING_THRESHOLD` constant + 3 query helpers + Steps 14-16 in `assembleBriefing.ts` (FR-1 to FR-4) | Developer | application | 1159 | task/1159-morning-briefing-enrichment | Todo |
...
**WIP state:** 0 tasks In Progress. Task 1159 in Review. Task 1160 is unblocked.

---

### Task 1159 — TDD: failing tests for all 3 briefing sections

**Branch**: `task/1159-morning-briefing-enrichment`
**Layer**: tests
**Depends on**: TECH-067 (approved — `docs/TECH_067.md` exists)
**Status**: Review — red tests committed, 30 fail / 1 pass on branch task/1159-morning-briefing-enrichment

#### Files to read first

- `src/application/usecases/assembleBriefing.ts` — existing `DailyBriefing` interface, existing test helper patterns
- `src/__tests__/` — naming conventions for existing test files

#### Files to create

- CREATE: `src/__tests__/1159-morning-briefing-enrichment.test.ts`

#### What to implement

Write tests that FAIL on commit (red phase). They must import the not-yet-defined types
(`InsiderBriefingRow`, `ForeignFlowBriefingRow`, `EvidenceScoreBriefingRow`,
`BEARISH_WARNING_THRESHOLD`) and call the not-yet-defined helpers
(`queryInsiderRecent`, `queryForeignFlowSummary`, `queryEvidenceTopScores`). Use
`bun:test` + `bun:sqlite` in-memory DB fixtures as in existing tests.

#### Acceptance Criteria

**Given** `src/__tests__/1159-morning-briefing-enrichment.test.ts` is created with an in-memory SQLite DB seeded with fixture rows
**When** `bun test src/__tests__/1159-morning-briefing-enrichment.test.ts` runs (before Task 1160)
**Then**
- Tests compile (TypeScript import errors acceptable; logic tests must be visible)
- AC-1: `queryInsiderRecent` returns up to 3 rows ordered by `executedVolume DESC` for watchlist codes active in the last 24h
- AC-2: `queryInsiderRecent` returns `[]` when `watchlistCodes` is empty
- AC-3: `queryForeignFlowSummary` returns top 3 net-buy + top 3 net-sell rows (up to 6 total); excludes `foreign_volume = 0` or NULL
- AC-4: `queryForeignFlowSummary` returns `[]` when `watchlistCodes` is empty
- AC-5: `queryEvidenceTopScores` returns top 3 bullish leaders (netScore > 0, not in bearish set) + all bearish warnings (netScore < -2.0) with `fragment_count >= 1`; bearish deduplication applied
- AC-6: `queryEvidenceTopScores` returns `[]` when `watchlistCodes` is empty
- All 6 AC tests FAIL (imports unresolved / functions not yet exported) — this is the expected red state
- `bun tsc --noEmit` → 0 errors once Task 1160 ships; at this stage type errors from missing exports are acceptable

---

### Task 1160 — Extend DailyBriefing + query helpers + Steps 14-16 in assembleBriefing.ts

**Branch**: `task/1159-morning-briefing-enrichment`
**Layer**: application
**Depends on**: 1159 (tests written — red state committed)
**Status**: Todo — blocked until 1159 committed

#### Files to read first

- `src/application/usecases/assembleBriefing.ts` — full file: `DailyBriefing` interface location (~line 30-80), Helpers section (`midnightVietnamAsUtc`, `todayVietnam`), Step 13a portfolioPnl block (insertion point), Step 13 persist block
- `docs/TECH_067.md` — all interface contracts and query helper implementations (canonical source)
...
1. Add named constant near top of file:
   ```typescript
   export const BEARISH_WARNING_THRESHOLD = -2.0;
   ```
2. Add 3 exported public types after existing type declarations:
   `InsiderBriefingRow`, `ForeignFlowBriefingRow`, `EvidenceScoreBriefingRow`
3. Add 3 private internal row types (not exported):
   `InsiderTransactionRow`, `VnstatsRow`, `EvidenceScoreRow`
4. Extend `DailyBriefing` with 3 optional fields after `portfolioPnl?`:
   `insiderRecent?`, `foreignFlowSummary?`, `evidenceTopScores?`
5. Add 3 private helper functions in the Helpers section:
   `queryInsiderRecent`, `queryForeignFlowSummary`, `queryEvidenceTopScores`
   (exact implementations in `docs/TECH_067.md`)
6. Insert Steps 14-16 in `assembleBriefing()` between Step 13a and Step 13 (persist):
   wrap each call in try/catch with `logger.warn` on failure
7. Add the 3 new fields to the `briefing` object literal: `insiderRecent`, `foreignFlowSummary`, `evidenceTopScores`

#### Acceptance Criteria

**Given** `src/application/usecases/assembleBriefing.ts` is modified per TECH-067
**When** `bun test src/__tests__/1159-morning-briefing-enrichment.test.ts` runs
**Then**
- AC-1 through AC-6 all PASS (green)
- `BEARISH_WARNING_THRESHOLD` is exported and equals `-2.0`
- `InsiderBriefingRow`, `ForeignFlowBriefingRow`, `EvidenceScoreBriefingRow` are exported
- `DailyBriefing` has 3 new optional fields that deserialise without error from pre-Sprint-067 JSON files
- Steps 14-16 each have independent try/catch; a failure in one does not abort briefing assembly
- `bun tsc --noEmit` → 0 errors
- `bun test src/__tests__/1159-morning-briefing-enrichment.test.ts` → all pass, 0 failures

---

### Task 1161 — Render 3 new Telegram sections in morningBriefingJob.ts

**Branch**: `task/1159-morning-briefing-enrichment`
**Layer**: interface/scheduler
**Depends on**: 1160 (types exported, helpers green)
**Status**: Todo — blocked until 1160 done

#### Files to read first

- `src/scheduler/morningBriefingJob.ts` — full file: locate `if (briefing.newReports.length > 0)` block and `const text = lines.join("\n")` line (insertion point for 3 new sections)
- `src/application/usecases/assembleBriefing.ts` — confirm `BEARISH_WARNING_THRESHOLD` is exported
...
Insert 3 new section blocks **after** the closing `}` of `if (briefing.newReports.length > 0)` and **before** `const text = lines.join("\n")`:

1. Insider Moi section (guard: `briefing.insiderRecent && briefing.insiderRecent.length > 0`)
2. Dong Tien Ngoai section (guard: `briefing.foreignFlowSummary && briefing.foreignFlowSummary.length > 0`)
3. Tich Luy Bang Chung section (guard: `briefing.evidenceTopScores && briefing.evidenceTopScores.length > 0`)

Import `BEARISH_WARNING_THRESHOLD` from `assembleBriefing.ts` at the top of the file.
Do NOT add `insiderRecent`, `foreignFlowSummary`, or `evidenceTopScores` to the `hasContent` guard (FR-6 is explicit).

See exact renderer code in `docs/TECH_067.md` — Telegram Formatter Changes section.

#### Acceptance Criteria

**Given** a `DailyBriefing` with `insiderRecent = [{code:"VCB", type:"buy", executedVolume:100000, insiderName:"Nguyen Van A", fromDate:"2026-04-13"}]`
**When** the Telegram formatter renders the briefing
**Then**
- Output contains line `"👤 Insider Mới:"`
- Output contains line `"  VCB: MUA 100,000 cp — Nguyen Van A"`
- Output does NOT contain Insider section when `insiderRecent` is empty or absent
- Output contains `"🌊 Dòng Tiền Ngoại:"` section when `foreignFlowSummary` has rows
- Output contains `"🧠 Tích Lũy Bằng Chứng:"` section when `evidenceTopScores` has rows
- `BEARISH_WARNING_THRESHOLD` is imported (not inlined as magic number)
- `hasContent` guard at line 91-97 is UNCHANGED
- `bun tsc --noEmit` → 0 errors

---

### Task 1162 — Advance project-stats.json to Sprint 67

**Branch**: `task/1159-morning-briefing-enrichment`
**Layer**: docs/data
**Depends on**: 1161 (all feature code merged)
**Status**: Todo — blocked until 1161 done

#### Files to read first

- `docs/data/project-stats.json` — current `currentSprint` value and `lastUpdated` field
...
Set `currentSprint` to `67` and update `lastUpdated` to `"2026-04-13"`.
No other fields change.

#### Acceptance Criteria

**Given** `docs/data/project-stats.json` before this task has `currentSprint` != 67
**When** the file is saved and committed
**Then**
- `currentSprint` = 67
- `lastUpdated` = `"2026-04-13"`
- JSON is valid (no trailing commas, no syntax errors)
- `bun tsc --noEmit` → 0 errors (no TypeScript impact expected)
- Sprint 067 Kanban row for 1162 updated to Done in TASKS.md
