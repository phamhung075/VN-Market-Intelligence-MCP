...
| 1036 | [janitor] Remove inline user_requests DDL from userRequestStore.ts — duplicates schema.ts | code-janitor auto-detected: src/infrastructure/db/userRequestStore.ts defines user_requests DDL inline; identical table already created by initDatabase() in schema.ts:625. Remove the local CREATE TABLE and rely on initDatabase(). | Developer | P3 | — | Backlog |
