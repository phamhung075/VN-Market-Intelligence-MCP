...
# parsing, and bad-usage exit code.
...
mkdir -p "$SANDBOX/docs/agent-memory/notebooks" "$SANDBOX/docs/agent-memory/decisions" \
  ...
  "$SANDBOX/docs/social" "$SANDBOX/docs/raw-input/expert-discussion"
...
echo "random" > "$SANDBOX/docs/social/fb-post-2026-07-01.md"
...
echo "$UNK" | grep -q "fb-post-2026-07-01.md" && ok "unknown-plain-doc-captured" || bad "unknown-plain-doc-captured"
...
[ "$OE_COUNT" = "5" ] && ok "owned-elsewhere-count-5" || bad "owned-elsewhere-count-5 (got=$OE_COUNT)"
