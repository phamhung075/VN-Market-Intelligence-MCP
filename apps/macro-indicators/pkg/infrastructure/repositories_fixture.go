// Package infrastructure — fixture-mode adapters for domain ports (P2-X3).
//
...
//
// Split from repositories.go (FACTORY-MACRO-split-repositories, 2026-07-09) into
// 5 per-adapter files, same package, no API change:
//
//	repositories_fixture.go       — HTTPCommodityFetcher, SBVRateRepository (this file)
//	repositories_market_index.go  — SQLiteMarketIndexRepository (MACRO-SEED-WIRING)
//	repositories_commodity.go     — SQLiteCommodityRepository, SQLiteCommodityHistoryRepository
//	repositories_sbv_rate.go      — SBVRateSQLiteAdapter (DPI-1)
//	repositories_carry_yield.go   — CarryYieldInputsSQLiteAdapter (DPI-2b) + shared
//	                                 openReadOnly(dbPath) helper reused by every
//	                                 live SQLite adapter above (was a private method
//	                                 on CarryYieldInputsSQLiteAdapter; promoted to a
//	                                 package-level function so the "open ro -> defer
//	                                 Close -> fetch" shape is not duplicated per-file).
//
