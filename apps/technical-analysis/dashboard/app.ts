 ...
 * <script> tags embedded in the HTML (works in file:// without a server —
 * no fetch() required, no backend, no credentials).
 *
 * Status values:
 *   "static"  — no sandbox result yet
 *   "green"   — sandbox confirmed all assertions passed
 *   "red"     — sandbox detected assertion failure
 *
 * Three tiers:
 *   Level 1 (primitive): 25 scenarios — pure Go functions
 *   Level 2 (module):     5 scenarios — pkg/module composition
 *   Level 3 (service):    3 scenarios — chi HTTP layer via httptest.NewServer
 */
...
export interface ScenarioData {
  ...
  module?: string;
  /** Set for service-tier scenarios. */
  service?: string;
...
declare global {
  interface Window {
    ...
    __SERVICE_DATA__: ScenarioData[];
    /**
     * Injected by dashboard/dist/scenario-results.js (generated by build.sh).
     * Maps scenario filename (without .json) → "green" | "red".
     * If missing, scenarios fall back to their embedded status ("static").
     */
    __SCENARIO_RESULTS__?: Record<string, string>;
...
//    Then apply build-time sandbox verdicts from __SCENARIO_RESULTS__
//    (generated by build.sh's bake step — sets status "green"|"red").
// ---------------------------------------------------------------------------

const PRIMITIVES: ScenarioData[] = window.__PRIMITIVES_DATA__ || [];
const MODULE_SCENARIOS: ScenarioData[] = window.__MODULE_DATA__ || [];
const SERVICE_SCENARIOS: ScenarioData[] = window.__SERVICE_DATA__ || [];

/**
 * Apply build-time sandbox verdicts to scenario arrays.
 * __SCENARIO_RESULTS__ is written by build.sh bake step.
 * If missing (e.g. file:// without a build), scenarios stay "static".
 * Manual paste-back via rerun-handler still works independently.
 */
function applyBuildVerdicts(): void {
  const results = window.__SCENARIO_RESULTS__;
  if (!results) return;
  ...
  for (const s of allScenarios) {
    const key = s.filename?.replace(/\.json$/, "") ?? "";
    const verdict = results[key];
    if (verdict === "green" || verdict === "red") {
      s.status = verdict as ScenarioStatus;
    }
  }
}
...
// ---------------------------------------------------------------------------
// 7. Render Microservice panel — real baked verdicts from httptest.NewServer
// ---------------------------------------------------------------------------

function renderServicePanel(): void {
  const panelBody = document.getElementById("service-panel-body")!;
  const totalChip = document.getElementById("svc-total-chip");
  const greenChipEl = document.getElementById("svc-green-chip");
  const redChipEl = document.getElementById("svc-red-chip");

  if (SERVICE_SCENARIOS.length === 0) {
    panelBody.innerHTML =
      '<p style="color:#9ca3af;font-size:0.82rem;padding:8px 0;">No service scenarios found.</p>';
    return;
  }

  const greenCount = SERVICE_SCENARIOS.filter((s) => s.status === "green").length;
  const redCount = SERVICE_SCENARIOS.filter((s) => s.status === "red").length;
  const total = SERVICE_SCENARIOS.length;

  if (totalChip) totalChip.textContent = `${total} scenarios`;
  if (greenChipEl) greenChipEl.textContent = `${greenCount} passed`;
  if (redChipEl) redChipEl.textContent = `${redCount} failed`;

  const allGreen = greenCount === total && total > 0;
  const anyRed = redCount > 0;
  const groupStatus = allGreen ? "PASSED" : anyRed ? "FAILED" : "NOT RUN";
  const groupStatusClass = allGreen
    ? "status-green-label"
    : anyRed
  ...
  const header = document.createElement("div");
  header.className = "primitive-group-header";
  header.style.borderLeft = "4px solid #0f766e";
  header.innerHTML = `
    <span>Go chi HTTP layer — <code>httptest.NewServer</code> (in-process, zero credentials)</span>
    <span class="group-status ${groupStatusClass}">${groupStatus}</span>
  `;
  ...
  SERVICE_SCENARIOS.forEach((s) => {
    ...
    const method = inp?.["method"] ?? "GET";
    const path = inp?.["path"] ?? "/";
    const inputSum = `${method} ${path}`;

    const out = s.expectedOutput as Record<string, unknown> | undefined;
    const statusCode = out?.["statusCode"] ?? "?";
    const outputSum = `→ ${statusCode}`;
  ...
  panelBody.appendChild(scenarioList);

  // Architecture note (compact, below cards)
  const note = document.createElement("div");
  note.style.cssText =
    "margin-top:12px;font-size:0.74rem;color:#64748b;border-top:1px solid #e5e7eb;padding-top:8px;";
  note.innerHTML = `
    Wiring: <code>infrastructure.NewTACalculator()</code> &rarr;
    <code>application.NewComputeTAUseCase(calc)</code> &rarr;
    <code>httpinterface.NewRouter(uc, logger)</code> &rarr;
    <code>httptest.NewServer(router)</code>.
    Zero DB credentials. Zero port binding. Env audit:
    <code>forbidden_matches: (empty)</code>.
    Rerun: <code>go run ./cmd/sandbox -tier=service -scenario=&lt;file&gt;</code>
  `;
...
window.renderServicePanel = renderServicePanel;
