...
/** @type {ScenarioData[]} */
const PRIMITIVES = window.__PRIMITIVES_DATA__ || [];
/** @type {ScenarioData[]} */
const MODULE_SCENARIOS = window.__MODULE_DATA__ || [];

/**
 * @typedef {Object} ScenarioData
 * @property {string} [primitive]
 * @property {string} [module]
 * @property {string} [category]      golden | edge | failure
 * @property {string} [description]
 * @property {string} [filename]      injected by index.html
 * @property {string} [status]        static | green | red
 * @property {Object} [input]
 * @property {Object} [expectedOutput]
 * @property {string[]} [primitives]  module-level: which primitives exercised
 */
...
function inputSummary(s) {
  ...
  if (s.primitive === 'CalculateRSI') {
    const len = input.closes ? input.closes.length : (input.closes_length || '?');
    return `${len} closes, period ${input.period || 14}`;
  }
  if (s.primitive === 'CalculateMACD') {
    const len = input.closes_count || (input.closes ? input.closes.length : '?');
    return `${len} closes, ${input.fast || 12}/${input.slow || 26}/${input.signalPeriod || 9}`;
  }
  if (s.primitive === 'CalculateBollingerBands') {
    const len = input.closes ? input.closes.length : '?';
  ...
  if (s.primitive === 'DetectCross') {
    const len = input.fastLine ? input.fastLine.length : '?';
...
function outputSummary(s) {
  const out = s.expectedOutput || {};
  if (s.primitive === 'CalculateRSI') {
    if (out.rsi) return `RSI[${out.length || out.rsi.length}], range [${out.rangeMin || 0}, ${out.rangeMax || 100}]`;
    if (out.error !== undefined) return `expects error`;
    return `length ${out.length || '?'}`;
  }
  if (s.primitive === 'CalculateMACD') {
    if (out.outputLength !== undefined) return `${out.outputLength} triples`;
    if (out.error !== undefined) return `expects error / null`;
    return JSON.stringify(out).slice(0, 50);
  }
  if (s.primitive === 'CalculateBollingerBands') {
    if (out.upper) return `${out.length || out.upper.length} windows`;
...
function openModal(scenario) {
  ...
  const label = scenario.primitive
    ? (PRIMITIVE_CONFIG.find(p => p.key === scenario.primitive) || { label: scenario.primitive }).label
    : `Module: ${scenario.module}`;

  title.textContent = scenario.filename ? `${label} — ${scenario.filename}` : label;

  catBadge.className = `modal-category category-chip ${catClass(scenario.category)}`;
  ...
  // Status bar
  const st = scenario.status || 'static';
  statusBar.className = `modal-status-bar`;
  if (st === 'green') {
    statusText.className = 'status-indicator green';
    statusText.innerHTML = '<span class="status-dot-lg dot-green"></span> PASSED';
    statusNote.textContent = 'Sandbox runner confirmed all assertions passed.';
  } else if (st === 'red') {
    statusText.className = 'status-indicator red';
    statusText.innerHTML = '<span class="status-dot-lg dot-red"></span> FAILED';
    statusNote.textContent = 'Sandbox runner detected assertion failure. Edit JSON and rerun to fix.';
  } else {
    statusText.className = 'status-indicator pending';
    statusText.innerHTML = '<span class="status-dot-lg dot-pending"></span> NOT YET RUN';
    statusNote.textContent = 'P1-E2 sandbox runner not yet wired. Scenarios loaded from JSON only.';
  }
...
function renderPrimitivesPanel() {
  ...
  PRIMITIVE_CONFIG.forEach(cfg => {
    ...
    const groupStatusClass = allGreen ? 'status-green-label' : anyRed ? 'status-red-label' : 'status-pending-label';
    ...
    scenarios.forEach(s => {
      ...
      card.setAttribute('aria-label', `View scenario: ${s.description ? s.description.slice(0, 60) : s.filename}`);

      const dotClass = s.status === 'green' ? 'dot-green' : s.status === 'red' ? 'dot-red' : 'dot-pending';
      ...
      card.innerHTML = `
        ...
        <span class="category-chip ${catClass(s.category)}" aria-hidden="true">${catLabel(s.category)}</span>
      `;
...
function renderModulePanel() {
  ...
  const greenCount = MODULE_SCENARIOS.filter(s => s.status === 'green').length;
  ...
  MODULE_SCENARIOS.forEach(s => {
    ...
    headerBtn.setAttribute('aria-label', `View module scenario: ${s.description ? s.description.slice(0, 60) : s.filename}`);
    ...
    body.innerHTML = `<p class="module-card-desc">${s.description ? s.description.slice(0, 200) : '(no description)'}</p>`;
