// runners.go — Scenario dispatch and primitive/module runners for sandbox.
// Extracted from main.go per FACTORY-KINHDICH-split-sandbox.
//
// size-justification: 400L — all 5 primitive runners + 1 module runner + dispatch
// + stubMarkovPort must reside together to avoid circular references between files
// and to keep the runner dispatch logic cohesive; the alternative (6 tiny files
// with 60-80L each) would fragment type definitions and complicate imports without
// reducing complexity.
package main
...
// runScenario loads and runs a single scenario file
