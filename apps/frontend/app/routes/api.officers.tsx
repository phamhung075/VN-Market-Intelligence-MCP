/**
 * Resource route — transparent proxy for GET /api/officers.
 *
 * Mirrors the api.shareholders.tsx proxy precedent (TASK-17 PAGE 14).
 * The officers dashboard page (dashboard.officers.tsx) calls
 * /api/officers from its Remix loader; this route intercepts that
 * request at the frontend origin (:3001) and forwards it to mcp-server (:3000).
 *
 * Upstream URL: ${MCP_SERVER_BASE_URL}/api/officers
 *
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const upstreamUrl = new URL(`${MCP_SERVER_BASE_URL}/api/officers`);
  if (code !== null) {
    upstreamUrl.searchParams.set("code", code);
  }
  const upstream = upstreamUrl.toString();

  return proxyUpstream(upstream, { method: "GET", headers: { Accept: "application/json" } }, { label: "api.officers" });
}
