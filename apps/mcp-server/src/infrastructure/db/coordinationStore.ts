...
// ---------------------------------------------------------------------------
// Scheduled Tasks — ST-3: query/mutation surface split to scheduledTaskStore.ts
// (FIX-CI-SIZELINT-COORDINATIONSTORE-BASELINE-1388L, 2026-08-08): every
// function there takes an already-open `Database` handle and never touches
// this module's private task_locks singleton, so it was always a cohesive,
// structurally independent module — just historically co-located here.
// Re-exported so existing call sites (scheduledTaskTools.ts,
// scheduledTasks.test.ts, coordinationStore.test.ts) keep importing from
// coordinationStore.js unchanged. Table DDL (CREATE TABLE scheduled_tasks)
// stays in THIS file's ensureCoordinationTable() — one startup migration
// pass for both tables sharing coordination.db.
// ---------------------------------------------------------------------------
export * from "./scheduledTaskStore.js";
