 ...
 * a row 'read' the FIRST time ANY recipient's inbox call touches it (see the
 * UPDATE at the end of getSignals above). For a `to_agent = agent-name` row
 * that is correct (single intended reader). For a `to_agent = 'all'` broadcast
 * row it is a structural bug: the first recipient to bootstrap consumes the
 * row for every OTHER recipient too, because a later unread-only query on the
 * same row now sees status='read' and silently excludes it — confirmed live
 * for chain_catalyst id=8779 (recurring 2026-07-21 08:20Z + 2026-07-22 04:29Z).
 *
 * This helper sidesteps the single-reader-consumed status gate entirely for
 * to_agent='all' rows: it is bounded only by `expires_at` (the row's own TTL),
 * never by `status`, and it does NOT mark rows as read — safe to call from
 * every recipient's bootstrap on every cycle. Callers merge this with the
 * normal getSignals() result and dedupe by id (see getCycleBootstrap.ts) so a
 * broadcast row a given agent is reading for the FIRST time (and which
 * getSignals() already returned + mark-read) is not double-counted.
 *
 * Precedent: getRecentSignals() (all-producers, non-consuming) and
 * getPriceAnomalySignals() (non-consuming) already establish this "read
 * without marking read" pattern elsewhere in this file.
 *
 * @param db - Active bun:sqlite Database connection
 * @returns  Array of hydrated AgentSignal objects (payload parsed from JSON), oldest-first
 */
