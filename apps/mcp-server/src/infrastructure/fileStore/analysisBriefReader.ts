 ...
 */
export function parseLatestPeriod(markdown: string, ticker: string): {
  latest_period: string | null;
  released: string | null;
} {
...
export function parseAnalysisBriefIndexItem(
  ...
  try { exchange = parseExchange(markdown); } catch { /* null */ }
  try {
  ...
  try { confidence = parseConfidence(markdown); } catch { /* null */ }

  return { ticker, exchange, latest_period, released, verdict_label, verdict_summary, confidence, updated_at: updatedAt };
}
 ...
 */
export function readAnalysisBriefIndex(projectRoot: string): AnalysisBriefIndexResponse {
  ...
  for (const filename of filenames) {
    ...
    // Normalise empty updatedAt back to null for the response
