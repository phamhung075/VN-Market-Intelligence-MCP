 ...
 *
 * FACTORY-DOMAIN-extract-sla-config (2026-07-31): the SignalType union,
 * SignalSlaConfig shape, and DEFAULT_SLA_CONFIG threshold table now live in
 * `./freshnessSlaConfig.ts` (pure data, no algorithm) — imported for local
 * use and re-exported unchanged below so every existing import path
 * (domain/services barrel, SLA MCP tools, the freshness monitor scheduler
 * job, and every SLA test file) keeps working with zero call-site changes.
 * This file keeps only the breach-classification ALGORITHM: market-hours /
 * news-quiet-hours / SBV-business-day / BCTC-earnings-window gating,
 * severity classification, and the per-signal + batch check orchestration.
 *
...
import {
  ...
  type SignalType,
} from "./freshnessSlaConfig.js";

export type { SignalSlaConfig, SignalType } from "./freshnessSlaConfig.js";
export { DEFAULT_SLA_CONFIG } from "./freshnessSlaConfig.js";
