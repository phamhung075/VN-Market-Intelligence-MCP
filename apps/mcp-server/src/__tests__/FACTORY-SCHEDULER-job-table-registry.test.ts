 ...
 *
 * Groups:
 *   A — buildJobTable() shape: 57 entries, unique names, cron keys resolve, options valid.
 ...
 *       startScheduler.ts does) yields exactly 79 total scheduleCron registrations with
...
describe("FACTORY-SCHEDULER-job-table-registry — Group A: buildJobTable() shape", () => {
  it("returns exactly 57 entries", async () => {
...
describe("FACTORY-SCHEDULER-job-table-registry — Group B: registerJobTable() generic loop", () => {
  it("registers all 57 buildJobTable() entries via scheduleCron(j.cron, ..., j.options)", async () => {
...
describe("FACTORY-SCHEDULER-job-table-registry — Group D: scheduler-boot smoke (registerJobTable + registerBespokeJobs together)", () => {
  it("registers exactly 79 total scheduleCron calls — reproduces the pre-split registration count exactly", async () => {
    // NOTE: cron EXPRESSION VALUES are not all unique across the 79 registrations (e.g.
    ...
    // resolved string values) is covered by Group A (57 unique buildJobTable names) and
