...
describe("Task 1352a — Group B: bctcReparseJob DPI-300 retry for low-confidence pages", () => {
  ...
  it("B-1: confidence < 0.3 in OCR cache triggers high-DPI retry path in deps", async () => {
    // This test verifies that when pdf-parse confidence < 0.3 AND OCR cache
    // confidence < 0.3, reparseSingleWithOcrFallback returns false (no pipeline).
    // The DPI-300 retry is supposed to be called by bctcReparseJob in this case.
    ...
    const deps: ReparseDeps = {
      ...
      extractText: async () => ({ text: "x".repeat(10), confidence: 0.1 }), // too short + low confidence
      getOcrCache: () => ({
        text: "short",
        pages: 1,
    ...
    // With confidence < 0.3, pipeline should NOT be called
  ...
  it("B-2: confidence >= 0.3 in OCR cache proceeds to pipeline without retry", async () => {
    let pipelineCalled = false;

    const deps: ReparseDeps = {
      fileExists: () => true,
      readFile: () => Buffer.from("fake pdf bytes"),
      extractText: async () => ({ text: "x".repeat(10), confidence: 0.1 }), // pdf-parse fails
      getOcrCache: () => ({
        text: "A".repeat(200), // sufficient text
        pages: 3,
        confidence: 0.45, // >= 0.3 — no retry needed
      }),
      pipeline: async () => {
        pipelineCalled = true;
        return { id: "ok-id" };
