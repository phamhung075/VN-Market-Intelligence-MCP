...
export function buildFinalizeBctcRefineHandler(
  ...
  return async (rawInput) => {
    ...
    try {
      ...
      // BUG 2: FIX-EXTRACTION-CONFIDENCE-NO-RECOMPUTE — recompute from refined section coverage
      //
      // Root cause (brief 2026-06-12-bctc-refine-state-machine-ruling §BUG 2):
      //   extraction_confidence is frozen at OCR-parse time and never updated after
      //   refine/finalize. A successful refine can produce 100+ high-quality rows, but
      //   PUB-5 still blocks publishing because the stale OCR confidence < 0.5.
      //
      ...
      //
      // Runs OUTSIDE the main transaction. Non-fatal (same pattern as BLOCK-1..4).
      // finalRows is already in scope; checkSectionCompleteness is already imported.
      try {
