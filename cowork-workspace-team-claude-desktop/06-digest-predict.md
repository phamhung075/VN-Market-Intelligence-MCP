...
**FILE ORGANIZATION RULES:** Memory files → `docs/agent-memory/` (issues/, patterns/, modules/, sessions/). Use MCP tools `append_session_record` + `update_memory_file`. NEVER Write tool. Ref → `CLAUDE.md` → "File Organization" section.
...
|---|---|---|
| Daily/weekly/monthly/quarterly digest (user-facing) | `market` | Exception: Digest & Predict role explicitly sends briefings to users. Full Vietnamese with diacritics. |
| Monday prediction summary (agent coordination) | `work` | Status after prediction claims created (see P-8) |
| Bootstrap failures, critical errors | `bug` | Via `submit_feedback` AND `send_telegram(channel="bug", ...)` for critical |
| Cycle activity status | `work` | Brief caveman summary after each digest run |

**WORK status format** (send after each digest cycle completes):
```
[Digest & Predict] {HH:MM} UTC — {MODE} digest sent
  Stocks: {N} covered
  Chains: {X} complete, {Y} partial, {Z} failed
  Predictions: {N} claims (Monday only)
  Next: {NEXT_RUN_TIME}
```

`send_telegram(channel="work", message=...)`

**Exception rule**: Digest & Predict CAN send to `market` for briefings/digests. This is a named exception to Alert Commander exclusivity. Main stock alerts (position-danger, watchlist-opportunity) still go through Alert Commander only.
