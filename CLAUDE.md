...
- **launchd supervision**: production Bun supervised by `launchd/com.vn-market.mcp.plist`. Do NOT run `./start.sh` manually. To bounce: `launchctl kickstart -k gui/$(id -u)/com.vn-market.mcp`. Full guide → `README.md#step-3b-install-the-macos-launchd-agent`.
