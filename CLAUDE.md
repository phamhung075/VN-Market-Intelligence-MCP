...
└── scheduler/                      ← 21 files: jobs.ts + summaryJobs.ts + 19 job handlers (see docs/CRON_JOBS.md)
    ...
    ├── vpsProxyWatchdogJob.ts     ← */10 2-8 * * 1-5 UTC: observe market_prices.updated_at freshness; alert Chat if >5 min stale; no SSH
...
| ***/10 min (market hours only)** | `vpsProxyWatchdog` | `*/10 2-8 * * 1-5` (UTC) | Read `MAX(market_prices.updated_at)`. If >5 min stale during VN market hours (Mon-Fri 02:00-08:59 UTC), send one Telegram Chat alert (30-min cooldown). No SSH — observe only. |
...
| **VN stock prices (geo-blocked)** | Vultr VPS Singapore (`139.180.185.18`) → `POST /api/push-prices` | — | — | ✅ systemd-managed, 60s market-hours interval |
...
### VPS Price Proxy — critical invariant (2026-04-06, commit c84a329)

VN stock APIs are geo-blocked from France. A Vultr VPS in Singapore bridges this gap.

**VPS liveness is owned by systemd on the Vultr host (`vn-price-fetch.service`, `Restart=always`). The MCP server only observes `market_prices.updated_at` freshness and alerts on staleness. Nothing on the MCP side ever SSHes into the VPS at runtime.**

- VPS schedule lives inside `vps-scripts/fetch-prices-loop.sh` — NOT in crontab (cron was the failure mode on 2026-03-27).
- MCP watchdog: `src/scheduler/vpsProxyWatchdogJob.ts` — observe-only, 30-min cooldown, sends to Chat Channel.
- Operator escape hatch: `deploy-vps-proxy.sh` (run locally, uploads unit + script, restarts service).
- Do NOT add SSH / sshpass logic to any Bun scheduler. Do NOT reinstate VPS crontab entries.
- Full design detail → `docs/ARCHITECTURE.md` (VPS Price Proxy section).
