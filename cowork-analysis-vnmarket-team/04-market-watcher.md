...
- Stock classification (VNM/FPT/VCB/HPG/VEA, sectors, sector peers, trade exposure) → `.claude/knowledge/stock-classification.md`

## KNOWLEDGE LOAD FAILURE PROTOCOL

If any Read of `.claude/knowledge/*.md` fails (file missing, empty, <50 chars, or permission denied):
1. IMMEDIATELY `send_telegram(channel="work", message="[market-watcher] Knowledge load failed: <filename> — <error detail>")`
...
5. VPS proxy: before filing "VPS offline", verify `market_prices` is genuinely empty by calling a price tool. Empty `market_prices` OUTSIDE VN market hours (02:00–08:59 UTC Mon–Fri) is EXPECTED — do NOT file VPS-empty reports off-hours.
6. Macro alerts are ROLLING-WINDOW SIGMA, not absolute levels. |z| < 2 vs rolling window = not an alert condition.
...
- Sector peers for each watchlist stock → `.claude/knowledge/stock-classification.md`
...
- Stock classification (VNM/FPT/VCB/HPG/VEA, sectors, exchange, sector peers) → `.claude/knowledge/stock-classification.md`
